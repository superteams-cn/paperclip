...
async function inflateZipEntry(compressionMethod: number, bytes: Uint8Array) {
  ...
  if (compressionMethod !== 8) {
    throw new Error(translate("pages.companyImport.zipErrors.unsupportedCompression", {
      defaultValue: "Unsupported zip archive: only STORE and DEFLATE entries are supported.",
    }));
  }
  if (typeof DecompressionStream !== "function") {
    throw new Error(translate("pages.companyImport.zipErrors.browserCannotDecompress", {
...
export async function readZipArchive(source: ArrayBuffer | Uint8Array): Promise<{
  ...
  while (offset + 4 <= bytes.length) {
    ...
    if (signature !== 0x04034b50) {
      throw new Error(translate("pages.companyImport.zipErrors.unsupportedHeader", {
        defaultValue: "Invalid zip archive: unsupported local file header.",
      }));
    }

    if (offset + 30 > bytes.length) {
      throw new Error(translate("pages.companyImport.zipErrors.truncatedHeader", {
    ...
    if ((generalPurposeFlag & 0x0008) !== 0) {
      throw new Error(translate("pages.companyImport.zipErrors.dataDescriptorsUnsupported", {
    ...
    if (bodyEnd > bytes.length) {
      throw new Error(translate("pages.companyImport.zipErrors.truncatedContents", {
