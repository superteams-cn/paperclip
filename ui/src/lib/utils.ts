...
export function relativeTime(date: Date | string): string {
  ...
  if (diffSec < 60) return t("common.time.justNow", { defaultValue: "just now" });
  const diffMin = Math.round(diffSec / 60);
  if (diffMin < 60) return t("common.time.minutesAgo", { defaultValue: "{{n}}m ago", n: diffMin });
  const diffHr = Math.round(diffMin / 60);
  if (diffHr < 24) return t("common.time.hoursAgo", { defaultValue: "{{n}}h ago", n: diffHr });
  const diffDay = Math.round(diffHr / 24);
  if (diffDay < 30) return t("common.time.daysAgo", { defaultValue: "{{n}}d ago", n: diffDay });
