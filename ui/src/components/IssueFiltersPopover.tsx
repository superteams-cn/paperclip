...
export function IssueFiltersPopover({
  ...
  const selectedCreatorOptions = useMemo(
    () => state.creators.map((creatorId) => {
      ...
      return {
        id: creatorId,
        label: formatAssigneeUserLabel(userId, currentUserId, null, assigneeDisplayLabels) ?? userId.slice(0, 5),
        kind: "user" as const,
      };
    }),
      ...
      >
        <div className="space-y-3 p-3">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("pages.issues.assignee", { defaultValue: "Assignee" })}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("pages.issues.noAssignee", { defaultValue: "No assignee" })}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
