...
export function IssueFiltersPopover({
  ...
  const labelForFilterValue = (value: string) =>
  ...
  const selectedCreatorOptions = useMemo(
    () => state.creators.map((creatorId) => {
      ...
      return {
        id: creatorId,
        label: formatAssigneeUserLabel(userId, currentUserId, assigneeDisplayLabels) ?? userId.slice(0, 5),
        kind: "user" as const,
      };
    }),
    [assigneeDisplayLabels, creatorOptionById, currentUserId, state.creators],
  );
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          ...
          title={iconOnly
            ? (activeFilterCount > 0
                ? t("pages.filters.filtersCount", { count: activeFilterCount, defaultValue: "Filters: {{count}}" })
                : t("pages.filters.filter", { defaultValue: "Filter" }))
            : undefined}
        >
      ...
      >
        <div className="space-y-3 p-3">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">{t("pages.filters.filters", { defaultValue: "Filters" })}</span>
            {activeFilterCount > 0 ? (
              <button
                type="button"
              ...
              >
                {t("pages.filters.clear", { defaultValue: "Clear" })}
              </button>
            ) : null}
          </div>

          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("pages.filters.quickFilters", { defaultValue: "Quick filters" })}</span>
            <div className="flex flex-wrap gap-1.5">
              {issueQuickFilterPresets.map((preset) => {
                ...
                return (
                  ...
                  >
                      {t(`pages.filters.${preset.label.toLowerCase()}`, { defaultValue: preset.label })}
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("pages.issues.status", { defaultValue: "Status" })}</span>
                <div className="space-y-0.5">
                  {issueStatusOrder.map((status) => (
                    <label key={status} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{labelForFilterValue(status)}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("pages.issues.priority", { defaultValue: "Priority" })}</span>
                <div className="space-y-0.5">
                  {issuePriorityOrder.map((priority) => (
                    <label key={priority} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{labelForFilterValue(priority)}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("pages.issues.assignee", { defaultValue: "Responsible" })}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("pages.issues.noAssignee", { defaultValue: "No responsible" })}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("pages.issues.me", { defaultValue: "Me" })}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("pages.filters.creator", { defaultValue: "Creator" })}</span>
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("pages.filters.removeCreator", {
                              label: creator.label,
                              defaultValue: "Remove creator {{label}}",
                            })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("pages.filters.searchCreators", { defaultValue: "Search creators..." })}
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">
                        {t("pages.filters.noCreatorsMatch", { defaultValue: "No creators match." })}
                      </div>
                    )}
                  </div>
                </div>
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("pages.issues.project", { defaultValue: "Project" })}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("pages.filters.labels", { defaultValue: "Labels" })}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {labels.map((label) => (
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("pages.issues.workspace", { defaultValue: "Workspace" })}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
              ...
              {enableExternalObjectFilters ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("pages.filters.externalObjectStatus", { defaultValue: "External object status" })}</span>
                  <div className="space-y-0.5">
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("pages.filters.visibility", { defaultValue: "Visibility" })}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{t("pages.filters.liveRunsOnly", { defaultValue: "Live runs only" })}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("pages.filters.hideRoutineRuns", { defaultValue: "Hide routine runs" })}</span>
                  </label>
                ) : null}
              </div>
            </div>
