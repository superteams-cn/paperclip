...
export function SearchFilterSheet({
  ...
  const applyLabel =
    previewTotal === null
      ? t("components.search.searchFilterSheet.showResults", { defaultValue: "Show results" })
      : previewTotal === 1
        ? t("components.search.searchFilterSheet.showResultCountSingular", {
            defaultValue: "Show {{count}} result",
            count: previewTotal,
          })
        : t("components.search.searchFilterSheet.showResultCountPlural", {
      ...
      <SheetContent side="bottom" className="max-h-(--sz-85vh) gap-0 rounded-t-xl p-0" data-testid="search-filter-sheet">
        <SheetHeader className="flex-row items-center justify-between border-b border-border">
          <SheetTitle className="text-base">
            {t("components.search.searchFilterSheet.title", { defaultValue: "Filters" })}
          </SheetTitle>
          <button
            type="button"
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          <ChipToggleGroup
            title={t("components.search.searchFilterSheet.statusTitle", { defaultValue: "Status" })}
            options={options.status}
            selected={draft.status ?? []}
            onToggle={(value) => toggleMulti("status", value)}
          />
          <ChipToggleGroup
            title={t("components.search.searchFilterSheet.priorityTitle", { defaultValue: "Priority" })}
            options={options.priority}
            selected={draft.priority ?? []}
            onToggle={(value) => toggleMulti("priority", value)}
          />
          <ChipToggleGroup
            title={t("components.search.searchFilterSheet.assigneeTitle", { defaultValue: "Assignee" })}
            options={options.assignee}
            selected={selectedAssignee ? [selectedAssignee] : []}
            onToggle={toggleAssignee}
          />
          <ChipToggleGroup
            title={t("components.search.searchFilterSheet.projectTitle", { defaultValue: "Project" })}
            options={options.project}
            selected={draft.projectId ? [draft.projectId] : []}
            onToggle={(value) => toggleSingle("projectId", value)}
          />
          <ChipToggleGroup
            title={t("components.search.searchFilterSheet.labelTitle", { defaultValue: "Label" })}
            options={options.label}
            selected={draft.labelId ? [draft.labelId] : []}
            onToggle={(value) => toggleSingle("labelId", value)}
          />
          <ChipToggleGroup
            title={t("components.search.searchFilterSheet.updatedTitle", { defaultValue: "Updated" })}
          ...
          <div className="space-y-1.5">
            <div className="text-xs font-medium text-muted-foreground">
              {t("components.search.searchFilterSheet.sortBy", { defaultValue: "Sort by" })}
            </div>
            <div className="flex flex-wrap gap-1.5">
...
export function SearchFilterSheetTrigger({
  ...
  return (
    <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs font-normal" onClick={onClick}>
      ...
      {t("components.search.searchFilterSheet.triggerLabel", { defaultValue: "Filters" })}
      {activeCount > 0 ? (
