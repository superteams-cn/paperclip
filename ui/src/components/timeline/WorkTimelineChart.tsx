...
export function formatVisibleDurationMinutes(minutes: number): string {
  ...
  if (rounded >= 7 * 24 * 60 && rounded % (7 * 24 * 60) === 0) {
    ...
    return t("components.timeline.workTimelineChart.weeksVisible", {
      defaultValue: "{{weeks}} weeks visible",
      weeks,
    });
  }
  if (rounded >= 24 * 60 && rounded % (24 * 60) === 0) {
    const days = rounded / (24 * 60);
    return t("components.timeline.workTimelineChart.daysVisible", {
      defaultValue: "{{days}} days visible",
      days,
    });
  }
  if (rounded >= 24 * 60) {
    ...
    return hours > 0
      ? t("components.timeline.workTimelineChart.daysHoursVisible", {
          defaultValue: "{{days}}d {{hours}}h visible",
          days,
          hours,
        })
      : t("components.timeline.workTimelineChart.daysCompactVisible", {
          defaultValue: "{{days}}d visible",
          days,
        });
  }
  if (rounded >= 60 && rounded % 60 === 0) {
    const hours = rounded / 60;
    return t("components.timeline.workTimelineChart.hoursVisible", {
      defaultValue: "{{hours}} hours visible",
      hours,
    });
  }
  if (rounded >= 60) {
    return t("components.timeline.workTimelineChart.hoursMinutesVisible", {
      defaultValue: "{{hours}}h {{minutes}}m visible",
      hours: Math.floor(rounded / 60),
      minutes: rounded % 60,
    });
  }
  return t("components.timeline.workTimelineChart.minutesVisible", {
    defaultValue: "{{minutes}} minutes visible",
    minutes: rounded,
  });
}
...
export function WorkTimelineChart({
  ...
  const connectorHintForBar = (bar: PositionedBar): string | null => {
    ...
    return related.some((c) => c.dashed)
      ? t("components.timeline.workTimelineChart.dashedHandoff", {
          defaultValue: "dashed handoff: retry or changes requested",
        })
      : t("components.timeline.workTimelineChart.solidHandoff", {
          defaultValue: "solid handoff: delegation or assignment",
        });
  };
...
function Tooltip({ tooltip, now }: { tooltip: TooltipState; now: number }) {
  ...
  const title =
    bar.span.issueTitle ??
    bar.span.issueIdentifier ??
    t("components.timeline.workTimelineChart.runFallback", { defaultValue: "run" });
  ...
  return (
    ...
    >
      ...
      <div className="mt-0.5 text-muted-foreground">
        {fmtClock(startMs)}–{bar.span.end ? fmtClock(endMs) : t("components.timeline.workTimelineChart.nowLabel", { defaultValue: "now" })} · {formatDuration(startMs, endMs)} ·{" "}
        <span className="font-medium text-foreground">{bar.span.status}</span>
      </div>
      {bar.kickoff && (
        <div className="text-muted-foreground">
          {t("components.timeline.workTimelineChart.kickedOffBy", {
            defaultValue: "kicked off by: {{name}}",
            name: (bar.kickoff as WorkTimelineActor).name,
          })}
          {bar.span.retryOfRunId
            ? t("components.timeline.workTimelineChart.retrySuffix", { defaultValue: " · retry" })
            : ""}
        </div>
      )}
    ...
    <div className="mt-2 border-t border-border bg-card px-3.5 py-2">
      ...
      >
        ...
        <MiniMapHandle
          ...
          label={t("components.timeline.workTimelineChart.dragLeftHandle", {
            defaultValue: "Drag left edge to resize visible range",
          })}
        ...
        <MiniMapHandle
          ...
          label={t("components.timeline.workTimelineChart.dragRightHandle", {
            defaultValue: "Drag right edge to resize visible range",
          })}
