...
export function IssueSiblingNavigation({ navigation, linkState }: IssueSiblingNavigationProps) {
  ...
  return (
    <nav
      aria-label={t("components.issueSiblingNavigation.ariaLabel", { defaultValue: "Sub-task navigation" })}
...
function SiblingLink({
  ...
  const ariaDirection = direction === "previous"
    ? t("components.issueSiblingNavigation.previousSubIssue", { defaultValue: "Previous sub-task" })
    : t("components.issueSiblingNavigation.nextSubIssue", { defaultValue: "Next sub-task" });
