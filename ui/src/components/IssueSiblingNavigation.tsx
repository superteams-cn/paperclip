...
export function IssueSiblingNavigation({ navigation, linkState }: IssueSiblingNavigationProps) {
  ...
  return (
    <nav
      aria-label={t("components.issueSiblingNavigation.ariaLabel", { defaultValue: "Sub-issue navigation" })}
...
function SiblingLink({
  ...
  const ariaDirection = direction === "previous"
    ? t("components.issueSiblingNavigation.previousSubIssue", { defaultValue: "Previous sub-issue" })
    : t("components.issueSiblingNavigation.nextSubIssue", { defaultValue: "Next sub-issue" });
