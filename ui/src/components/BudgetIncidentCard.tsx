...
function incidentStateLabel(incident: BudgetIncident, t: TFunction) {
  if (incident.status === "resolved") return t("components.budgetIncidentCard.states.resolved", { defaultValue: "Resolved" });
  if (incident.status === "dismissed") return t("components.budgetIncidentCard.states.dismissed", { defaultValue: "Dismissed" });
  if (incident.approvalStatus === "revision_requested") return t("components.budgetIncidentCard.states.escalated", { defaultValue: "Escalated" });
  if (incident.approvalStatus === "pending") return t("components.budgetIncidentCard.states.pendingApproval", { defaultValue: "Pending approval" });
  return t("components.budgetIncidentCard.states.open", { defaultValue: "Open" });
}
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="text-[11px] uppercase tracking-[0.22em] text-red-200/80">
                {t("components.budgetIncidentCard.hardStop", {
                  defaultValue: "{{scope}} hard stop",
                  scope: t(`components.budgetIncidentCard.scopes.${incident.scopeType}`, { defaultValue: incident.scopeType }),
                })}
              </div>
            ...
            <CardDescription className="mt-1 text-red-100/70">
              {t("components.budgetIncidentCard.spendReached", {
        ...
        <div className="flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-50/90">
          ...
          <div>
            {incident.scopeType === "project"
              ? t("components.budgetIncidentCard.projectPaused", { defaultValue: "Project execution is paused. New work in this project will not start until you resolve the budget incident." })
              : t("components.budgetIncidentCard.scopePaused", { defaultValue: "This scope is paused. New heartbeats will not start until you resolve the budget incident." })}
          </div>
        </div>

        <div className="rounded-xl border border-border/60 bg-background/60 p-3">
          <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
            {t("components.budgetIncidentCard.newBudgetUsd", { defaultValue: "New budget (USD)" })}
          </label>
          <div className="mt-2 flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {isMutating
                ? t("components.budgetIncidentCard.applying", { defaultValue: "Applying..." })
                : t("components.budgetIncidentCard.raiseResume", { defaultValue: "Raise budget & resume" })}
            </Button>
          </div>
          {parsed !== null && parsed <= incident.amountObserved ? (
            <p className="mt-2 text-xs text-red-200/80">
              {t("components.budgetIncidentCard.mustExceedSpend", { defaultValue: "The new budget must exceed current observed spend." })}
            </p>
          ) : null}
        </div>
