...
export function IssueAssignedBacklogNotice({
  ...
  const assigneeLabel = assigneeAgent?.name ?? t("pages.issues.assignedBacklog.assigneeFallback", { defaultValue: "the assignee" });
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          <p className="leading-5">
            ...
            {t("pages.issues.assignedBacklog.noticeBeforeStatus", {
              assignee: assigneeLabel,
              defaultValue: "{{assignee}} will not be woken until status changes to",
            })}{" "}
          ...
          {assigneeAgent ? (
            <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
              {t("pages.issues.assignedBacklog.description", { defaultValue: "Comments still wake the assignee for questions or triage. Leave this parked only if the work is intentionally on hold." })}
            </p>
          ) : null}
          {onResume ? (
