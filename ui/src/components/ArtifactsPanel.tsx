...
function statusBadge(status: string) {
  switch (status) {
    case "active":
    case "draft":
      return { label: t("components.artifactsPanel.status.inProgress", { defaultValue: "In Progress" }), className: "bg-blue-500/10 text-blue-600 dark:text-blue-400" };
    case "ready_for_review":
      return { label: t("components.artifactsPanel.status.forReview", { defaultValue: "For Review" }), className: "bg-amber-500/10 text-amber-600 dark:text-amber-400" };
    case "approved":
    case "merged":
      return { label: t("components.artifactsPanel.status.completed", { defaultValue: "Completed" }), className: "bg-green-500/10 text-green-600 dark:text-green-400" };
    case "changes_requested":
      return { label: t("components.artifactsPanel.status.changesRequested", { defaultValue: "Changes Requested" }), className: "bg-orange-500/10 text-orange-600 dark:text-orange-400" };
    case "failed":
      return { label: t("components.artifactsPanel.status.failed", { defaultValue: "Failed" }), className: "bg-red-500/10 text-red-600 dark:text-red-400" };
    default:
  ...
  return (
    <div className="flex flex-col h-full" data-artifacts-panel>
      <div className="px-4 py-3 border-b border-border flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold">{t("components.artifactsPanel.title", { defaultValue: "Artifacts" })}</h3>
      </div>
      ...
      <div className="px-4 py-2 flex flex-wrap gap-1 border-b border-border">
        {FILTERS.map((f) => (
          ...
          >
            {t(`components.artifactsPanel.filters.${f.value}`, { defaultValue: f.label })}
          </button>
        ))}
      </div>
          ...
          <div className="px-4 py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {workProducts?.length === 0
                ? t("components.artifactsPanel.emptyStateNoProducts", { defaultValue: "Your team's deliverables and plans will appear here as they're produced." })
                : t("components.artifactsPanel.emptyStateNoMatch", { defaultValue: "No artifacts match this filter." })}
            </p>
          </div>
        ) : (
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-auto-hide p-4">
        {isLoading ? (
          <div className="flex items-center justify-center py-8 text-muted-foreground text-sm">
            <Loader2 className="h-4 w-4 mr-2 animate-spin" />
            {t("components.artifactsPanel.documentViewer.loading", { defaultValue: "Loading document..." })}
          </div>
        ) : error ? (
          <p className="text-sm text-muted-foreground">{t("components.artifactsPanel.documentViewer.notAvailable", { defaultValue: "Document not available yet." })}</p>
        ) : doc?.body ? (
          <div className="prose prose-sm dark:prose-invert max-w-none">
            <MarkdownBody>{doc.body}</MarkdownBody>
          </div>
        ) : (
          <p className="text-sm text-muted-foreground">{t("components.artifactsPanel.documentViewer.empty", { defaultValue: "Document is empty." })}</p>
        )}
      </div>
      ...
      {needsAction && (
        <div className="border-t border-border px-4 py-3 bg-background shrink-0">
          <p className="text-(length:--text-micro) text-muted-foreground mb-2">{t("components.artifactsPanel.documentViewer.needsReview", { defaultValue: "This document needs your review." })}</p>
          <div className="flex items-center gap-3">
            <Button size="lg" className="h-11 px-8 text-base font-semibold flex-1 rounded-lg bg-green-700 hover:bg-green-800 text-white border-0" onClick={onApprove}>
              {t("components.artifactsPanel.documentViewer.approve", { defaultValue: "Approve" })}
            </Button>
            <Button size="lg" className="h-11 px-8 text-base font-semibold flex-1 rounded-lg bg-red-900 hover:bg-red-950 text-white border-0" onClick={() => {
              ...
              {t("components.artifactsPanel.documentViewer.reject", { defaultValue: "Reject" })}
            </Button>
          </div>
        </div>
      )}
        ...
        <div className="border-t border-green-500/30 bg-green-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-(length:--text-compact) font-medium text-green-700 dark:text-green-400">
              {t("components.artifactsPanel.documentViewer.approved", { defaultValue: "Approved — hire tasks created" })}
            </p>
          </div>
        </div>
      )}
        ...
        <div className="border-t border-orange-500/30 bg-orange-500/5 px-4 py-3 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-(length:--text-compact) font-medium text-orange-700 dark:text-orange-400">
              {t("components.artifactsPanel.documentViewer.changesRequested", { defaultValue: "Changes requested — CEO is revising" })}
            </p>
          </div>
        </div>
      )}
