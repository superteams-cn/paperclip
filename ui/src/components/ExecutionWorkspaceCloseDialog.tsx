      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="break-words">
            {t("components.executionWorkspaceClose.descriptionPrefix")}{" "}
            <span className="font-medium text-foreground">{workspaceName}</span>{" "}
            {t("components.executionWorkspaceClose.descriptionSuffix")}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("components.executionWorkspaceClose.sections.blockingIssues")}</h3>
              ...
              <section className="space-y-2">
                ...
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("components.executionWorkspaceClose.git.branch")}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("common.unknown")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("components.executionWorkspaceClose.git.baseRef")}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("components.executionWorkspaceClose.notSet")}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("components.executionWorkspaceClose.git.mergedIntoBase")}</div>
                      <div>
                    ...
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("components.executionWorkspaceClose.git.aheadBehind")}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("components.executionWorkspaceClose.git.dirtyTracked")}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">{t("components.executionWorkspaceClose.git.untracked")}</div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("components.executionWorkspaceClose.sections.otherIssues")}</h3>
                <div className="space-y-2">
