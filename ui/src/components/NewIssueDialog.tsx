...
const IssueTitleTextarea = memo(function IssueTitleTextarea({
  ...
  return (
    <textarea
      ...
      placeholder={t("pages.issues.newDialog.titlePlaceholder", { defaultValue: "Task title" })}
      rows={1}
      value={draftValue}
      onChange={(e) => {
...
export function NewIssueDialog() {
  ...
  const createIssue = useMutation({
    ...
    onSuccess: ({ issue, companyId, failures }) => {
      ...
      if (failures.length > 0) {
        ...
        pushToast({
          title: t("pages.issues.newDialog.createdWithUploadWarnings", {
            issueRef,
            defaultValue: "Created {{issueRef}} with upload warnings",
          }),
          body: t("pages.issues.newDialog.uploadWarnings", {
            count: failures.length,
            defaultValue: "{{count}} staged file could not be added.",
            defaultValue_other: "{{count}} staged files could not be added.",
          }),
          tone: "warn",
          action: prefix
            ? {
                label: t("pages.issues.newDialog.openIssue", {
                  issueRef,
                  defaultValue: "Open {{issueRef}}",
                }),
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!effectiveCompanyId) throw new Error(t("common.noCompanySelected", { defaultValue: "No company selected" }));
  ...
  const assigneeOptionsTitle =
    assigneeAdapterType === "claude_local"
      ? t("pages.issues.newDialog.claudeOptions", { defaultValue: "Claude options" })
      : assigneeAdapterType === "codex_local"
        ? t("pages.issues.newDialog.codexOptions", { defaultValue: "Codex options" })
        : assigneeAdapterType === "opencode_local"
          ? t("pages.issues.newDialog.openCodeOptions", { defaultValue: "OpenCode options" })
        : t("pages.issues.newDialog.agentOptions", { defaultValue: "Agent options" });
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error
      ...
      : t("pages.issues.newDialog.createError", { defaultValue: "Failed to create task. Try again." });
  ...
  const statusLabel = useCallback(
    (value: string, fallback: string) =>
      t(`pages.issues.statusLabels.${value}`, { defaultValue: fallback }),
    [t],
  );
  const statusDescription = useCallback(
    (value: string, fallback?: string) =>
      fallback
        ? t(`pages.issues.newDialog.statusDescriptions.${value}`, { defaultValue: fallback })
        : undefined,
    [t],
  );
  const priorityLabel = useCallback(
    (value: string, fallback: string) =>
      t(`labels.priority.${value}`, { defaultValue: fallback }),
    [t],
  );
  const workModeLabel = useCallback(
    (value: IssueWorkMode, fallback: string) =>
      t(`pages.issues.newDialog.workModes.${value}`, { defaultValue: fallback }),
    [t],
  );
  const modelLaneLabel = useCallback(
    (lane: IssueModelLane) =>
  ...
  const thinkingEffortLabel = useCallback(
    (value: string, fallback: string) =>
      t(`pages.issues.newDialog.thinkingEfforts.${value || "default"}`, { defaultValue: fallback }),
    [t],
  );
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>
              {isSubIssueMode
                ? t("pages.issues.newDialog.newSubIssue", { defaultValue: "New sub-task" })
                : t("pages.issues.newDialog.newIssue", { defaultValue: "New task" })}
            </span>
          </div>
          ...
          <div className="px-4 pb-2">
            <div className="overflow-x-auto overscroll-x-contain">
              ...
              <span className="w-6 shrink-0 text-center">
                {t("pages.issues.newDialog.for", { defaultValue: "For" })}
              </span>
              <InlineEntitySelector
                ...
                placeholder={t("pages.issues.assignee", { defaultValue: "Assignee" })}
                disablePortal
                noneLabel={t("pages.issues.noAssignee", { defaultValue: "No assignee" })}
                searchPlaceholder={t("pages.issues.searchAssignees", { defaultValue: "Search assignees..." })}
                emptyMessage={t("pages.issues.newDialog.noAssigneesFound", { defaultValue: "No assignees found." })}
                onChange={(value) => {
                  ...
                  return (
                    <>
                      ...
                      {assignee && getTrustPreset(assignee.permissions) === "low_trust_review" ? (
                        <ShieldAlert className="ml-auto h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-300" aria-label={t("pages.issues.newDialog.lowTrustReviewAgent", { defaultValue: "Low-trust review agent" })} />
                      ) : null}
                    </>
                  );
                }}
              />
              <span>{t("pages.issues.newDialog.in", { defaultValue: "in" })}</span>
              <InlineEntitySelector
                ref={projectSelectorRef}
                value={projectId}
                options={projectOptions}
                recentOptionIds={recentProjectIds}
                placeholder={t("pages.issues.project", { defaultValue: "Project" })}
                disablePortal
                noneLabel={t("pages.issues.noProject", { defaultValue: "No project" })}
                searchPlaceholder={t("pages.issues.newDialog.searchProjects", { defaultValue: "Search projects..." })}
                emptyMessage={t("pages.issues.newDialog.noProjectsFound", { defaultValue: "No projects found." })}
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">
                      {t("pages.issues.project", { defaultValue: "Project" })}
                    </span>
                  )
                }
                renderOption={(option) => {
              ...
              <Popover open={participantMenuOpen} onOpenChange={setParticipantMenuOpen}>
                  ...
                  <button
                    ...
                    title={
                      taskWatchdogsEnabled
                        ? t("pages.issues.newDialog.addReviewerApproverOrWatchdog", {
                            defaultValue: "Add reviewer, approver, or watchdog",
                          })
                        : t("pages.issues.newDialog.addReviewerOrApprover", { defaultValue: "Add reviewer or approver" })
                    }
                  >
                ...
                <PopoverContent className="w-44 p-1" align="start">
                  ...
                  >
                    ...
                    {t("pages.issues.newDialog.reviewer", { defaultValue: "Reviewer" })}
                  </button>
                  <button
                    className={cn(
                      "flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50",
                  ...
                  >
                    ...
                    {t("pages.issues.newDialog.approver", { defaultValue: "Approver" })}
                  </button>
                  {taskWatchdogsEnabled && (
                    <button
                      className={cn(
            ...
            {showReviewerRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("pages.issues.newDialog.reviewer", { defaultValue: "Reviewer" })}
                disablePortal
                noneLabel={t("pages.issues.newDialog.noReviewer", { defaultValue: "No reviewer" })}
                searchPlaceholder={t("pages.issues.newDialog.searchReviewers", { defaultValue: "Search reviewers..." })}
                emptyMessage={t("pages.issues.newDialog.noReviewersFound", { defaultValue: "No reviewers found." })}
            ...
            {showApproverRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                options={assigneeOptions}
                recentOptionIds={recentAssigneeOptionIds}
                placeholder={t("pages.issues.newDialog.approver", { defaultValue: "Approver" })}
                disablePortal
                noneLabel={t("pages.issues.newDialog.noApprover", { defaultValue: "No approver" })}
                searchPlaceholder={t("pages.issues.newDialog.searchApprovers", { defaultValue: "Search approvers..." })}
                emptyMessage={t("pages.issues.newDialog.noApproversFound", { defaultValue: "No approvers found." })}
                ...
                <Popover open={watchdogEditorOpen} onOpenChange={setWatchdogEditorOpen}>
                  <PopoverTrigger asChild>
                    <button
                      ...
                      title={t("pages.issues.newDialog.watchdog.configure", { defaultValue: "Configure watchdog" })}
                    >
                      {selectedWatchdogAgent ? (
                        ...
                        <span className="text-muted-foreground">{t("pages.issues.newDialog.watchdog.setWatchdog", { defaultValue: "Set watchdog" })}</span>
                      )}
                    </button>
                  </PopoverTrigger>
                  <PopoverContent className="w-80 p-3 space-y-3" align="start">
                    <div className="space-y-1.5">
                      <div className="text-xs font-medium text-foreground">{t("pages.issues.newDialog.watchdog.agentLabel", { defaultValue: "Watchdog agent" })}</div>
                      <InlineEntitySelector
                        ...
                        renderTriggerValue={(option) =>
                          option ? (
                            ...
                            <span className="text-muted-foreground">{t("pages.issues.newDialog.watchdog.selectAgent", { defaultValue: "Select agent" })}</span>
                          )
                        }
                        renderOption={(option) => {
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Textarea
                        ...
                        placeholder={t("pages.issues.newDialog.watchdog.instructionsPlaceholder", { defaultValue: "What should the watchdog watch for and how should it keep work moving?" })}
                        rows={4}
                        className="text-xs"
                      />
                    </div>
            ...
            <div className="max-w-full rounded-md border border-border bg-muted/30 px-2.5 py-1.5 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                <span className="shrink-0">
                  {t("pages.issues.newDialog.subIssueOf", { defaultValue: "Sub-task of" })}
                </span>
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              <div className="text-xs font-medium">
                {t("pages.issues.newDialog.executionWorkspace", { defaultValue: "Execution workspace" })}
              </div>
              <div className="text-(length:--text-micro) text-muted-foreground">
                {t("pages.issues.newDialog.executionWorkspaceDescription", {
                  defaultValue: "Control whether this task runs in the shared workspace, a new isolated workspace, or an existing one.",
                })}
              </div>
              <select
              ...
              >
                {EXECUTION_WORKSPACE_MODES.map((option) => (
                  <option key={option.value} value={option.value}>
                    {t(`pages.issues.newDialog.executionWorkspaceModes.${option.value}`, { defaultValue: option.label })}
                  </option>
                ))}
              </select>
              {executionWorkspaceMode === "reuse_existing" && (
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-(length:--text-micro) text-muted-foreground">
                  {t("pages.issues.newDialog.reusingWorkspace", {
                    name: selectedReusableExecutionWorkspace.name,
                    source: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("pages.issues.newDialog.existingExecutionWorkspace", { defaultValue: "existing execution workspace" }),
              ...
              {showParentWorkspaceWarning ? (
                <div className="rounded-md border border-amber-300/60 bg-amber-50 px-2 py-1.5 text-(length:--text-micro) text-amber-900 dark:border-amber-800/70 dark:bg-amber-950/30 dark:text-amber-100">
                  {t("pages.issues.newDialog.parentWorkspaceWarning", {
                    workspace: parentExecutionWorkspaceLabel ? ` (${parentExecutionWorkspaceLabel})` : "",
                    defaultValue: "Warning: this sub-task will no longer use the parent task workspace{{workspace}}.",
                  })}
                </div>
              ) : null}
            </div>
              ...
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">
                    {t("pages.issues.newDialog.modelLane", { defaultValue: "Model lane" })}
                  </div>
                  <div
                    className="flex w-full overflow-hidden rounded-md border border-border"
                    role="radiogroup"
                    aria-label={t("pages.issues.newDialog.modelLane", { defaultValue: "Model lane" })}
                  >
                  ...
                  {assigneeModelLane === "primary" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">
                      {t("pages.issues.newDialog.primaryModelHint", { defaultValue: "Runs on the agent's primary model." })}
                    </p>
                  )}
                  {assigneeModelLane === "custom" && (
                    <p className="text-(length:--text-micro) text-muted-foreground">
                      {t("pages.issues.newDialog.customModelHint", { defaultValue: "Override the model and effort for this task only." })}
                    </p>
                  )}
                </div>
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">
                      {t("pages.issues.newDialog.model", { defaultValue: "Model" })}
                    </div>
                    <InlineEntitySelector
                ...
                {assigneeModelLane === "custom" && (
                  <div className="space-y-1.5">
                    <div className="text-xs text-muted-foreground">
                      {t("pages.issues.newDialog.thinkingEffort", { defaultValue: "Thinking effort" })}
                    </div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {thinkingEffortOptions.map((option) => (
                        ...
                        >
                          {thinkingEffortLabel(option.value, option.label)}
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {assigneeAdapterType === "claude_local" && assigneeModelLane === "custom" && (
                  <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
                    <div className="text-xs text-muted-foreground">
                      {t("pages.issues.newDialog.enableChrome", { defaultValue: "Enable Chrome (--chrome)" })}
                    </div>
                    <ToggleSwitch
          ...
          >
            ...
            {stagedFiles.length > 0 ? (
              ...
              {stagedDocuments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">
                    {t("pages.issues.newDialog.documents", { defaultValue: "Documents" })}
                  </div>
                  <div className="space-y-2">
                    {stagedDocuments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          disabled={createIssue.isPending}
                          title={t("pages.issues.newDialog.removeDocument", { defaultValue: "Remove document" })}
                        >
              ...
              {stagedAttachments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">
                    {t("pages.issues.newDialog.attachments", { defaultValue: "Attachments" })}
                  </div>
                  <div className="space-y-2">
                    {stagedAttachments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          onClick={() => removeStagedFile(file.id)}
                          disabled={createIssue.isPending}
                          title={t("pages.issues.newDialog.removeAttachment", { defaultValue: "Remove attachment" })}
                        >
                          <X className="h-3.5 w-3.5" />
                        </Button>
                      </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-t border-border flex-wrap shrink-0">
              ...
              {statuses.map((s) => (
                ...
                >
                  ...
                  <span className="flex flex-col text-left leading-tight">
                    <span>{statusLabel(s.value, s.label)}</span>
                    {s.description ? (
                      <span className="text-(length:--text-nano) text-muted-foreground">{statusDescription(s.value, s.description)}</span>
                    ) : null}
                  </span>
                </button>
              ))}
              ...
              >
                {currentPriority ? (
                  <>
                    ...
                    {priorityLabel(currentPriority.value, currentPriority.label)}
                  </>
                ) : (
                  <>
                    <Minus className="h-3 w-3 text-muted-foreground" />
                    {t("pages.issues.priority", { defaultValue: "Priority" })}
                  </>
                )}
              </button>
            </PopoverTrigger>
          ...
          >
            ...
            {t("pages.issues.newDialog.upload", { defaultValue: "Upload" })}
          </button>
          ...
          <Popover open={workModeOpen} onOpenChange={setWorkModeOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {t(`pages.issues.newDialog.workModesShort.${currentWorkMode.value}`, {
                  defaultValue: currentWorkMode.shortLabel,
                })}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-36 p-1" align="start">
              {workModeOptions.map((option) => {
                ...
                return (
                  ...
                  >
                    ...
                    {workModeLabel(option.value, option.label)}
          ...
          <Popover open={moreOpen} onOpenChange={setMoreOpen}>
            ...
            <PopoverContent className="w-44 p-1" align="start" data-testid="new-issue-more-menu">
              ...
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                ...
                {t("pages.issues.newDialog.startDate", { defaultValue: "Start date" })}
              </button>
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                <Calendar className="h-3 w-3" />
                {t("pages.issues.newDialog.dueDate", { defaultValue: "Due date" })}
              </button>
            </PopoverContent>
          </Popover>
        </div>
        ...
        {assigneeValue && status === "backlog" ? (
          ...
          >
            ...
            <span className="leading-snug">
              <Trans
                i18nKey="pages.issues.newDialog.assignedBacklogNote"
                values={{
                  backlog: statusLabel("backlog", "Backlog"),
                  todo: statusLabel("todo", "Todo"),
                  inProgress: statusLabel("in_progress", "In Progress"),
                }}
                components={{ strong: <span className="font-medium" /> }}
                defaults="Assigning implies executable intent — leave status as <strong>{{backlog}}</strong> only to deliberately park this. The assignee will not be woken until status moves to <strong>{{todo}}</strong> or <strong>{{inProgress}}</strong>."
              />
            </span>
          </div>
        ) : null}
        ...
        {currentAssigneeLowTrust ? (
          ...
          >
            ...
            <span className="leading-snug">
              {t("pages.issues.newDialog.lowTrustAssigneeNote", {
                defaultValue:
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border shrink-0">
          ...
          >
            {t("pages.issues.newDialog.discardDraft", { defaultValue: "Discard Draft" })}
          </Button>
          <div className="flex items-center gap-3">
            <div className="min-h-5 text-right">
              {createIssue.isPending ? (
                <span className="inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                  ...
                  {t("pages.issues.newDialog.creatingIssue", { defaultValue: "Creating issue..." })}
                </span>
              ) : createIssue.isError ? (
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>
                  {createIssue.isPending
                    ? t("pages.issues.newDialog.creating", { defaultValue: "Creating..." })
                    : isSubIssueMode
                      ? t("pages.issues.newDialog.createSubIssue", { defaultValue: "Create Sub-Task" })
                      : t("pages.issues.createIssue", { defaultValue: "Create Task" })}
                </span>
              </span>
            </Button>
          </div>
