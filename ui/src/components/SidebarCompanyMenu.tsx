      ...
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={selectedCompany
            ? t("components.sidebarCompanyMenu.openNamedSwitcher", { name: selectedCompany.name })
            : t("components.sidebarCompanyMenu.openSwitcher")}
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            {selectedCompany ? <WorkspaceIcon company={selectedCompany} /> : null}
            <span className="truncate text-sm font-bold text-foreground">
              {selectedCompany?.name ?? t("components.sidebarCompanyMenu.selectWorkspace")}
            </span>
          </span>
      ...
      <DropdownMenuContent align="start" sideOffset={8} className="w-64 p-1">
        <div className="flex items-center justify-between gap-2 px-2 py-1.5">
          <DropdownMenuLabel className="p-0 text-[11px] font-semibold uppercase text-muted-foreground">
            {t("components.sidebarCompanyMenu.switchWorkspace")}
          </DropdownMenuLabel>
          <button
            type="button"
            onClick={(event) => {
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          {orderedCompanies.length === 0 ? (
            <DropdownMenuItem disabled>{t("components.sidebarCompanyMenu.noWorkspaces")}</DropdownMenuItem>
          ) : null}
        </div>
        <DropdownMenuSeparator />
        <DropdownMenuItem
        ...
        >
          ...
          <span>{t("components.sidebarCompanyMenu.addCompany")}</span>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
