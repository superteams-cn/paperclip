    ...
    >
      ...
      {isEditing ? (
        <button
          ...
          aria-label={t("components.sidebarCompanyMenu.reorder", { name: company.name })}
      ...
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label={selectedCompany
            ? t("components.sidebarCompanyMenu.openNamedSwitcher", { name: selectedCompany.name, defaultValue: "Open {{name}} company switcher" })
            : t("components.sidebarCompanyMenu.openSwitcher", { defaultValue: "Open company switcher" })}
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <span className={cn("truncate text-sm font-bold text-foreground", rail && SIDEBAR_RAIL_HIDDEN_LABEL)}>
              {selectedCompany?.name ?? t("components.sidebarCompanyMenu.selectWorkspace", { defaultValue: "Select company" })}
            </span>
          </span>
      ...
      <DropdownMenuContent align="start" sideOffset={8} className="w-64 p-1">
        <div className="flex items-center justify-between gap-2 px-2 py-1.5">
          <DropdownMenuLabel className="p-0 text-(length:--text-micro) font-semibold uppercase text-muted-foreground">
            {t("components.sidebarCompanyMenu.switchWorkspace", { defaultValue: "Switch company" })}
          </DropdownMenuLabel>
          <button
            type="button"
            onClick={(event) => {
          ...
          >
            {isEditingOrder ? t("components.sidebarCompanyMenu.done") : t("common.edit")}
          </button>
        </div>
        <div className="max-h-96 overflow-y-auto">
          ...
          {orderedCompanies.length === 0 ? (
            <DropdownMenuItem disabled>{t("components.sidebarCompanyMenu.noWorkspaces", { defaultValue: "No companies" })}</DropdownMenuItem>
          ) : null}
        </div>
        <DropdownMenuSeparator />
        <DropdownMenuItem
        ...
        >
          ...
          <span>{t("components.sidebarCompanyMenu.addCompany", { defaultValue: "Create new company..." })}</span>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span className="truncate">
              {selectedCompany
                ? t("components.sidebarCompanyMenu.invitePeopleTo", { name: selectedCompany.name })
                : t("components.sidebarCompanyMenu.invitePeople")}
            </span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild disabled={isEditingOrder}>
          ...
          >
            ...
            <span>{t("components.sidebarCompanyMenu.companySettings")}</span>
          </Link>
        </DropdownMenuItem>
        {session?.session ? (
          <>
            ...
            >
              ...
              <span>{signOutMutation.isPending ? t("settings.general.signingOut") : t("settings.general.signOut")}</span>
            </DropdownMenuItem>
          </>
        ) : null}
      </DropdownMenuContent>
