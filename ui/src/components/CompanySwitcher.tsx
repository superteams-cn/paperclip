  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-medium truncate">
              {selectedCompany?.name ?? t("nav.selectCompany", { defaultValue: "Select company" })}
            </span>
          </div>
      ...
      <DropdownMenuContent align="start" className="w-(--sz-220px)">
        <DropdownMenuLabel>{t("nav.companies", { defaultValue: "Companies" })}</DropdownMenuLabel>
        <DropdownMenuSeparator />
        ...
        {sidebarCompanies.length === 0 && (
          <DropdownMenuItem disabled>{t("nav.noCompanies", { defaultValue: "No companies" })}</DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
        ...
        <DropdownMenuItem asChild>
          <Link to="/companies" className="no-underline text-inherit">
            ...
            {t("nav.manageCompanies", { defaultValue: "Manage Companies" })}
          </Link>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
