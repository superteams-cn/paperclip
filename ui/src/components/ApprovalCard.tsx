...
export function ApprovalCard({
  ...
  const kindLabel = approvalTypeLabel(approval.type, t);
            ...
            <div className="min-w-0 flex-1 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {requesterAgent && (
                  <div className="inline-flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
                    <span>{t("pages.approvalDetail.requestedBy", { defaultValue: "Requested by" })}</span>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-5 text-muted-foreground">
                  {t("components.approvalCard.createdAt", {
        ...
        <div className="shrink-0">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-border/70 bg-background/80 px-2.5 py-1 text-xs text-muted-foreground">
            ...
            <span className="capitalize">
              {t(`components.approvalCard.statuses.${approval.status}`, {
      ...
      {approval.decisionNote && (
        <div className="mt-4 rounded-lg border border-border/60 bg-muted/30 px-3.5 py-3 text-xs leading-5 text-muted-foreground">
          <span className="font-medium text-foreground">
            {t("pages.approvalDetail.decisionNoteLabel", { defaultValue: "Decision note." })}
          </span>{" "}
          {approval.decisionNote}
        </div>
      )}
              ...
              <>
                ...
                >
                  {pendingAction === "approve"
                    ? t("components.approvalCard.approving", { defaultValue: "Approving..." })
                ...
                >
                  {pendingAction === "reject"
                    ? t("components.approvalCard.rejecting", { defaultValue: "Rejecting..." })
