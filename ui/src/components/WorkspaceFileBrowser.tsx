...
export function describeUnavailable(reason: string): { title: string; body: string; icon: ReactNode } {
  ...
  if (lower.includes("remote")) {
    return {
      ...
      title: t("components.workspaceFileBrowser.unavailable.remote.title", { defaultValue: "Remote workspace preview not supported" }),
      body: t("components.workspaceFileBrowser.unavailable.remote.body", { defaultValue: "This workspace is hosted remotely and is not available for inline preview yet." }),
    };
  }
  if (lower.includes("no_workspace") || lower.includes("no_local")) {
    return {
      ...
      title: t("components.workspaceFileBrowser.unavailable.noWorkspace.title", { defaultValue: "No workspace yet" }),
      body: t("components.workspaceFileBrowser.unavailable.noWorkspace.body", { defaultValue: "This issue does not have a workspace to browse. Files appear here once a run creates one." }),
    };
  }
  if (lower.includes("archiv") || lower.includes("cleaned") || lower.includes("unavailable")) {
    return {
      icon: <FolderOpen aria-hidden="true" className="h-5 w-5 text-muted-foreground" />,
      title: t("components.workspaceFileBrowser.unavailable.cleaned.title", { defaultValue: "Workspace is no longer available" }),
      body: t("components.workspaceFileBrowser.unavailable.cleaned.body", { defaultValue: "The isolated worktree for this issue has been cleaned up, so files cannot be previewed." }),
    };
  }
  return {
    icon: <AlertTriangle aria-hidden="true" className="h-5 w-5 text-amber-500" />,
    title: t("components.workspaceFileBrowser.unavailable.generic.title", { defaultValue: "Workspace unavailable" }),
    body: t("components.workspaceFileBrowser.unavailable.generic.body", { defaultValue: "These workspace files can't be browsed right now." }),
  };
}
...
function WorkspaceFileBreadcrumbs({
  ...
  return (
    <nav aria-label={t("components.workspaceFileBrowser.breadcrumbs.ariaLabel", { defaultValue: "Current folder" })} className="min-w-0 overflow-hidden text-(length:--text-micro) text-muted-foreground">
    ...
    >
      ...
      {downloadUrl ? (
        <a
          href={downloadUrl}
          download={name}
          aria-label={t("components.workspaceFileBrowser.row.downloadAriaLabel", { defaultValue: "Download {{name}}", name })}
          title={t("components.workspaceFileBrowser.row.downloadTitle", { defaultValue: "Download {{name}}", name })}
...
function WorkspaceFileTree({
            ...
            <>
              ...
              {loading ? (
                ...
                >
                  ...
                  <span>{t("components.workspaceFileBrowser.tree.loadingFolder", { defaultValue: "Loading folder…" })}</span>
                </div>
              ) : null}
              {truncated ? (
                <button
                ...
                >
                  ...
                  <span>{t("components.workspaceFileBrowser.tree.loadMore", { defaultValue: "Load more from this folder" })}</span>
                </button>
              ) : null}
            </>
          ) : null}
  ...
  return (
    <div role="tree" id={listboxId} aria-label={t("components.workspaceFileBrowser.tree.ariaLabel", { defaultValue: "Workspace files" })} className="space-y-0.5 py-1">
...
export function WorkspaceFileBrowser({
  ...
  const announcement = useMemo(() => {
    if (listQuery.isFetching) return t("components.workspaceFileBrowser.announcement.loading", { defaultValue: "Loading workspace files…" });
    if (listQuery.isError) return t("components.workspaceFileBrowser.announcement.error", { defaultValue: "Unable to load workspace files." });
    if (data?.state === "unavailable") return describeUnavailable(data.unavailableReason ?? "").title;
    if (items.length === 0) return t("components.workspaceFileBrowser.announcement.noMatches", { defaultValue: "No matching files." });
    return t("components.workspaceFileBrowser.announcement.itemsFound", { defaultValue: "{{count}} item{{plural}} found.", count: items.length, plural: items.length === 1 ? "" : "s" });
  ...
  if (source === "other" && !companyId) {
    body = (
      <StateMessage
        icon={<FolderOpen aria-hidden="true" className="h-5 w-5 text-muted-foreground" />}
        title={t("components.workspaceFileBrowser.state.noCompany.title", { defaultValue: "No company selected" })}
        body={t("components.workspaceFileBrowser.state.noCompany.body", { defaultValue: "Choose a company before browsing another project workspace." })}
      />
    );
    ...
    body = (
      <StateMessage
        ...
        title={t("components.workspaceFileBrowser.state.loadingProjects.title", { defaultValue: "Loading project workspaces" })}
        body={t("components.workspaceFileBrowser.state.loadingProjects.body", { defaultValue: "Registered workspaces will appear here." })}
      />
    );
    ...
    body = (
      <StateMessage
        icon={<FolderOpen aria-hidden="true" className="h-5 w-5 text-muted-foreground" />}
        title={t("components.workspaceFileBrowser.state.noProjectWorkspaces.title", { defaultValue: "No project workspaces" })}
        body={t("components.workspaceFileBrowser.state.noProjectWorkspaces.body", { defaultValue: "No same-company project has a registered workspace to browse." })}
      />
    );
    ...
    body = (
      <StateMessage
        icon={<AlertTriangle aria-hidden="true" className="h-5 w-5 text-amber-500" />}
        title={t("components.workspaceFileBrowser.state.loadError.title", { defaultValue: "Couldn't load files" })}
        body={
          status === 404
            ? t("components.workspaceFileBrowser.state.loadError.notAvailable", { defaultValue: "Workspace browsing isn't available for this issue." })
            : t("components.workspaceFileBrowser.state.loadError.generic", { defaultValue: "Something went wrong loading workspace files." })
        }
      />
    );
    ...
    body = (
      <StateMessage
        ...
        title={isSearch
          ? t("components.workspaceFileBrowser.state.empty.noMatch", { defaultValue: "No files match “{{query}}”", query: q })
          : t("components.workspaceFileBrowser.state.empty.noChanges", { defaultValue: "No recently changed files yet" })}
        body={t("components.workspaceFileBrowser.state.empty.body", { defaultValue: "Try searching by name or path." })}
      />
    );
  } else {
    body = (
    ...
    <div className={cn("flex min-h-0 min-w-0 flex-col gap-2", className)}>
      <div className="relative min-w-0 max-w-full overflow-hidden">
        ...
        <Input
          ...
          placeholder={t("components.workspaceFileBrowser.search.placeholder", { defaultValue: "Search files by name or path…" })}
          aria-label={t("components.workspaceFileBrowser.search.ariaLabel", { defaultValue: "Search workspace files" })}
          role="combobox"
      ...
      {lazyTruncatedFolders.has(currentFolderKey) || (data?.truncated && !isLazyBrowse) ? (
        <div className="border-t border-border pt-2 text-(length:--text-micro) text-muted-foreground">
          {isLazyBrowse ? (
            ...
            >
              {t("components.workspaceFileBrowser.footer.loadMore", { defaultValue: "Load more from this folder" })}
            </button>
          ) : (
            <>{t("components.workspaceFileBrowser.footer.showingFirst", { defaultValue: "Showing first {{count}} — refine the search to narrow.", count: items.length })}</>
          )}
        </div>
      ) : null}
    </div>
