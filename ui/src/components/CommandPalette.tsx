...
export function CommandPalette() {
  ...
  return (
    <CommandDialog open={open} onOpenChange={(v) => {
      ...
      <CommandInput
        placeholder={t("command.placeholder", { defaultValue: "Search tasks, agents, projects..." })}
        value={query}
      ...
      <CommandList>
        <CommandEmpty>
          {showSearchAll ? (
            <span>
              {t("command.noQuickMatchesPrefix", { defaultValue: "No quick task matches. Press" })}{" "}
              <kbd className="rounded border border-border bg-muted px-1 py-0.5 text-(length:--text-nano)">↵</kbd>{" "}
              {t("command.noQuickMatchesTo", { defaultValue: "to" })}{" "}
              <span className="font-medium">{t("command.searchAll", { defaultValue: "search all" })}</span>{" "}
              {t("command.noQuickMatchesSuffix", { defaultValue: "or keep typing to refine." })}
            </span>
          ) : (
        ...
        {showSearchAll ? (
          <CommandGroup heading={t("command.searchGroup", { defaultValue: "Search" })}>
            <CommandItem
            ...
            >
              ...
              <span className="flex-1 truncate">
                {t("command.searchAllFor", {
                  query: searchQuery,
                  defaultValue: "Search all for \"{{query}}\"",
                })}
              </span>
              <span className="ml-auto inline-flex items-center gap-1 text-xs text-muted-foreground">
                <span>{t("command.openFullSearch", { defaultValue: "open full search" })}</span>
        ...
        <CommandGroup heading={t("command.actions", { defaultValue: "Actions" })}>
          <CommandItem
            onSelect={() => {
              setOpen(false);
              openNewIssue();
          ...
          >
            ...
            {t("command.createIssue", { defaultValue: "Create new task" })}
          ...
          {onIssueDetail && fileViewerEnabled && (
            ...
            >
              ...
              {t("command.openFileInIssue", { defaultValue: "Open file in this issue..." })}
          ...
          >
            ...
            {t("command.createAgent", { defaultValue: "Create new agent" })}
          </CommandItem>
          <CommandItem onSelect={() => go("/projects")}>
            <Plus className="mr-2 h-4 w-4" />
            {t("command.createProject", { defaultValue: "Create new project" })}
          </CommandItem>
        </CommandGroup>

        <CommandSeparator />

        <CommandGroup heading={t("command.pages", { defaultValue: "Pages" })}>
          ...
          <CommandItem onSelect={() => go("/issues")}>
            ...
            {t("nav.issues", { defaultValue: "Tasks" })}
          </CommandItem>
          <CommandItem onSelect={() => go("/projects")}>
          ...
          <CommandItem onSelect={() => go("/goals")}>
            ...
            {t("nav.goals", { defaultValue: "Goals" })}
          </CommandItem>
          <CommandItem onSelect={() => go("/agents")}>
            <Bot className="mr-2 h-4 w-4" />
            {t("nav.agents", { defaultValue: "Agents" })}
          </CommandItem>
          <CommandItem onSelect={() => go("/costs")}>
            <DollarSign className="mr-2 h-4 w-4" />
            {t("nav.costs", { defaultValue: "Costs" })}
          </CommandItem>
          <CommandItem onSelect={() => go("/activity")}>
            <History className="mr-2 h-4 w-4" />
            {t("nav.activity", { defaultValue: "Activity" })}
          </CommandItem>
        </CommandGroup>
        ...
        {visibleIssues.length > 0 && (
          <>
            <CommandSeparator />
            <CommandGroup heading={t("nav.issues", { defaultValue: "Tasks" })}>
        ...
        {agents.length > 0 && (
          <>
            <CommandSeparator />
            <CommandGroup heading={t("nav.agents", { defaultValue: "Agents" })}>
        ...
        {projects.length > 0 && !showSearchAll && (
          <>
            <CommandSeparator />
            <CommandGroup heading={t("nav.projects", { defaultValue: "Projects" })}>
