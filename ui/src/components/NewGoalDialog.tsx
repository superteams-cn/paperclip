...
export function NewGoalDialog() {
  ...
  const uploadDescriptionImage = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) throw new Error(t("components.newGoal.noCompanySelected"));
  ...
  const levelLabel = (value: string) => t(`components.newGoal.levels.${value}`, { defaultValue: value });
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{newGoalDefaults.parentId ? t("components.newGoal.newSubGoal") : t("components.newGoal.newGoal")}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={t("components.newGoal.titlePlaceholder")}
            value={title}
        ...
        <div className="px-4 pb-2 overflow-y-auto max-h-(--sz-50vh)">
          <MarkdownEditor
            ...
            placeholder={t("components.newGoal.descriptionPlaceholder")}
            bordered={false}
          ...
          <Popover open={parentOpen} onOpenChange={setParentOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                ...
                {currentParent ? currentParent.title : t("components.newGoal.parentGoal")}
              </button>
            </PopoverTrigger>
        ...
        <div className="flex items-center justify-end px-4 py-2.5 border-t border-border">
          ...
          >
            {createGoal.isPending
              ? t("components.newGoal.creating")
              : newGoalDefaults.parentId
                ? t("components.newGoal.createSubGoal")
                : t("components.newGoal.createGoal")}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
