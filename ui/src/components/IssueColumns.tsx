...
export function issueActivityText(issue: Issue, t?: TFunction): string {
  const updatedAt = timeAgo(issue.lastActivityAt ?? issue.lastExternalCommentAt ?? issue.updatedAt);
  return t?.("components.issueColumns.updatedActivity", { time: updatedAt }) ?? `Updated ${updatedAt}`;
}
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={t("components.issueColumns.columns")}
        >
          <Columns3 className={iconOnly ? "h-3.5 w-3.5" : "mr-1 h-3.5 w-3.5"} />
          {!iconOnly && t("components.issueColumns.columns")}
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-(--sz-300px) rounded-xl border-border/70 p-1.5 shadow-xl shadow-black/10">
        <DropdownMenuLabel className="px-2 pb-1 pt-1.5">
          <div className="space-y-1">
            <div className="text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
              {t("components.issueColumns.desktopRows", { defaultValue: "Desktop task rows" })}
            </div>
          ...
          >
            <span className="flex flex-col gap-0.5">
              <span className="text-sm font-medium text-foreground">
                {t(`components.issueColumns.labels.${column}`, { defaultValue: issueColumnLabels[column] })}
              </span>
              <span className="text-xs leading-relaxed text-muted-foreground">
                {t(`components.issueColumns.descriptions.${column}`, { defaultValue: issueColumnDescriptions[column] })}
              </span>
            </span>
          </DropdownMenuCheckboxItem>
        ))}
        ...
        >
          {t("components.issueColumns.resetDefaults")}
          <span className="ml-auto text-xs text-muted-foreground">{t("components.issueColumns.defaultColumnsHint", { defaultValue: "status, id, updated" })}</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
export function InboxIssueTrailingColumns({
  ...
  const userLabel =
    assigneeUserName ?? formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("common.user", { defaultValue: "User" });
  const originatingActor = deriveOriginatingActor(issue);
  const originatingUserId = originatingActor?.kind === "user" ? originatingActor.id : null;
  const creatorUserLabel =
    creatorUserName ?? formatAssigneeUserLabel(originatingUserId, currentUserId) ?? t("common.user", { defaultValue: "User" });
    ...
    >
      {columns.map((column) => {
        if (column === "assignee") {
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">
              {t("pages.routines.groups.unassigned")}
            </span>
          );
        }
        ...
        if (column === "project") {
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">
              {t("pages.routines.groups.noProject")}
            </span>
          );
        }
              ...
              {workspaceId && onFilterWorkspace ? (
                <Tooltip>
                  ...
                  <TooltipContent side="top" sideOffset={6}>
                    {t("components.issueColumns.filterByWorkspace")}
                  </TooltipContent>
                </Tooltip>
              ) : (
                workspaceName
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground" title={parentTitle ?? undefined}>
              {parentIdentifier ? (
                ...
                <span className="italic">{t("components.issueColumns.subIssue", { defaultValue: "Sub-task" })}</span>
              )}
            </span>
          );
        }
