    ...
    const scheduleCheck = () => {
      ...
      timeoutId = window.setTimeout(() => {
        ...
        if (isRichEditorDomEmpty(editable, editorValue, placeholder)) {
          setRichEditorError(t("components.markdownEditor.richLoadFailed", { defaultValue: "Rich editor failed to load content" }));
        }
      }, 0);
    };
      ...
      ? async (file: File) => {
          ...
          try {
            ...
            const message = formatApiError(
              err,
              t,
              t("components.markdownEditor.imageUploadFailed", { defaultValue: "Image upload failed" }),
            );
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between gap-3 px-3 pt-2 text-xs text-muted-foreground">
          <p>{t("components.markdownEditor.richUnavailable", { defaultValue: "Rich editor unavailable for this markdown. Showing raw source instead." })}</p>
          <button
            type="button"
