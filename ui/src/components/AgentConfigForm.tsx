...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  useEffect(() => {
    ...
    props.onTestFeedbackChange({
      errorMessage: testEnvironment.error
        ? formatApiError(testEnvironment.error, t, t("pages.agents.config.environmentTestFailed"))
        : null,
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {environmentsEnabled ? (
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{configLabel("execution", "Execution")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{configLabel("execution", "Execution")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={configLabel("defaultEnvironment", "Default environment")}
              hint={configLabel("defaultEnvironmentHint", "Agent-level default execution target. Project and issue settings can still override this.")}
            >
              <select
                className={inputClass}
              ...
              >
                <option value="">{configLabel("companyDefaultLocal", "Company default (Local)")}</option>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {showInlineAdapterTestEnvironmentFeedback && testEnvironment.error && (
            <div className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              {formatApiError(testEnvironment.error, t, configLabel("environmentTestFailed", "Environment test failed"))}
            </div>
          )}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{configLabel("primaryModel", "Primary model")}</div>
              )}
              <ModelDropdown
                models={models}
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">
                {t("pages.agents.config.comingSoon", { defaultValue: "Coming soon" })}
              </span>
            )}
          </button>
        ))}
    ...
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("pages.agents.config.cheapModel", { defaultValue: "Cheap model" })}</div>
          <p className="text-xs text-muted-foreground">
