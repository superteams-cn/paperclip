  ...
  if (state === "saved") {
    return (
      <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-green-600 dark:text-green-400">
        ...
        {t("components.projectProperties.saved", { defaultValue: "Saved" })}
      </span>
    );
  }
  if (state === "error") {
    return (
      <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-destructive">
        ...
        {t("components.projectProperties.failed", { defaultValue: "Failed" })}
      </span>
    );
  }
  return null;
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {t(`labels.status.${status}`, { defaultValue: status.replace("_", " ") })}
        </button>
      </PopoverTrigger>
...
function ArchiveDangerZone({
  ...
  const action = isArchive
    ? t("components.projectProperties.archive", { defaultValue: "Archive" })
    : t("components.projectProperties.unarchive", { defaultValue: "Unarchive" });

  return (
    <div className="space-y-3 rounded-md border border-destructive/40 bg-destructive/5 px-4 py-4">
      <p className="text-sm text-muted-foreground">
        {isArchive
          ? t("components.projectProperties.archiveDescription", { defaultValue: "Archive this project to hide it from the sidebar and project selectors." })
          : t("components.projectProperties.unarchiveDescription", { defaultValue: "Unarchive this project to restore it in the sidebar and project selectors." })}
      </p>
      {archivePending ? (
        <Button size="sm" variant="destructive" disabled>
          <Loader2 className="h-3 w-3 animate-spin mr-1" />
          {isArchive
            ? t("components.projectProperties.archiving", { defaultValue: "Archiving..." })
            : t("components.projectProperties.unarchiving", { defaultValue: "Unarchiving..." })}
        </Button>
      ) : confirming ? (
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {t("components.projectProperties.confirmArchive", {
          ...
          >
            {t("components.projectProperties.confirm", { defaultValue: "Confirm" })}
          </Button>
          <Button
            size="sm"
            variant="outline"
        ...
        >
          {isArchive ? (
            <><Archive className="h-3 w-3 mr-1" />{t("components.projectProperties.archiveProject", { defaultValue: "Archive project" })}</>
          ) : (
            <><ArchiveRestore className="h-3 w-3 mr-1" />{t("components.projectProperties.unarchiveProject", { defaultValue: "Unarchive project" })}</>
          )}
        </Button>
      )}
    </div>
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("pages.agents.config.selectCompanyToCreateSecrets", { defaultValue: "Select a company to create secrets" }));
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("components.projectProperties.localFolderAbsoluteError", { defaultValue: "Local folder must be a full absolute path." }));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("components.projectProperties.repoUrlError", { defaultValue: "Repo must use a valid GitHub or GitHub Enterprise repo URL." }));
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? t("components.projectProperties.clearLocalConfirm", { defaultValue: "Clear local folder from this workspace?" })
        : t("components.projectProperties.deleteLocalConfirm", { defaultValue: "Delete this workspace local folder?" }),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  const clearRepoWorkspace = () => {
    ...
    const confirmed = window.confirm(
      hasLocalFolder
        ? t("components.projectProperties.clearRepoConfirm", { defaultValue: "Clear repo from this workspace?" })
        : t("components.projectProperties.deleteRepoConfirm", { defaultValue: "Delete this workspace repo?" }),
    );
    if (!confirmed) return;
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("components.projectProperties.name", { defaultValue: "Name" })} state={fieldState("name")} />}>
        ...
        <PropertyRow
          label={<FieldLabel label={t("components.projectProperties.description", { defaultValue: "Description" })} state={fieldState("description")} />}
          alignStart
        ...
        >
          {onUpdate || onFieldUpdate ? (
            ...
            <p className="text-sm text-muted-foreground">
              {project.description?.trim() || t("components.projectProperties.noDescription", { defaultValue: "No description" })}
            </p>
          )}
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("components.projectProperties.status", { defaultValue: "Status" })} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t("components.projectProperties.lead", { defaultValue: "Lead" })}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("components.projectProperties.goals", { defaultValue: "Goals" })} state={fieldState("goals")} />}
          alignStart
                ...
                >
                  ...
                  {(onUpdate || onFieldUpdate) && (
                    <button
                      ...
                      aria-label={t("components.projectProperties.removeGoal", {
            ...
            <Popover open={goalOpen} onOpenChange={setGoalOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {t("components.projectProperties.goal", { defaultValue: "Goal" })}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-56 p-1" align="start">
                {availableGoals.length === 0 ? (
                  <div className="px-2 py-1.5 text-xs text-muted-foreground">
                    {t("components.projectProperties.allGoalsLinked", { defaultValue: "All goals linked." })}
                  </div>
                ) : (
        ...
        <PropertyRow
          label={<FieldLabel label={t("components.projectProperties.env", { defaultValue: "Env" })} state={fieldState("env")} />}
          alignStart
          valueClassName="space-y-2"
        >
          <div className="space-y-2">
            ...
            <p className="text-(length:--text-micro) text-muted-foreground">
              {t("components.projectProperties.envHelp", {
                defaultValue: "Applied to all runs for tasks in this project. Project values override agent env on key conflicts.",
              })}
            </p>
          </div>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("components.projectProperties.created", { defaultValue: "Created" })} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("components.projectProperties.updated", { defaultValue: "Updated" })} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("components.projectProperties.targetDate", { defaultValue: "Target Date" })} state="idle" />}>
      ...
      <div className="space-y-1 py-4">
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span>{t("components.projectProperties.codebase", { defaultValue: "Codebase" })}</span>
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  aria-label={t("components.projectProperties.codebaseHelpLabel", { defaultValue: "Codebase help" })}
                >
                  ?
                </button>
              </TooltipTrigger>
              <TooltipContent side="top">
                {t("components.projectProperties.codebaseHelp", {
          ...
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
                {t("components.projectProperties.repo", { defaultValue: "Repo" })}
              </div>
              {codebase.repoUrl ? (
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    >
                      {t("components.projectProperties.changeRepo", { defaultValue: "Change repo" })}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon-xs"
                      onClick={clearRepoWorkspace}
                      aria-label={t("components.projectProperties.clearRepo", { defaultValue: "Clear repo" })}
                    >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">
                    {t("components.projectProperties.notSet", { defaultValue: "Not set." })}
                  </div>
                  <Button
                    variant="outline"
                    size="xs"
                  ...
                  >
                    {t("components.projectProperties.setRepo", { defaultValue: "Set repo" })}
                  </Button>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1">
              <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
                {t("components.projectProperties.localFolder", { defaultValue: "Local folder" })}
              </div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-(length:--text-micro) text-muted-foreground">
                      {t("components.projectProperties.managedFolder", { defaultValue: "Paperclip-managed folder." })}
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  ...
                  >
                    {codebase.localFolder
                      ? t("components.projectProperties.changeLocalFolder", { defaultValue: "Change local folder" })
                      : t("components.projectProperties.setLocalFolder", { defaultValue: "Set local folder" })}
                  </Button>
                  {codebase.localFolder ? (
                    <Button
                      variant="ghost"
                      ...
                      aria-label={t("components.projectProperties.clearLocalFolder", { defaultValue: "Clear local folder" })}
                    >
                      <Trash2 className="h-3 w-3" />
                    </Button>
                  ) : null}
            ...
            {hasAdditionalLegacyWorkspaces && (
              <div className="text-(length:--text-micro) text-muted-foreground">
                {t("components.projectProperties.legacyWorkspaceNotice", {
                    ...
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <div className="text-(length:--text-micro) text-muted-foreground">
                        {service.url ? (
                          ...
                          service.command ?? t("components.projectProperties.noUrl", { defaultValue: "No URL" })
                        )}
                      </div>
                    </div>
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">
              {t("components.projectProperties.saveWorkspaceFailed", { defaultValue: "Failed to save workspace." })}
            </p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">
              {t("components.projectProperties.deleteWorkspaceFailed", { defaultValue: "Failed to delete workspace." })}
            </p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">
              {t("components.projectProperties.updateWorkspaceFailed", { defaultValue: "Failed to update workspace." })}
            </p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{t("components.projectProperties.executionWorkspaces", { defaultValue: "Execution Workspaces" })}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-border text-(length:--text-nano) text-muted-foreground hover:text-foreground"
                      aria-label={t("components.projectProperties.executionWorkspacesHelpLabel", { defaultValue: "Execution workspaces help" })}
                    >
                      ?
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top">
                    {t("components.projectProperties.executionWorkspacesHelp", {
                      defaultValue: "Project-owned defaults for isolated task checkouts and execution workspace behavior.",
                    })}
                  </TooltipContent>
                </Tooltip>
              </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("components.projectProperties.enableIsolatedCheckouts", { defaultValue: "Enable isolated task checkouts" })}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("components.projectProperties.enableIsolatedCheckoutsHelp", {
                        defaultValue: "Let tasks choose between the project's primary checkout and an isolated execution workspace.",
                      })}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled
                        ? t("labels.status.enabled", { defaultValue: "Enabled" })
                        : t("labels.status.disabled", { defaultValue: "Disabled" })}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("components.projectProperties.defaultIsolatedCheckout", { defaultValue: "New tasks default to isolated checkout" })}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-(length:--text-micro) text-muted-foreground">
                          {t("components.projectProperties.defaultIsolatedCheckoutHelp", {
                            defaultValue: "If disabled, new tasks stay on the project's primary checkout unless someone opts in.",
                          })}
                        </div>
                      </div>
                      <ToggleSwitch
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? t("components.projectProperties.hideAdvancedCheckout", { defaultValue: "Hide advanced checkout settings" })
                          : t("components.projectProperties.showAdvancedCheckout", { defaultValue: "Show advanced checkout settings" })}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("components.projectProperties.hostManagedImplementation", { defaultValue: "Host-managed implementation:" })}{" "}
                          <span className="text-foreground">{t("components.projectProperties.gitWorktree", { defaultValue: "Git worktree" })}</span>
                        </div>
                        {showExecutionWorkspaceEnvironmentControl ? (
                          <div>
                            <div className="mb-1 flex items-center gap-1.5">
                              <label className="flex items-center gap-2 text-xs text-muted-foreground">
                                <span>{t("components.projectProperties.environment", { defaultValue: "Environment" })}</span>
                            ...
                            >
                              <option value="">{t("pages.issues.workspaceCard.noEnvironment", { defaultValue: "No environment" })}</option>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("components.projectProperties.baseRef", { defaultValue: "Base ref" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("components.projectProperties.branchTemplate", { defaultValue: "Branch template" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("components.projectProperties.worktreeParentDir", { defaultValue: "Worktree parent dir" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("components.projectProperties.provisionCommand", { defaultValue: "Provision command" })}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("components.projectProperties.teardownCommand", { defaultValue: "Teardown command" })}</span>
                        ...
                        <p className="text-(length:--text-micro) text-muted-foreground">
                          {t("components.projectProperties.provisionHelp", {
        ...
        <>
          ...
          <div className="space-y-4 py-4">
            <div className="text-xs font-medium text-destructive uppercase tracking-wide">
              {t("components.projectProperties.dangerZone", { defaultValue: "Danger Zone" })}
            </div>
            <ArchiveDangerZone
              project={project}
              onArchive={onArchive}
