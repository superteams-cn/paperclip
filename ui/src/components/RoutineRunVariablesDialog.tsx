    ...
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="flex h-(--sz-calc-18) max-h-(--sz-calc-18) max-w-xl flex-col gap-0 overflow-hidden p-0 sm:h-auto sm:max-h-(--sz-calc-20)">
        <DialogHeader className="shrink-0 border-b border-border/60 px-6 pb-4 pr-12 pt-6">
          ...
          <DialogTitle>{t("components.routineRunVariablesDialog.title")}</DialogTitle>
          <DialogDescription>
            {t("components.routineRunVariablesDialog.description")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1.5">
              <Label className="text-xs">{t("components.routineRunVariablesDialog.agentRequired")}</Label>
              <InlineEntitySelector
                ...
                emptyMessage={t("components.routineRunVariablesDialog.noAgentsFound")}
                disablePortal
                openOnFocus={false}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("components.routineRunVariablesDialog.selectAgent")}</span>
                  )
                }
                renderOption={(option) => {
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">{t("components.routineRunVariablesDialog.project")}</Label>
              <InlineEntitySelector
                ...
                emptyMessage={t("components.routineRunVariablesDialog.noProjectsFound")}
                disablePortal
                openOnFocus={false}
                onChange={(projectId) => {
                ...
                renderTriggerValue={(option) =>
                  option && selectedProject ? (
                    ...
                    <span className="text-muted-foreground">{t("components.routineRunVariablesDialog.noProject")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div key={variable.name} className="space-y-1.5">
              ...
              {isAutoWorkspaceBranchVariable(variable) ? (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__unset__">{t("components.routineRunVariablesDialog.noValue")}</SelectItem>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("components.routineRunVariablesDialog.chooseValue")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("components.routineRunVariablesDialog.noValue")}</SelectItem>
        ...
        >
          {!selection.assigneeAgentId ? (
            <p className="mr-auto text-xs text-amber-600">{t("components.routineRunVariablesDialog.defaultAgentRequired")}</p>
          ) : missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("components.routineRunVariablesDialog.missing", { values: missingRequired.join(", ") })}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("components.routineRunVariablesDialog.chooseWorkspace")}
            </p>
          ) : (
          ...
          >
            {isPending ? t("components.routineRunVariablesDialog.running") : t("components.routineRunVariablesDialog.run")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
