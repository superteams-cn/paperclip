...
export function EnvironmentVariableRow({
  ...
  async function submitSecretPopover(name: string, value: string) {
    ...
    onToast(
      t("components.environmentVariablesEditor.row.secretCreatedToast", {
  ...
  const sourceLabel =
    row.source === "text"
      ? t("components.environmentVariablesEditor.row.textValue", { defaultValue: "Text value" })
      : row.source === "secret"
        ? t("components.environmentVariablesEditor.row.companySecretReference", {
            defaultValue: "Company secret reference",
          })
        : t("components.environmentVariablesEditor.row.userSecretReference", {
            defaultValue: "User secret reference",
          });
  ...
  const versionTagLabel =
    row.version === "latest"
      ? t("components.environmentVariablesEditor.row.latest", { defaultValue: "latest" })
      : `v${row.version}`;
    ...
    >
      ...
      <div className="col-start-1 row-start-1 min-w-0 @[40rem]/env:row-start-1 @[40rem]/env:self-start">
        <input
          ...
          placeholder={t("components.environmentVariablesEditor.row.keyPlaceholder", { defaultValue: "KEY" })}
          value={row.name}
          spellCheck={false}
          disabled={disabled}
          aria-label={t("components.environmentVariablesEditor.row.variableName", { defaultValue: "Variable name" })}
      ...
      <div className="col-span-2 col-start-1 row-start-2 min-w-0 @[40rem]/env:col-span-1 @[40rem]/env:col-start-2 @[40rem]/env:row-start-1">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <DropdownMenuTrigger asChild disabled={disabled}>
                      <button
                        type="button"
                        aria-label={t("components.environmentVariablesEditor.row.valueSource", { defaultValue: "Value source" })}
                ...
                <DropdownMenuContent align="start" className="w-56">
                  <DropdownMenuItem className="flex-col items-start gap-0.5" onSelect={() => switchSource("text")}>
                    ...
                    <span className="text-(length:--text-micro) text-muted-foreground">
                      {t("components.environmentVariablesEditor.row.textValueDescription", {
                        defaultValue: "Store the value inline as plain text.",
                      })}
                    </span>
                  </DropdownMenuItem>
                  <DropdownMenuItem className="flex-col items-start gap-0.5" onSelect={() => switchSource("secret")}>
                    <span className="text-sm">
                      {t("components.environmentVariablesEditor.row.companySecret", { defaultValue: "Company secret" })}
                    </span>
                    <span className="text-(length:--text-micro) text-muted-foreground">
                      {t("components.environmentVariablesEditor.row.companySecretDescription", {
                        defaultValue: "Resolve a stored company secret at run start.",
                      })}
                    </span>
                  </DropdownMenuItem>
                  <DropdownMenuItem className="flex-col items-start gap-0.5" onSelect={() => switchSource("user_secret")}>
                    <span className="text-sm">
                      {t("components.environmentVariablesEditor.row.userSecret", { defaultValue: "User secret" })}
                    </span>
                    <span className="text-(length:--text-micro) text-muted-foreground">
                      {t("components.environmentVariablesEditor.row.userSecretDescription", {
              ...
              {row.source === "text" ? (
                <>
                  <input
                    ...
                    placeholder={t("components.environmentVariablesEditor.row.valuePlaceholder", { defaultValue: "value" })}
                    value={row.textValue}
                    type={sensitive ? "password" : "text"}
                    spellCheck={false}
                    disabled={disabled}
                    aria-label={t("components.environmentVariablesEditor.row.variableValue", { defaultValue: "Variable value" })}
                  ...
                  {sensitive ? (
                    <div className="flex shrink-0 items-stretch border-l border-border">
                      <button
                        ...
                        title={t("components.environmentVariablesEditor.row.sensitiveStoreTitle", {
                          defaultValue: "This value looks sensitive — store it as a secret",
                        })}
                      >
                        <ShieldAlert className="size-3.5" />
                        <span className="hidden @[30rem]/env:inline">
                          {t("components.environmentVariablesEditor.row.storeAsSecret", { defaultValue: "Store as secret" })}
                        </span>
                      </button>
                      <button
                        type="button"
                        ...
                        disabled={disabled}
                        aria-label={t("components.environmentVariablesEditor.row.dismissSensitiveAria", {
                          defaultValue: "Dismiss sensitive-value suggestion",
                        })}
                        title={t("components.environmentVariablesEditor.row.dismissSensitiveTitle", {
                          defaultValue: "Dismiss — keep this value as plain text",
                        })}
                    ...
                    <Popover open={versionOpen} onOpenChange={setVersionOpen}>
                      <PopoverAnchor asChild>
                        <button
                          ...
                          aria-label={t("components.environmentVariablesEditor.row.version", { defaultValue: "Version" })}
                          className={cn(
                      ...
                      <PopoverContent
                        ...
                        className="w-44 p-1"
                        role="radiogroup"
                        aria-label={t("components.environmentVariablesEditor.row.secretVersion", { defaultValue: "Secret version" })}
                      >
                        <button
                          type="button"
                          role="radio"
                        ...
                        >
                          {t("components.environmentVariablesEditor.row.latest", { defaultValue: "latest" })}{" "}
                          <span className="text-(length:--text-micro) text-muted-foreground">
                            {t("components.environmentVariablesEditor.row.recommended", { defaultValue: "(recommended)" })}
                          </span>
                        </button>
                ...
                <div className="grid min-w-0 flex-1 grid-cols-(--gtc-13)">
                  {userSecretsEnabled ? (
                    <select
                      aria-label={t("components.environmentVariablesEditor.row.userSecret", { defaultValue: "User secret" })}
                    ...
                    >
                      <option value="">
                        {t("components.environmentVariablesEditor.row.selectUserSecret", { defaultValue: "Select user secret..." })}
                      </option>
                      {row.userSecretKey && !userSecretDefinitions?.some((definition) => definition.key === row.userSecretKey) ? (
                        <option value={row.userSecretKey}>
                          {t("components.environmentVariablesEditor.row.unknownOption", {
                    ...
                    <input
                      className={valueTextInputClass}
                      placeholder={t("components.environmentVariablesEditor.row.userSecretKeyPlaceholder", {
                        defaultValue: "user-secret key",
                      })}
                      value={row.userSecretKey}
                      spellCheck={false}
                      disabled={disabled}
                      aria-label={t("components.environmentVariablesEditor.row.userSecretKeyAria", { defaultValue: "User secret key" })}
                      onChange={(event) => onPatch({ userSecretKey: event.target.value })}
                    />
                  )}
                  <select
                    aria-label={t("components.environmentVariablesEditor.row.requirement", { defaultValue: "Requirement" })}
                  ...
                  >
                    <option value="required">
                      {t("components.environmentVariablesEditor.row.required", { defaultValue: "Required" })}
                    </option>
                    <option value="optional">
                      {t("components.environmentVariablesEditor.row.optional", { defaultValue: "Optional" })}
                    </option>
                  </select>
                </div>
              )}
        ...
        {undoPrev ? (
          <p className="mt-0.5 inline-flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
            {t("components.environmentVariablesEditor.row.revertedToText", { defaultValue: "Reverted to text —" })}{" "}
            <button
              type="button"
            ...
            >
              {t("components.environmentVariablesEditor.row.undo", { defaultValue: "Undo" })}
            </button>
          </p>
        ) : null}
      </div>
      ...
      <div className="col-start-2 row-start-1 flex items-center justify-end gap-0.5 self-start @[40rem]/env:col-start-3 @[40rem]/env:self-center">
        {row.source === "text" && !sensitive && (row.name.trim() || row.textValue) ? (
          <DropdownMenu>
            <DropdownMenuTrigger asChild disabled={disabled}>
              <button
                type="button"
                aria-label={t("components.environmentVariablesEditor.row.moreActions", { defaultValue: "More actions" })}
            ...
            <DropdownMenuContent align="end">
              ...
              >
                {t("components.environmentVariablesEditor.row.storeAsSecretMenu", { defaultValue: "Store as secret…" })}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ) : null}
        <button
          ...
          onClick={onRemove}
          disabled={disabled}
          aria-label={t("components.environmentVariablesEditor.row.removeAria", {
            defaultValue: "Remove {{name}}",
            name:
              row.name.trim() ||
              t("components.environmentVariablesEditor.row.variableFallback", { defaultValue: "variable" }),
          })}
