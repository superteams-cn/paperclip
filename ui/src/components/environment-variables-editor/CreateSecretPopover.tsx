...
export function SecretPopoverForm({
  ...
  const nameError = (() => {
    if (!trimmedName)
      return touched
        ? t("components.environmentVariablesEditor.createSecretPopover.nameRequired", {
            defaultValue: "Name is required",
          })
        : null;
    if (!SECRET_NAME_RE.test(trimmedName))
      return t("components.environmentVariablesEditor.createSecretPopover.nameFormat", {
        defaultValue: "Use lowercase letters, digits and _",
      });
    if (existingSecretNames?.some((existing) => existing.toLowerCase() === trimmedName)) {
      return t("components.environmentVariablesEditor.createSecretPopover.nameExists", {
        defaultValue: "A secret with this name already exists",
      });
    }
    return null;
  })();
  const valueError =
    value.length === 0
      ? touched
        ? t("components.environmentVariablesEditor.createSecretPopover.valueRequired", {
    ...
    try {
      ...
      setError(
        submitError instanceof Error
          ? submitError.message
          : t("components.environmentVariablesEditor.createSecretPopover.submitError", {
              defaultValue: "Failed to create secret",
            }),
      );
      setSubmitting(false);
    }
  ...
  const ctaLabel =
    mode === "create"
      ? t("components.environmentVariablesEditor.createSecretPopover.createCta", {
          defaultValue: "Create & bind",
        })
      : t("components.environmentVariablesEditor.createSecretPopover.storeCta", {
          defaultValue: "Store & bind",
        });
  const heading =
    mode === "create"
      ? t("components.environmentVariablesEditor.createSecretPopover.createHeading", {
          defaultValue: "Create secret",
        })
      : t("components.environmentVariablesEditor.createSecretPopover.storeHeading", {
          defaultValue: "Store value as secret",
        });
    ...
    <div className="w-72 space-y-3">
      <div className="space-y-1">
        ...
        {mode === "store" ? (
          <PopoverDescription className="text-(length:--text-micro) text-muted-foreground">
            {t("components.environmentVariablesEditor.createSecretPopover.descriptionPrefix", {
              defaultValue: "Moves the typed value into an encrypted company secret and binds",
            })}{" "}
            <span className="font-mono">
              {initialName ||
                t("components.environmentVariablesEditor.createSecretPopover.thisVariable", {
                  defaultValue: "this variable",
                })}
            </span>{" "}
            {t("components.environmentVariablesEditor.createSecretPopover.descriptionSuffix", {
              defaultValue: "to it.",
            })}
          </PopoverDescription>
        ) : null}
      ...
      <label className="block space-y-1">
        <span className="text-(length:--text-micro) font-medium text-muted-foreground">
          {t("components.environmentVariablesEditor.createSecretPopover.nameLabel", { defaultValue: "Name" })}
        </span>
        <input
          ...
          aria-label={t("components.environmentVariablesEditor.createSecretPopover.nameAriaLabel", {
      ...
      <label className="block space-y-1">
        <span className="text-(length:--text-micro) font-medium text-muted-foreground">
          {t("components.environmentVariablesEditor.createSecretPopover.valueLabel", { defaultValue: "Value" })}
        </span>
        <div className="relative">
          <input
            ...
            placeholder={
              mode === "create"
                ? t("components.environmentVariablesEditor.createSecretPopover.valuePlaceholder", {
                    defaultValue: "value",
                  })
                : undefined
            }
            aria-label={t("components.environmentVariablesEditor.createSecretPopover.valueAriaLabel", {
          ...
          <button
            ...
            aria-label={
              reveal
                ? t("components.environmentVariablesEditor.createSecretPopover.hideValue", {
                    defaultValue: "Hide value",
                  })
                : t("components.environmentVariablesEditor.createSecretPopover.showValue", {
