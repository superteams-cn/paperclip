...
const FILTER_LABEL_KEYS: Record<FilterValue, string> = {
  all: "components.activityFeed.filter.all",
  "in-progress": "components.activityFeed.filter.inProgress",
  "for-review": "components.activityFeed.filter.inReview",
  completed: "components.activityFeed.filter.done",
};
...
function CollapsedFeedGroup({
  ...
  const actorName = actor?.name
    ?? (group.latestEvent.actorType === "system" ? t("components.activityFeed.actor.system", { defaultValue: "System" })
      : group.latestEvent.actorType === "user" ? t("components.activityFeed.actor.board", { defaultValue: "Board" })
      : t("components.activityFeed.actor.unknown", { defaultValue: "Unknown" }));
    ...
    <div>
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          ...
          <span data-fc="verb" className="ml-1 text-(--hex-959596)">{t("components.activityFeed.collapsedGroup.updatesTo", { defaultValue: "made {{count}} updates to", count: group.events.length })}</span>
...
export function ActivityFeed({ className }: ActivityFeedProps) {
        ...
        separator = (
          <div className="flex items-center gap-2 px-4 py-1.5" key={`sep-${index}`}>
            ...
            <span className="text-(length:--text-nano) font-medium text-muted-foreground uppercase tracking-wider">
              {t("components.activityFeed.separator.earlier", { defaultValue: "Earlier" })}
            </span>
            <div className="h-px flex-1 bg-border" />
          </div>
        );
  ...
  const renderGrouped = () => {
    ...
    return Array.from(groups.entries()).map(([groupKey, items]) => {
      ...
      const label = isOther
        ? t("components.activityFeed.group.other", { defaultValue: "Other activity" })
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (agents.length === 0) {
      return {
        text: t("components.activityFeed.empty.noAgents", { defaultValue: "No agents set up yet. Add an agent to get started." }),
        showPulse: false,
      };
    }
    ...
    if (allPaused) {
      return {
        text: t("components.activityFeed.empty.allPaused", { defaultValue: "All agents are paused. Resume agents from the sidebar to see activity." }),
        showPulse: false,
      };
    }
    return {
      text: t("components.activityFeed.empty.running", { defaultValue: "Your agents are running — activity will appear here shortly." }),
      showPulse: true,
    };
  }, [agents, t]);
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t("components.activityFeed.groupByTask.ariaLabel", { defaultValue: "group by task" })}
            ...
            <TooltipContent side="bottom">
              {groupMode === "flat"
                ? t("components.activityFeed.groupByTask.tooltipGroup", { defaultValue: "Group by task" })
                : t("components.activityFeed.groupByTask.tooltipFlat", { defaultValue: "Show flat" })}
            </TooltipContent>
          </Tooltip>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={t("components.activityFeed.filterBy.ariaLabel", { defaultValue: "filter by" })}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent side="bottom">{t("components.activityFeed.filterBy.tooltip", { defaultValue: "Filter by" })}</TooltipContent>
            </Tooltip>
              ...
              >
                {FILTER_OPTIONS.map(({ value, label }) => (
                  <DropdownMenuRadioItem key={value} value={value}>
                    {t(FILTER_LABEL_KEYS[value], { defaultValue: label })}
                  </DropdownMenuRadioItem>
                ))}
              </DropdownMenuRadioGroup>
              <DropdownMenuSeparator />
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center p-6 h-full">
            <div className="flex flex-col items-center gap-2 max-w-(--sz-16rem)">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {emptyMessage?.text ?? t("components.activityFeed.empty.fallback", { defaultValue: "Activity from your agents will appear here." })}
              </p>
            </div>
          </div>
        ) : groupMode === "flat" ? (
