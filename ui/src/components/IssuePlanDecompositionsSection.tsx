    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-medium text-muted-foreground">
          {t("components.issuePlanDecompositionsSection.title", { defaultValue: "Plan decomposition" })}
        </h3>
        <span className="text-(length:--text-micro) text-muted-foreground/80">
          {items.length === 1
            ? t("components.issuePlanDecompositionsSection.revisionCountOne", {
                defaultValue: "1 accepted plan revision",
              })
            : t("components.issuePlanDecompositionsSection.revisionCountOther", {
        ...
        {items.map((record) => {
          ...
          const revisionLabel =
            record.acceptedPlanRevisionNumber != null
              ? t("components.issuePlanDecompositionsSection.revisionLabel", {
                  defaultValue: "revision {{number}}",
                  number: record.acceptedPlanRevisionNumber,
                })
              : t("components.issuePlanDecompositionsSection.revisionLabel", {
                  defaultValue: "revision {{number}}",
                  number: record.acceptedPlanRevisionId.slice(0, 8),
                });
          const completedAt =
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {t("components.issuePlanDecompositionsSection.planRevision", {
                ...
                <span className="inline-flex items-center gap-1 text-xs text-foreground">
                  ...
                  {requested === 1
                    ? t("components.issuePlanDecompositionsSection.childTaskCreatedOne", {
                        defaultValue: "{{created}} of {{requested}} child task created",
                        created,
                        requested,
                      })
                    : t("components.issuePlanDecompositionsSection.childTasksCreatedOther", {
                        defaultValue: "{{created}} of {{requested}} child tasks created",
                        created,
                        requested,
                      })}
                </span>
                {record.status === "completed" && requested > 0 ? (
                  <span
                    ...
                    title={t("components.issuePlanDecompositionsSection.idempotentTooltip", {
                      defaultValue:
              ...
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-(length:--text-micro) text-muted-foreground">
                ...
                {completedAt ? (
                  <span title={formatDateTime(completedAt)}>
                    {t("components.issuePlanDecompositionsSection.completedAt", {
