    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-40 p-1" align="end">
        {options.map((opt) => (
          ...
          >
            {formatOption ? formatOption(opt) : fallbackLabel(opt)}
          </Button>
        ))}
      </PopoverContent>
    </Popover>
...
export function GoalProperties({ goal, onUpdate }: GoalPropertiesProps) {
  ...
  const levelLabel = (value: string) => t(`components.newGoal.levels.${value}`, { defaultValue: fallbackLabel(value) });
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("components.goalProperties.status")}>
          {onUpdate ? (
            <PickerButton
              current={goal.status}
        ...
        <PropertyRow label={t("components.goalProperties.level")}>
          {onUpdate ? (
            <PickerButton
              current={goal.level}
              options={GOAL_LEVELS}
              onChange={(level) => onUpdate({ level })}
              formatOption={levelLabel}
            >
              <span className="text-sm">{levelLabel(goal.level)}</span>
            </PickerButton>
          ) : (
            <span className="text-sm">{levelLabel(goal.level)}</span>
          )}
        ...
        <PropertyRow label={t("components.goalProperties.owner")}>
          {ownerAgent ? (
            <Link
        ...
        {goal.parentId && (
          <PropertyRow label={t("components.goalProperties.parentGoal")}>
            <Link
      ...
      <div className="space-y-1">
        <PropertyRow label={t("components.goalProperties.created")}>
          <span className="text-sm">{formatDate(goal.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={t("components.goalProperties.updated")}>
