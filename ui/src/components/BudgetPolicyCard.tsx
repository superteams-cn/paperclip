...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: TFunction) {
  return windowKind === "lifetime"
    ? t("components.budgetPolicy.lifetimeBudget", { defaultValue: "Lifetime budget" })
    : t("components.budgetPolicy.monthlyUtcBudget", { defaultValue: "Monthly UTC budget" });
}
...
export function BudgetPolicyCard({
  ...
  const statusLabel = summary.paused
    ...
    : summary.status === "warning"
      ? t("components.budgetPolicy.warning", { defaultValue: "Warning" })
      : summary.status === "hard_stop"
        ? t("components.budgetPolicy.hardStop", { defaultValue: "Hard stop" })
        : t("components.budgetPolicy.healthy", { defaultValue: "Healthy" });
  const budgetSubtext = summary.amount > 0
    ? t("components.budgetPolicy.percentOfLimit", {
      defaultValue: "{{percent}}% of limit",
      percent: summary.utilizationPercent,
    })
    : t("components.budgetPolicy.noCapConfigured", { defaultValue: "No cap configured" });
  const softAlertText = t("components.budgetPolicy.softAlertAt", {
    defaultValue: "Soft alert at {{percent}}%",
    percent: summary.warnPercent,
  });
  const pauseSuffix = summary.paused && summary.pauseReason
    ? t("components.budgetPolicy.pauseSuffix", { defaultValue: " · {{reason}} pause", reason: summary.pauseReason })
    : "";
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {t("components.budgetPolicy.observed", { defaultValue: "Observed" })}
        </div>
      ...
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {t("components.budgetPolicy.budget", { defaultValue: "Budget" })}
        </div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("labels.status.disabled", { defaultValue: "Disabled" })}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {softAlertText}{pauseSuffix}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("components.budgetPolicy.remaining", { defaultValue: "Remaining" })}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("components.budgetPolicy.unlimited", { defaultValue: "Unlimited" })}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("components.budgetPolicy.projectPaused", { defaultValue: "Execution is paused for this project until the budget is raised or the incident is dismissed." })
          : t("components.budgetPolicy.scopePaused", { defaultValue: "Heartbeats are paused for this scope until the budget is raised or the incident is dismissed." })}
      </div>
    </div>
  ) : null;
    ...
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      <div className="min-w-0 flex-1">
        <label className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          {t("components.budgetPolicy.budgetUsd", { defaultValue: "Budget (USD)" })}
        </label>
        <Input
          value={draftBudget}
      ...
      >
        {isSaving
          ...
          : summary.amount > 0
            ? t("components.budgetPolicy.updateBudget", { defaultValue: "Update budget" })
            : t("components.budgetPolicy.setBudget", { defaultValue: "Set budget" })}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-muted-foreground">
              {t(`labels.scope.${summary.scopeType}`, { defaultValue: summary.scopeType })}
            </div>
        ...
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">
            {t("components.budgetPolicy.invalidAmount", { defaultValue: "Enter a valid non-negative dollar amount." })}
          </p>
        ) : null}
      </div>
    );
