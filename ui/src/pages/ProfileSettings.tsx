...
export function ProfileSettings() {
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("sidebar.account.board", { defaultValue: "Board" });
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(formatApiError(error, t, t("settings.profile.updateError", { defaultValue: "Failed to update profile." })));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("settings.profile.uploadCompanyRequired", { defaultValue: "Select a company before uploading a profile avatar." }));
      }
    ...
    onError: (error) => {
      setActionError(formatApiError(error, t, t("settings.profile.uploadError", { defaultValue: "Failed to upload avatar." })));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(formatApiError(error, t, t("settings.profile.removeError", { defaultValue: "Failed to remove avatar." })));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.profile.loading", { defaultValue: "Loading profile..." })}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {formatApiError(sessionQuery.error, t, t("settings.profile.loadError", { defaultValue: "Failed to load profile." }))}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("sidebar.account.board", { defaultValue: "Board" });
  ...
  const uploadHint = selectedCompany
    ? t("settings.profile.storedInCompany", {
        company: selectedCompany.name,
        defaultValue: "Stored in Paperclip file storage for {{company}}.",
      })
    : t("settings.profile.selectCompanyForAvatar", {
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("settings.profile.title", { defaultValue: "Profile" })}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("settings.profile.description", { defaultValue: "Control how your account appears in the sidebar and other board surfaces." })}
        </p>
      </div>
      ...
      <section className="space-y-8">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    ...
                    {currentImage
                      ...
                      : t("settings.profile.uploadPhoto", { defaultValue: "Upload photo" })}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
              ...
              <div className="min-w-0 flex-1 space-y-2 pb-1">
                <div>
                  ...
                  <p className="truncate text-sm text-muted-foreground">
                    {sessionQuery.data.user.email ?? t("settings.profile.noEmail", { defaultValue: "No email" })}
                  </p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("settings.profile.avatarHint", {
                    hint: uploadHint,
                    defaultValue: "Click the avatar to upload a new image. {{hint}}",
                  })}
                </p>
              </div>
            </div>
        ...
        >
          <div className="space-y-2">
            <Label htmlFor="profile-name">{t("settings.profile.displayName", { defaultValue: "Display name" })}</Label>
            <Input
              id="profile-name"
              value={name}
              onChange={(event) => setName(event.target.value)}
              maxLength={120}
              placeholder={t("sidebar.account.board", { defaultValue: "Board" })}
            />
            <p className="text-xs text-muted-foreground">
              {t("settings.profile.displayNameHint", { defaultValue: "Shown in the sidebar account footer and comment author surfaces." })}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="profile-email">{t("settings.profile.email", { defaultValue: "Email" })}</Label>
            <Input
              id="profile-email"
            ...
            />
            <p className="text-xs text-muted-foreground">
              {t("settings.profile.emailHint", { defaultValue: "Email is managed by your auth session and is read-only here." })}
            </p>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending
                ? t("settings.profile.saving", { defaultValue: "Saving..." })
                : t("settings.profile.saveProfile", { defaultValue: "Save profile" })}
            </Button>
          </div>
        </form>
      </section>
