          ...
          <PopoverContent className="w-44 p-1" align="end">
            ...
            {issueIdProp && (
              ...
              >
                ...
                {t("pages.issues.detail.hideThisIssue", { defaultValue: "Hide this issue" })}
              </button>
            )}
          </PopoverContent>
        </Popover>
          ...
          {!issueCostSummary.hasCost && !issueCostSummary.hasTokens && !hasIssueTreeCost ? (
            ...
            <div className="space-y-1 text-xs text-muted-foreground tabular-nums">
              <div className="flex flex-wrap gap-3">
                <span className="font-medium text-foreground">{t("pages.issues.detail.thisIssue", { defaultValue: "This issue" })}</span>
                ...
                <div className="flex flex-wrap gap-3">
                  <span className="font-medium text-foreground">
                    {t("pages.issues.detail.includingSubIssues", {
                      defaultValue: "Including sub-issues {{amount}}",
                  ...
                  <span>
                    {t("pages.issues.detail.issueCount", {
                      defaultValue: "{{count}} issue",
                      defaultValue_plural: "{{count}} issues",
...
export function IssueDetail() {
  ...
  const updateIssue = useMutation({
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("pages.issues.detail.toasts.issueUpdateFailed", { defaultValue: "Issue update failed" }),
        body: formatApiError(err, t, t("pages.issues.detail.toasts.saveIssueChangesFailed", { defaultValue: "Unable to save issue changes" })),
        tone: "error",
      });
    },
    onSettled: () => {
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        ...
        body: cancelCount > 0
          ...
          : t("pages.issues.treeControl.toasts.workPausedHeld", { defaultValue: "Work paused. This issue is held until resume." }),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.issues.detail(issueId!) }),
  ...
  const updateChildIssue = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("pages.issues.detail.toasts.issueUpdateFailed", { defaultValue: "Issue update failed" }),
        body: formatApiError(err, t, t("pages.issues.detail.toasts.saveSubIssueChangesFailed", { defaultValue: "Unable to save sub-issue changes" })),
        tone: "error",
      });
    },
  });
  ...
  const archiveFromInbox = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("pages.issues.detail.toasts.issueArchivedFromInbox", { defaultValue: "Issue archived from inbox" }), tone: "success" });
    },
    onError: (err) => {
      pushToast({
        title: t("pages.issues.detail.toasts.archiveFailed", { defaultValue: "Archive failed" }),
        body: formatApiError(err, t, t("pages.issues.detail.toasts.archiveFromInboxFailed", { defaultValue: "Unable to archive this issue from the inbox" })),
        tone: "error",
      });
    },
  });
    ...
    <div className="max-w-3xl space-y-6">
      ...
      {issue.hiddenAt && (
        <div className="flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          ...
          {t("pages.issues.detail.issueHidden", { defaultValue: "This issue is hidden" })}
        </div>
      )}
      {activePauseHold && (
        <div className="rounded-md border border-amber-500/35 bg-amber-500/10 p-3 text-sm text-amber-800 dark:text-amber-200">
          {activePauseHold.isRoot ? (
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-amber-900/80 dark:text-amber-100/80">
                  {childIssues.length === 0
                    ? t("pages.issues.detail.issueHeldUntilResume")
                    : t("pages.issues.detail.subtreeHeldUntilResume")}
                </span>
              </div>
              <div className="text-xs text-amber-900/80 dark:text-amber-100/80">
                {childIssues.length === 0
                  ? t("pages.issues.detail.oneIssueHeld")
                  : t("pages.issues.detail.descendantsHeld", { count: heldDescendantCount })}
                {activeRootPauseHold?.createdAt ? t("pages.issues.detail.pauseStarted", { time: relativeTime(activeRootPauseHold.createdAt) }) : ""}
              </div>
            ...
            <div className="text-xs">
              {t("pages.issues.detail.pausedByAncestorPrefix", { defaultValue: "This issue is paused by ancestor" })}{" "}
              ...
              {t("pages.issues.detail.pausedByAncestorSuffix", { defaultValue: ". Resume from the root issue to deliver deferred work." })}
            </div>
          )}
        </div>
      )}
        ...
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          {issue.workMode === "planning" ? (
            <span
              ...
              title={t("pages.issues.detail.planningModeTitle", { defaultValue: "This issue is in planning mode." })}
            >
              {t("pages.issues.detail.planning", { defaultValue: "Planning" })}
            </span>
          ) : null}
          ...
          {!(isMobile && isFromInbox) && (
            <div className="ml-auto flex items-center gap-0.5 md:hidden shrink-0">
              <Button
                ...
                title={t("pages.issues.detail.copyIssueAsMarkdown", { defaultValue: "Copy issue as markdown" })}
              >
            ...
            <Popover open={moreOpen} onOpenChange={setMoreOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  size="icon-xs"
                  className="shrink-0"
                  aria-label={t("pages.issues.detail.moreIssueActions", { defaultValue: "More issue actions" })}
                  title={t("pages.issues.detail.moreIssueActions", { defaultValue: "More issue actions" })}
                  onKeyDown={(event) => {
      ...
      {showRichSubIssuesSection ? (
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-sm font-medium text-muted-foreground">{t("pages.issues.properties.subIssues", { defaultValue: "Sub-issues" })}</h3>
          </div>
          <IssuesList
            issues={childIssues}
            ...
            createIssueLabel={t("pages.issues.subIssue", { defaultValue: "Sub-issue" })}
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
          <Button variant="outline" size="sm" onClick={openNewSubIssue} className="shrink-0 shadow-none">
            ...
            {t("pages.issues.detail.newSubIssue", { defaultValue: "New Sub-issue" })}
          </Button>
        </div>
      )}
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-sm font-medium text-muted-foreground">{t("pages.issues.detail.attachments", { defaultValue: "Attachments" })}</h3>
            ...
            {imageAttachments.map((attachment) => (
              ...
              >
                ...
                {confirmDeleteId === attachment.id ? (
                  ...
                  >
                    <p className="text-xs text-white font-medium">{t("pages.issues.detail.deleteQuestion", { defaultValue: "Delete?" })}</p>
                  ...
                  <button
                    ...
                    title={t("pages.issues.detail.deleteAttachment", { defaultValue: "Delete attachment" })}
                  >
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
            {treeControlMode === "cancel" ? (
              <div className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive">
                {t("pages.issues.treeControl.cancelWarning", { defaultValue: "Cancelling a subtree is destructive. Non-terminal issues will be marked cancelled, and running or queued work will be interrupted where possible." })}
              </div>
            ) : null}
                  ...
                  <span>
                    ...
                    <span className="text-xs text-muted-foreground">
                      {previewAffectedAgentCount === 0
                        ? t("pages.issues.treeControl.noAgentsToWake", { defaultValue: "No assigned agents are eligible to wake from this preview." })
                        : t("pages.issues.treeControl.wakeAfterComplete", { defaultValue: "Wake assigned agents after this operation completes." })}
                    </span>
                  </span>
                </label>
