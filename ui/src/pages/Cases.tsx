...
const STATUS_FILTER_OPTIONS: { value: CaseStatus; label: string }[] = [
  { value: "draft", label: t("pages.cases.status.draft", { defaultValue: "Draft" }) },
  { value: "in_progress", label: t("pages.cases.status.inProgress", { defaultValue: "In progress" }) },
  { value: "in_review", label: t("pages.cases.status.inReview", { defaultValue: "In review" }) },
  { value: "approved", label: t("pages.cases.status.approved", { defaultValue: "Approved" }) },
  { value: "done", label: t("pages.cases.status.done", { defaultValue: "Done" }) },
  { value: "cancelled", label: t("pages.cases.status.cancelled", { defaultValue: "Cancelled" }) },
];
...
const CASE_COLUMN_LABELS: Record<CaseColumn, string> = {
  id: t("pages.cases.column.id", { defaultValue: "ID" }),
  key: t("pages.cases.column.key", { defaultValue: "Key" }),
  title: t("pages.cases.column.title", { defaultValue: "Title" }),
  status: t("pages.cases.column.status", { defaultValue: "Status" }),
  updated: t("pages.cases.column.updated", { defaultValue: "Updated" }),
  created: t("pages.cases.column.created", { defaultValue: "Created at" }),
  type: t("pages.cases.column.type", { defaultValue: "Type" }),
  project: t("pages.cases.column.project", { defaultValue: "Project" }),
  parent: t("pages.cases.column.parent", { defaultValue: "Parent case" }),
};
const CASE_SORT_LABELS: Record<CaseSortField, string> = {
  updated: t("pages.cases.sort.updated", { defaultValue: "Last updated" }),
  created: t("pages.cases.sort.created", { defaultValue: "Created at" }),
  title: t("pages.cases.sort.title", { defaultValue: "Title" }),
  status: t("pages.cases.sort.status", { defaultValue: "Status" }),
  id: t("pages.cases.sort.id", { defaultValue: "ID" }),
  type: t("pages.cases.sort.type", { defaultValue: "Type" }),
  project: t("pages.cases.sort.project", { defaultValue: "Project" }),
};
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("pages.cases.changeStatusAriaLabel", { defaultValue: "Change case status" })}
          onClick={(event) => {
        ...
        if (column === "id") {
          return (
            <CaseCopyableToken
              ...
              label={t("pages.cases.caseIdTokenLabel", { defaultValue: "case ID" })}
        ...
        if (column === "key") {
          return row.key ? (
            <CaseCopyableToken
              key={column}
              value={row.key}
              label={t("pages.cases.caseKeyTokenLabel", { defaultValue: "case key" })}
              className="font-mono text-xs text-muted-foreground"
              stopPropagation
            />
          ) : (
                  ...
                  {childCount > 0 ? (
                    <button
                      ...
                      aria-label={treeCollapsed
                        ? t("pages.cases.expandRowAriaLabel", { defaultValue: "Expand {{title}}", title: row.title })
                        : t("pages.cases.collapseRowAriaLabel", { defaultValue: "Collapse {{title}}", title: row.title })}
        ...
        <IssueGroupHeader
          ...
          trailing={(
            <span className="text-xs text-muted-foreground tabular-nums">
              {count} {count === 1
                ? t("pages.cases.caseUnitSingular", { defaultValue: "case" })
                : t("pages.cases.caseUnitPlural", { defaultValue: "cases" })}
            </span>
          )}
        />
      </div>
...
function CaseColumnPicker({
  ...
  const { t } = useTranslation();
  return (
    <CaseToolbarButton icon={Columns3} title={t("pages.cases.columnsButton", { defaultValue: "Columns" })} active={!sameStringSet([...visibleColumns], DEFAULT_CASE_COLUMNS)}>
...
function CaseSortPicker({
  ...
  const { t } = useTranslation();
  return (
    <CaseToolbarButton icon={ArrowUpDown} title={t("pages.cases.sortButton", { defaultValue: "Sort" })} active={sortField !== "updated" || sortDir !== "desc"}>
...
function CasesEmptyHero() {
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col items-center gap-4 py-16 text-center">
      ...
      <h2 className="text-lg font-semibold">{t("pages.cases.emptyHeroTitle", { defaultValue: "No cases yet" })}</h2>
      <p className="text-sm text-muted-foreground">
        {t("pages.cases.emptyHeroDescription", { defaultValue: "Cases are durable work products — blog posts, tweet storms, docs pages — that tasks create and iterate on. In v1 they're created by agents, not from the UI." })}
      </p>
...
{slug, target_audience, publish_url} and key <release>/<slug>."`}
      ...
      <p className="text-xs text-muted-foreground">
        {t("pages.cases.emptyHeroFlagPrefix", { defaultValue: "Feature is gated by the " })}<code className="font-mono">enableCases</code>{t("pages.cases.emptyHeroFlagSuffix", { defaultValue: " experimental flag (Settings → Experimental)." })}
      </p>
    </div>
  );
}
...
export function Cases() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.cases.breadcrumb", { defaultValue: "Cases" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  const groupedRows = useMemo((): CaseGroupedRows[] => {
    ...
    for (const c of sorted) {
      ...
      if (viewState.groupBy === "type") key = c.caseType;
        ...
        ? projectName.get(c.projectId) ?? t("pages.cases.unknownProject", { defaultValue: "Unknown project" })
    ...
    return [...map.entries()]
      ...
      .map(([key, rows]) => ({
        key,
        label: viewState.groupBy === "status"
          ? STATUS_FILTER_OPTIONS.find((option) => option.value === key)?.label ?? key
          : key,
        rows,
      }));
  ...
  if (viewState.search.trim()) activeFilters.push({ key: "search", label: t("pages.cases.filterLabelSearch", { defaultValue: "Search" }), value: viewState.search.trim() });
  if (viewState.typeFilters.length > 0) {
    activeFilters.push({ key: "type", label: t("pages.cases.filterLabelType", { defaultValue: "Type" }), value: viewState.typeFilters.join(", ") });
  }
  if (!usesDefaultStatusFilter) {
    activeFilters.push({
      key: "status",
      label: t("pages.cases.filterLabelStatus", { defaultValue: "Status" }),
      value: viewState.statusFilters.length === CASE_STATUSES.length
        ? t("pages.cases.filterValueAll", { defaultValue: "All" })
        : viewState.statusFilters
  ...
  if (viewState.projectFilters.length > 0) {
    activeFilters.push({
      key: "project",
      label: t("pages.cases.filterLabelProject", { defaultValue: "Project" }),
      value: viewState.projectFilters
        .map((projectId) => projectId === ALL ? t("pages.cases.noProject", { defaultValue: "No project" }) : projectName.get(projectId) ?? t("pages.cases.filterValueProjectFallback", { defaultValue: "Project" }))
        .join(", "),
    });
  }
  if (viewState.labelFilter !== ALL) {
    const name = (labelsQuery.data ?? []).find((l) => l.id === viewState.labelFilter)?.name ?? t("pages.cases.filterValueLabelFallback", { defaultValue: "Label" });
    activeFilters.push({ key: "label", label: t("pages.cases.filterLabelLabel", { defaultValue: "Label" }), value: name });
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold">{t("pages.cases.heading", { defaultValue: "Cases" })}</h1>
          <Badge variant="secondary">{t("pages.cases.experimentalBadge", { defaultValue: "Experimental" })}</Badge>
        </div>
      </div>
      ...
      {noCasesAtAll ? (
        ...
        <>
          <div className="flex items-center justify-between gap-2 sm:gap-3">
            <div className="relative w-48 sm:w-64 md:w-80">
              ...
              <Input
                ...
                aria-label={t("pages.cases.searchAriaLabel", { defaultValue: "Search cases" })}
            ...
            <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
              <Button
                ...
                title={viewState.treeView ? t("pages.cases.showFlatList", { defaultValue: "Show flat case list" }) : t("pages.cases.showTree", { defaultValue: "Show parent/children tree" })}
                aria-label={viewState.treeView ? t("pages.cases.showFlatList", { defaultValue: "Show flat case list" }) : t("pages.cases.showTree", { defaultValue: "Show parent/children tree" })}
              ...
              <CaseToolbarButton icon={Filter} title={t("pages.cases.filtersButton", { defaultValue: "Filters" })} active={hasActiveFilters}>
                <PopoverContent align="end" className="w-72 p-3">
                  <div className="grid gap-3">
                    <FilterField label={t("pages.cases.filterFieldType", { defaultValue: "Type" })}>
                    ...
                    <FilterField label={t("pages.cases.filterFieldStatus", { defaultValue: "Status" })}>
                      <div>
                        {STATUS_FILTER_OPTIONS.map((option) => (
                          <FilterCheckboxRow
                            key={option.value}
                    ...
                    <FilterField label={t("pages.cases.filterFieldProject", { defaultValue: "Project" })}>
                      <div className="max-h-40 overflow-y-auto">
                        <FilterCheckboxRow
                    ...
                    <FilterField label={t("pages.cases.filterFieldLabel", { defaultValue: "Label" })}>
                      <div className="max-h-40 overflow-y-auto">
                        <FilterCheckboxRow
              ...
              <CaseToolbarButton icon={Layers} title={t("pages.cases.groupButton", { defaultValue: "Group" })} active={viewState.groupBy !== "type"}>
                <PopoverContent align="end" className="w-44 p-2">
                  {([
                    ["type", t("pages.cases.groupByType", { defaultValue: "Type" })],
                    ["project", t("pages.cases.groupByProject", { defaultValue: "Project" })],
                    ["status", t("pages.cases.groupByStatus", { defaultValue: "Status" })],
                    ["none", t("pages.cases.groupByNone", { defaultValue: "None" })],
          ...
          {filtered.length === 0 ? (
            <EmptyState icon={SearchX} message={t("pages.cases.noMatchingCases", { defaultValue: "No cases match these filters." })} action={t("pages.cases.clearFilters", { defaultValue: "Clear filters" })} onAction={clearFilters} />
          ) : (
            <div ref={caseListRef}>
