...
function surfaceOptions(t: TFunction): Array<{
  id: SurfaceId;
  label: string;
  eyebrow: string;
  description: string;
  ...
  return [
    {
      id: "detail",
      label: t("pages.uxLabs.runTranscript.surfaces.detail.label"),
      eyebrow: t("pages.uxLabs.runTranscript.surfaces.detail.eyebrow"),
      description: t("pages.uxLabs.runTranscript.surfaces.detail.description"),
      icon: MonitorCog,
    },
    {
      id: "live",
      label: t("pages.uxLabs.runTranscript.surfaces.live.label"),
      eyebrow: t("pages.uxLabs.runTranscript.surfaces.live.eyebrow"),
      description: t("pages.uxLabs.runTranscript.surfaces.live.description"),
      icon: RadioTower,
    },
    {
      id: "dashboard",
      label: t("pages.uxLabs.runTranscript.surfaces.dashboard.label"),
      eyebrow: t("pages.uxLabs.runTranscript.surfaces.dashboard.eyebrow"),
      description: t("pages.uxLabs.runTranscript.surfaces.dashboard.description"),
      icon: PanelsTopLeft,
    },
  ];
}
      ...
      <div className="border-b border-border/60 bg-background/90 px-5 py-4">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant="outline" className="uppercase tracking-(--tracking-caps) text-(length:--text-nano)">
            {t("pages.uxLabs.runTranscript.surfaces.detail.label")}
          </Badge>
        ...
        <div className="mt-2 text-sm font-medium">
          {t("pages.uxLabs.runTranscript.transcriptCount", { count: runTranscriptFixtureEntries.length })}
        </div>
      </div>
    ...
    <div className="overflow-hidden rounded-xl border border-cyan-500/25 bg-background/85 shadow-[0_20px_50px_rgba(6,182,212,0.10)]">
      <div className="border-b border-border/60 bg-cyan-500/[0.05] px-5 py-4">
        <div className="text-xs font-semibold uppercase tracking-(--tracking-caps) text-cyan-700 dark:text-cyan-300">
          {t("pages.uxLabs.runTranscript.liveRuns")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("pages.uxLabs.runTranscript.liveRunsDescription")}
        </div>
      </div>
      <div className="px-5 py-4">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Badge variant="outline" className="border-border/70 bg-background/70 px-2.5 py-1 text-(length:--text-micro) text-muted-foreground">
            {t("pages.uxLabs.runTranscript.openRun")}
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-2 text-(length:--text-micro) text-muted-foreground">
                {streaming ? t("pages.uxLabs.runTranscript.liveNow") : t("pages.uxLabs.runTranscript.finishedAgo")}
              </div>
            </div>
...
export function RunTranscriptUxLab() {
  ...
  const options = surfaceOptions(t);
  const selected = options.find((option) => option.id === selectedSurface) ?? options[0];
          ...
          <aside className="border-b border-border/60 bg-background/75 p-5 lg:border-b-0 lg:border-r">
            <div className="mb-5">
              <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/25 bg-cyan-500/[0.08] px-3 py-1 text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-cyan-700 dark:text-cyan-300">
                ...
                {t("pages.uxLabs.common.uxLab")}
              </div>
              <h1 className="mt-4 text-2xl font-semibold tracking-tight">
                {t("pages.uxLabs.runTranscript.title")}
              </h1>
              <p className="mt-2 text-sm text-muted-foreground">
                {t("pages.uxLabs.runTranscript.description")}
              </p>
            </div>
          ...
          <main className="min-w-0 p-5">
            <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Badge variant="outline" className="rounded-full px-3 py-1 text-(length:--text-nano) uppercase tracking-(--tracking-caps)">
                  {t("pages.uxLabs.runTranscript.sourceRun", {
                    id: runTranscriptFixtureMeta.sourceRunId.slice(0, 8),
                  })}
                </Badge>
                <Badge variant="outline" className="rounded-full px-3 py-1 text-(length:--text-nano) uppercase tracking-(--tracking-caps)">
            ...
            <div className="mb-5 flex flex-wrap items-center gap-2">
              <span className="text-(length:--text-micro) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
                {t("pages.uxLabs.runTranscript.controls")}
              </span>
              <div className="inline-flex rounded-full border border-border/70 bg-background/80 p-1">
                {(["nice", "raw"] as const).map((mode) => (
                  ...
                  >
                    {t(`pages.uxLabs.runTranscript.modes.${mode}`)}
                  </button>
                ))}
              </div>
              <div className="inline-flex rounded-full border border-border/70 bg-background/80 p-1">
                {(["comfortable", "compact"] as const).map((nextDensity) => (
                  ...
                  >
                    {t(`pages.uxLabs.runTranscript.densities.${nextDensity}`)}
                  </button>
                ))}
              </div>
              <Button
              ...
              >
                {streaming
                  ? t("pages.uxLabs.runTranscript.showSettledState")
                  : t("pages.uxLabs.runTranscript.showStreamingState")}
              </Button>
            </div>
