...
import { formatCompanyPortabilityWarning } from "../lib/api-feedback-format";
...
function ExportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("pages.companyExport.selectFile")} />
    );
  }
      ...
      <div className="min-h-(--sz-560px) px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
            {t("pages.companyExport.binaryPreviewUnavailable")}
          </div>
        )}
      </div>
    </div>
...
export function CompanyExport() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("pages.orgChart.title"), href: "/org" },
      { label: t("pages.companyExport.export") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("pages.companyExport.toasts.exportFailed"),
        body: formatApiError(err, t, t("pages.companyExport.toasts.loadFailedBody")),
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("pages.companyExport.toasts.downloaded"),
        body: t("pages.companyExport.toasts.downloadedBody", {
          count: resultCheckedFiles.size,
          fileName: `${result.rootPath}.zip`,
        }),
      });
    },
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("pages.companyExport.toasts.exportFailed"),
        body: formatApiError(err, t, t("pages.companyExport.toasts.buildFailedBody")),
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message={t("pages.companyExport.selectCompany")} />;
  }
  ...
  if (!exportData) {
    return <EmptyState icon={Package} message={t("pages.companyExport.loading")} />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className="font-medium">
              {t("pages.companyExport.companyExport", { company: selectedCompany?.name ?? t("common.company") })}
            </span>
            <span className="text-muted-foreground">
              {t("pages.companyExport.filesSelected", { selected: selectedCount, total: totalFiles })}
            </span>
            {warnings.length > 0 && (
              <span className="text-amber-500">
                {t("pages.companyExport.warnings", { count: warnings.length })}
              </span>
            )}
          </div>
          <Button
          ...
          >
            ...
            {downloadMutation.isPending
              ? t("pages.companyExport.building")
              : t("pages.companyExport.exportFiles", { count: selectedCount })}
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-4 xl:h-(--sz-calc-30) xl:grid-cols-(--gtc-25) xl:gap-0">
        <aside className="flex max-h-(--sz-24rem) flex-col overflow-hidden border-b border-border xl:max-h-none xl:border-b-0 xl:border-r">
          <div className="border-b border-border px-4 py-3 shrink-0">
            <h2 className="text-base font-semibold">{t("pages.companyExport.packageFiles")}</h2>
          </div>
          <div className="border-b border-border px-3 py-2 shrink-0">
            <div className="flex items-center gap-2 rounded-md border border-border px-2 py-1">
              ...
              <input
                ...
                placeholder={t("pages.companyExport.searchFiles")}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {totalTaskChildren > visibleTaskChildren && !treeSearch && (
              <div className="px-4 py-2">
                ...
                >
                  {t("pages.companyExport.showMoreIssues", { defaultValue: "Show more tasks ({{visible}} of {{total}})", visible: visibleTaskChildren, total: totalTaskChildren })}
                </button>
              </div>
            )}
          </div>
