...
function AccessSection({
  ...
  const summary =
    access.mode === "all"
      ? t("apps.permissions.everyAgent")
      : t("apps.permissions.agentCountCanUse", { count: access.agentIds.size });
  ...
  return (
    <section className="rounded-xl border border-border bg-card">
      <div className="flex items-center justify-between px-5 py-4">
        <div>
          <h2 className="text-sm font-bold text-foreground">{t("apps.permissions.whoCanUse")}</h2>
      ...
      {editing && (
        <div className="space-y-3 border-t border-border px-5 py-4">
          <label className="flex items-start gap-3">
            ...
            />
            <span>
              <span className="text-sm font-semibold text-foreground">{t("apps.connect.access.allAgents")}</span>
              <span className="block text-xs text-muted-foreground">{t("apps.permissions.allAgentsDescription")}</span>
            </span>
          </label>
          <label className="flex items-start gap-3">
            <input
            ...
            />
            <span>
              <span className="text-sm font-semibold text-foreground">{t("apps.connect.access.specificAgents")}</span>
              <span className="block text-xs text-muted-foreground">{t("apps.permissions.pickAgents")}</span>
            </span>
          </label>
  ...
  return (
    <section className="rounded-xl border border-border bg-card">
      <div className="flex items-center justify-between gap-3 px-5 py-4">
        <div>
          <h2 className="text-sm font-bold text-foreground">{t("apps.permissions.installedTitle")}</h2>
          <p className="mt-0.5 text-sm text-muted-foreground">
            {t("apps.permissions.installedDescription", { appName })}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {disabled && <span className="text-xs text-muted-foreground">{t("common.saving")}</span>}
          {install.onAll ? (
            <InstalledBadge label={t("apps.connect.success.installedAll")} />
          ) : install.agentIds.size > 0 ? (
            <InstalledBadge label={t("apps.permissions.installedCount", { count: installedCount })} />
          ) : (
            <span className="rounded-full border border-border bg-muted px-2 py-0.5 text-xs font-medium text-muted-foreground">
              {t("apps.permissions.permittedOnly")}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="space-y-3 border-t border-border px-5 py-4">
        ...
        {!install.onAll && (
          <AgentMultiSelect
            ...
            triggerLabel={
              install.agentIds.size === 0
                ? t("apps.permissions.chooseInstallAgents")
                : t("apps.connect.success.installedCount", { count: install.agentIds.size })
            }
            getDescription={(agent) => hasAccess(agent.id)
              ? t("apps.permissions.hasAccess")
              : t("apps.permissions.noAccessYet")}
            renderNameSuffix={(agent) =>
              !hasAccess(agent.id) && install.agentIds.has(agent.id) ? (
                <span className={cn("rounded border px-1 py-0 text-xs font-medium", brandChipBadge.amber)}>
                  {t("apps.permissions.willGrantAccess")}
                </span>
              ) : null
            }
        ...
        >
          <Checkbox
            ...
            disabled={disabled}
            aria-label={t("apps.permissions.installAll")}
            onCheckedChange={(checked) =>
              onSave(checked ? { onAll: true, agentIds: new Set() } : { onAll: false, agentIds: new Set() })
            }
          />
          <span className="text-xs text-foreground">
            <span className="font-semibold">{t("apps.permissions.installAll")}</span>
            ...
            <span>
              {autoExtendNotice(
                extendingAgents.length === 1
                  ? liveAgents.find((a) => a.id === extendingAgents[0])?.name ?? t("apps.connect.install.agentCount", { count: 1 })
                  : t("apps.connect.install.agentCount", { count: extendingAgents.length }),
              )}{" "}
              <span className="font-medium">
                {t("apps.permissions.reviewAccessChanges", { count: extendingAgents.length })}
              </span>
            </span>
          </InlineBanner>
        ) : null}
  ...
  return (
    <section className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-sm font-bold text-foreground">{t("apps.permissions.actionPermissions")}</h2>
          <p className="mt-0.5 text-sm text-muted-foreground">
            {t("apps.permissions.actionPermissionsDescription")}
          </p>
        </div>
        <div className="flex items-center gap-2">
          {disabled && <span className="text-xs text-muted-foreground">{t("common.saving")}</span>}
          ...
          >
            ...
            {t("apps.permissions.refreshActions")}
          </Button>
        </div>
      </div>
      ...
      <ActionGroup
        ...
        hint={t("apps.permissions.readOnlyHint")}
        actions={readOnly}
        enabledIds={enabledIds}
        askFirstIds={askFirstIds}
        disabled={disabled}
      ...
      <ActionGroup
        ...
        hint={t("apps.permissions.canChangeHint")}
        actions={canChange}
        enabledIds={enabledIds}
        askFirstIds={askFirstIds}
        disabled={disabled}
        ...
        {actions.map((action) => {
          ...
          return (
            ...
            >
              ...
              <select
                aria-label={t("apps.permissions.permissionAria", { action: action.title ?? action.toolName })}
                className={cn(
              ...
              >
                <option value="off">{t("apps.permissions.off")}</option>
                <option value="allowed">{t("apps.permissions.allowed")}</option>
                <option value="ask">{t("apps.permissions.askHuman")}</option>
              </select>
            </div>
          );
        })}
