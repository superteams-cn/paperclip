  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        {t("apps.gateways.appsPanel.description", {
          profile: profile ? ` (${profile.name})` : "",
          tools: profile ? ` — ${allowedToolsLabel(profile)}` : "",
        })}
      </p>

      {apps.length === 0 ? (
        <div className="rounded-md border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
          {t("apps.gateways.appsPanel.empty")}
        </div>
      ) : (
          ...
          <table className="w-full min-w-(--sz-32rem) text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40 text-left text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                <th className="px-4 py-2.5">{t("apps.gateways.table.app")}</th>
                <th className="px-4 py-2.5">{t("apps.gateways.table.tools")}</th>
