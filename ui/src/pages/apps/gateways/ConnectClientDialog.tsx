  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("apps.gateways.client.title")}</DialogTitle>
          <DialogDescription>
            {t("apps.gateways.client.description")}
          </DialogDescription>
        </DialogHeader>

        <div className="grid gap-4 sm:grid-cols-(--gtc-10)">
          <nav className="flex gap-1 overflow-x-auto sm:flex-col" aria-label={t("apps.gateways.client.clientsAria")}>
          ...
          <div className="min-w-0 space-y-3">
            {active === "raw_url" ? (
              <div className="space-y-1.5">
                <div className="text-sm font-medium text-foreground">{t("apps.gateways.client.endpointUrl")}</div>
                <div className="flex items-center gap-2">
                  <code className="min-w-0 flex-1 truncate rounded-md bg-muted px-3 py-2 font-mono text-xs text-muted-foreground">
                    {endpoint}
                  </code>
                  <Button variant="outline" size="sm" onClick={() => void copyText(endpoint, t("apps.gateways.client.endpointUrl"))}>
                    <Copy className="mr-1 h-3.5 w-3.5" />
                    {t("common.copy")}
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground">
                  {t("apps.gateways.client.authenticatePrefix")} <code>Authorization: Bearer &lt;token&gt;</code> {t("apps.gateways.client.authenticateSuffix")}
                </p>
              </div>
            ) : activeSnippet ? (
              <div className="space-y-1.5">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => void copyText(configText, t("apps.gateways.client.configLabel", { name: activeSnippet.label }))}
                  >
                    <Copy className="mr-1 h-3.5 w-3.5" />
                    {t("common.copy")}
                  </Button>
              ...
              <p className="text-sm text-muted-foreground">{t("apps.gateways.client.noSnippets")}</p>
            )}

            <div className="space-y-1.5 rounded-md border border-border p-3">
              <div className="text-xs font-medium text-muted-foreground">{t("apps.gateways.table.tokens")}</div>
              {createdToken ? (
                <div className="flex items-center gap-2">
                  ...
                  {revealed ? (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => void copyText(createdToken.token, t("apps.gateways.client.accessToken"))}
                    >
                      <Copy className="mr-1 h-3.5 w-3.5" />
                      {t("common.copy")}
                    </Button>
                ...
                <p className="text-xs text-muted-foreground">
                  {t("apps.gateways.client.mintPrefix")} <span className="font-medium">{t("apps.gatewayTabs.tokens")}</span> {t("apps.gateways.client.mintMiddle")}{" "}
                  <code>Bearer …</code>{t("apps.gateways.client.mintSuffix")}
                </p>
              )}
              <p className="text-xs text-muted-foreground">
                {t("apps.gateways.client.securityNote")}
              </p>
            </div>
          </div>
        </div>
