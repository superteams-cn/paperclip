...
const CAPABILITY_OPTIONS: Array<{ value: ToolRiskLevel; label: string; sentence: string }> = [
  { value: "read", get label() { return translate("tools.policies.capability.read.label"); }, get sentence() { return translate("tools.policies.capability.read.sentence"); } },
  { value: "write", get label() { return translate("tools.policies.capability.write.label"); }, get sentence() { return translate("tools.policies.capability.write.sentence"); } },
  { value: "destructive", get label() { return translate("tools.policies.capability.destructive.label"); }, get sentence() { return translate("tools.policies.capability.destructive.sentence"); } },
];
const OUTCOMES: Array<{ value: BuilderPolicyType; label: string }> = [
  { value: "allow", get label() { return translate("tools.policies.outcomes.allow"); } },
  { value: "block", get label() { return translate("tools.policies.outcomes.block"); } },
  { value: "require_approval", get label() { return translate("tools.policies.outcomes.askFirst"); } },
  { value: "rate_limit", get label() { return translate("tools.policies.outcomes.limit"); } },
];
...
function buildPolicyPayload(form: PolicyFormState) {
  ...
  if (!Number.isInteger(priority) || priority < 0 || priority > 10000) {
    throw new Error(translate("tools.policies.errors.priority"));
  }
  ...
  if (form.policyType === "rate_limit") {
    ...
    if (!Number.isInteger(limit) || limit <= 0) throw new Error(translate("tools.policies.errors.limit"));
    if (!Number.isInteger(windowSeconds) || windowSeconds <= 0) {
      throw new Error(translate("tools.policies.errors.window"));
    }
...
function windowLabel(seconds: string | number | null | undefined) {
  ...
  if (value === 3600) return translate("tools.policies.time.hour");
  if (value === 86400) return translate("tools.policies.time.day");
  if (value === 60) return translate("tools.policies.time.minute");
...
function outcomeLabel(policy: Pick<ToolPolicy, "policyType" | "config"> | PolicyFormState) {
  ...
  if (policyType === "allow") return translate("tools.policies.outcomes.allow");
  if (policyType === "block") return translate("tools.policies.outcomes.block");
  if (policyType === "require_approval") return translate("tools.policies.outcomes.askFirst");
  if (policyType === "redact") return translate("tools.policies.outcomes.unsupportedRedact");
  if (policyType === "trust_rule") return translate("tools.policies.outcomes.allow");
  if (policyType === "validate") return translate("tools.policies.outcomes.unsupportedCheck");
  ...
  return translate("tools.policies.outcomes.limitTo", { limit: limit ?? 50, window: windowLabel(seconds as string | number | null | undefined) });
}

function capabilitySentence(value: string | null | undefined) {
  return CAPABILITY_OPTIONS.find((item) => item.value === value)?.sentence ?? translate("tools.policies.sentences.valueActions", { value });
}
...
function policySentence(
  ...
  let who = translate("tools.policies.sentences.anyAgent");
  if (agentIds.length === 1) who = maps.agent.get(agentIds[0]!) ?? translate("tools.policies.sentences.oneAgent");
  else if (agentIds.length > 1) who = translate("tools.policies.sentences.agentCount", { count: agentIds.length });
  else if (projectIds.length === 1) who = translate("tools.policies.sentences.agentsInProject", { project: maps.project.get(projectIds[0]!) ?? translate("tools.policies.sentences.oneProject") });
  else if (projectIds.length > 1) who = translate("tools.policies.sentences.agentsInProjectCount", { count: projectIds.length });
  else if (actorType !== ANY_VALUE && actorType !== "agent") who = translate("tools.policies.sentences.actorType", { actorType });

  let uses = translate("tools.policies.sentences.anything");
  if (tools.length === 1) uses = toolDisplayName(tools[0]!, catalogByToolName);
  else if (tools.length > 1) uses = translate("tools.policies.sentences.specificActions", { count: tools.length });
  else if (catalogEntryIds.length === 1) {
    const tool = catalogById.get(catalogEntryIds[0]!);
    uses = tool ? tool.title || toolDisplayName(tool.toolName, catalogByToolName) : translate("tools.policies.sentences.oneAction");
  }
  else if (catalogEntryIds.length > 1) uses = translate("tools.policies.sentences.specificActions", { count: catalogEntryIds.length });
  else if (appIds.length === 1) uses = maps.application.get(appIds[0]!) ?? translate("tools.policies.sentences.oneApp");
  else if (appIds.length > 1) uses = translate("tools.policies.sentences.appCount", { count: appIds.length });
  else if (connectionIds.length === 1) uses = maps.connection.get(connectionIds[0]!) ?? translate("tools.policies.sentences.oneApp");
...
function sentenceText(sentence: ReturnType<typeof policySentence>) {
  return translate("tools.policies.sentences.rule", sentence);
}
...
function RuleSentence({ sentence }: { sentence: ReturnType<typeof policySentence> }) {
  return <span>{sentenceText(sentence)}</span>;
}
...
function PolicySimulator({
  ...
  const test = useMutation({
    ...
    onError: (err) => {
      setResult(null);
      pushToast({
        title: t("tools.policies.simulator.failed"),
  ...
  const verdict = result
    ? `${outcomeLabel({ policyType: result.decision as ToolPolicyType, config: null })} - ${
        decidingPolicy ? t("tools.policies.simulator.ruleNumber", { number: policies.findIndex((policy) => policy.id === decidingPolicy.id) + 1 }) : result.explanation
      }`
    : null;
      ...
      <SheetContent className="w-full gap-0 p-0 sm:max-w-xl">
        <SheetHeader className="border-b border-border">
          <SheetTitle className="flex items-center gap-2 text-base">
            ...
            {t("tools.policies.simulator.title")}
          </SheetTitle>
          <SheetDescription>{t("tools.policies.simulator.description")}</SheetDescription>
        </SheetHeader>
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1.5">
              <Label>{t("tools.policies.simulator.agent")}</Label>
              <Select value={agentId} onValueChange={setAgentId}>
                <SelectTrigger>
                  <SelectValue placeholder={t("tools.policies.simulator.selectAgent")} />
                </SelectTrigger>
                <SelectContent>
                  {agents.map((a) => (
            ...
            <div className="space-y-1.5">
              <Label htmlFor="test-action">{t("tools.policies.simulator.action")}</Label>
              <Input
                id="test-action"
                value={toolName}
                onChange={(e) => setToolName(e.target.value)}
                placeholder={t("tools.policies.simulator.actionPlaceholder")}
              />
            </div>
          </div>
          <Button size="sm" disabled={!agentId || !toolName.trim() || test.isPending} onClick={() => test.mutate()}>
            {test.isPending ? t("tools.policies.simulator.checking") : t("tools.policies.simulator.check")}
          </Button>
            ...
            <div className="space-y-3 rounded-md border border-border p-3">
              ...
              <details className="text-xs text-muted-foreground">
                ...
                <div className="mt-2 space-y-1 font-mono">
                  <div>{t("tools.policies.simulator.reason")}: {result.reasonCode}</div>
                  <div>{t("tools.policies.simulator.matchedRuleIds")}: {result.matchedPolicyIds.length ? result.matchedPolicyIds.join(", ") : t("common.none")}</div>
                  <div>{t("tools.policies.simulator.effectiveProfiles")}: {result.effectiveProfileIds.length ? result.effectiveProfileIds.join(", ") : t("common.none")}</div>
                </div>
              </details>
            </div>
          ) : null}
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-1">
          <Button variant="ghost" size="sm" className="px-0" onClick={onCancel}>
            {t("tools.policies.builder.back")}
          </Button>
          <h2 className="text-lg font-semibold text-foreground">{form.id ? t("tools.policies.builder.editTitle") : t("tools.policies.builder.newTitle")}</h2>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={onCancel}>{t("common.cancel")}</Button>
          <Button size="sm" onClick={onSave} disabled={saving}>
            {saving ? t("common.saving") : t("tools.policies.builder.save")}
          </Button>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-(--gtc-61)">
        <section className="space-y-3">
          <h3 className="text-sm font-semibold text-foreground">{t("tools.policies.builder.when")}</h3>
          <div className="grid gap-2">
            {[
              ["everyone", t("tools.policies.builder.everyone")],
              ["agent", t("tools.policies.builder.specificAgent")],
              ["project", t("tools.policies.builder.agentsInProject")],
            ].map(([value, label]) => (
              <Button
                key={value}
          ...
          {form.whenMode === "agent" ? (
            <Select value={form.agentId} onValueChange={(agentId) => setForm({ ...form, agentId })}>
              <SelectTrigger><SelectValue placeholder={t("tools.policies.builder.chooseAgent")} /></SelectTrigger>
              <SelectContent>
                <SelectItem value={ANY_VALUE}>{t("tools.policies.builder.chooseAgent")}</SelectItem>
          ...
          {form.whenMode === "project" ? (
            <Select value={form.projectId} onValueChange={(projectId) => setForm({ ...form, projectId })}>
              <SelectTrigger><SelectValue placeholder={t("tools.policies.builder.chooseProject")} /></SelectTrigger>
              <SelectContent>
                <SelectItem value={ANY_VALUE}>{t("tools.policies.builder.chooseProject")}</SelectItem>
        ...
        <section className="space-y-3">
          <h3 className="text-sm font-semibold text-foreground">{t("tools.policies.builder.uses")}</h3>
          <div className="grid gap-2">
            {[
              ["anything", t("tools.policies.builder.anything")],
              ["app", t("tools.policies.builder.specificApp")],
              ["actions", t("tools.policies.builder.specificActions")],
              ["capability", t("tools.policies.builder.actionsByCapability")],
            ].map(([value, label]) => (
              <Button
                key={value}
          ...
          {form.usesMode === "app" ? (
            <Select value={form.applicationId} onValueChange={(applicationId) => setForm({ ...form, applicationId })}>
              <SelectTrigger><SelectValue placeholder={t("tools.policies.builder.chooseApp")} /></SelectTrigger>
              <SelectContent>
                <SelectItem value={ANY_VALUE}>{t("tools.policies.builder.chooseApp")}</SelectItem>
          ...
          {form.usesMode === "capability" ? (
            <Select value={form.riskLevel} onValueChange={(riskLevel) => setForm({ ...form, riskLevel })}>
              <SelectTrigger><SelectValue placeholder={t("tools.policies.builder.chooseCapability")} /></SelectTrigger>
              <SelectContent>
                <SelectItem value={ANY_VALUE}>{t("tools.policies.builder.chooseCapability")}</SelectItem>
          ...
          {form.usesMode === "actions" ? (
            <div className="max-h-80 overflow-y-auto rounded-md border border-border">
              {appGroups.length === 0 ? (
                <div className="p-3 text-sm text-muted-foreground">{t("tools.policies.builder.noActions")}</div>
              ) : (
                appGroups.map((group) => {
                  ...
                  return (
                    <div key={group.appKey} className="border-b border-border last:border-b-0">
                      <label className="flex items-center gap-2 px-3 py-2 text-sm font-medium">
                        ...
                        <span className="text-xs text-muted-foreground">{t("tools.policies.builder.selectedCount", { selected: selectedCount, total: group.tools.length })}</span>
                      </label>
        ...
        <section className="space-y-3">
          <h3 className="text-sm font-semibold text-foreground">{t("tools.policies.builder.then")}</h3>
          <div className="grid gap-2">
          ...
          {form.policyType === "rate_limit" ? (
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1.5">
                <Label htmlFor="limit-count">{t("tools.policies.builder.times")}</Label>
                <Input id="limit-count" inputMode="numeric" value={form.rateLimitLimit} onChange={(e) => setForm({ ...form, rateLimitLimit: e.target.value })} />
              </div>
              <div className="space-y-1.5">
                <Label>{t("tools.policies.builder.per")}</Label>
                <Select value={form.rateLimitWindowSeconds} onValueChange={(rateLimitWindowSeconds) => setForm({ ...form, rateLimitWindowSeconds })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="3600">{t("tools.policies.time.hour")}</SelectItem>
                    <SelectItem value="86400">{t("tools.policies.time.day")}</SelectItem>
                    <SelectItem value="60">{t("tools.policies.time.minute")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
      ...
      <details className="rounded-md border border-border p-3">
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-2">
          ...
          <div className="flex items-center justify-between gap-3 rounded-md border border-border px-3 py-2 lg:col-span-2">
            <div>
              <p className="text-sm font-medium text-foreground">{t("common.on")}</p>
              <p className="text-xs text-muted-foreground">{t("tools.policies.builder.enabledHint")}</p>
            </div>
...
function StarterCards({ onStart }: { onStart: (form: PolicyFormState) => void }) {
  ...
  const starters = [
    {
      title: t("tools.policies.starters.blockDestructive"),
      form: emptyPolicyForm({ policyType: "block", usesMode: "capability", riskLevel: "destructive", name: t("tools.policies.starters.blockDestructive") }),
    },
    {
      title: t("tools.policies.starters.askFirst"),
      form: emptyPolicyForm({ policyType: "require_approval", usesMode: "actions", name: t("tools.policies.starters.askFirst") }),
    },
    {
      title: t("tools.policies.starters.limitNoisy"),
      form: emptyPolicyForm({ policyType: "rate_limit", usesMode: "actions", rateLimitLimit: "50", name: t("tools.policies.starters.limitNoisy") }),
    },
    { title: t("tools.policies.starters.scratch"), form: emptyPolicyForm() },
  ];
  return (
...
export function PoliciesTab({ companyId }: { companyId: string }) {
  ...
  const createPolicy = useMutation({
    ...
    onSuccess: () => {
      invalidatePolicies();
      setForm(null);
      pushToast({ title: t("tools.policies.toasts.created"), tone: "success" });
    },
    onError: (err) => pushToast({ title: t("tools.policies.toasts.saveFailed"), body: err instanceof ApiError ? err.message : String(err), tone: "error" }),
  });
  const updatePolicy = useMutation({
    ...
    onSuccess: () => {
      invalidatePolicies();
      setForm(null);
      pushToast({ title: t("tools.policies.toasts.updated"), tone: "success" });
    },
    onError: (err) => pushToast({ title: t("tools.policies.toasts.saveFailed"), body: err instanceof ApiError ? err.message : String(err), tone: "error" }),
  });
  const reorder = useMutation({
    ...
    onSuccess: () => {
      invalidatePolicies();
      pushToast({ title: t("tools.policies.toasts.reordered"), tone: "success" });
    },
    onError: (err) => pushToast({ title: t("tools.policies.toasts.reorderFailed"), body: err instanceof ApiError ? err.message : String(err), tone: "error" }),
  });
  const duplicate = useMutation({
    mutationFn: (policy: ToolPolicy) => toolsApi.duplicatePolicy(companyId, policy.id),
    onSuccess: () => {
      invalidatePolicies();
      pushToast({ title: t("tools.policies.toasts.duplicated"), body: t("tools.policies.toasts.duplicateHint"), tone: "success" });
    },
    onError: (err) => pushToast({ title: t("tools.policies.toasts.duplicateFailed"), body: err instanceof ApiError ? err.message : String(err), tone: "error" }),
  });
  const deletePolicy = useMutation({
    ...
    onSuccess: () => {
      invalidatePolicies();
      setConfirm(null);
      pushToast({ title: t("tools.policies.toasts.deleted"), tone: "success" });
    },
    onError: (err) => pushToast({ title: t("tools.policies.toasts.deleteFailed"), body: err instanceof ApiError ? err.message : String(err), tone: "error" }),
  });
  const revoke = useMutation({
    ...
    onSuccess: () => {
      ...
      setConfirm(null);
      pushToast({ title: t("tools.policies.toasts.approvalForgotten"), tone: "success" });
    },
    onError: (err) => pushToast({ title: t("tools.policies.toasts.forgetFailed"), body: err instanceof ApiError ? err.message : String(err), tone: "error" }),
  });
  ...
  function submitPolicy() {
    ...
    try {
      ...
      pushToast({ title: t("tools.policies.toasts.invalid"), body: err instanceof Error ? err.message : String(err), tone: "error" });
    }
  }
  ...
  return (
    <div className="space-y-5">
        ...
        actions={
          <>
            ...
            <Button size="sm" onClick={() => setForm(emptyPolicyForm())}>
              ...
              {t("tools.policies.builder.newTitle")}
            </Button>
          </>
        }
      />
      ...
      <div className="space-y-2">
        {policies.isLoading ? (
          ...
          <div className="space-y-3">
            <EmptyState
              icon={Shield}
              message={t("tools.policies.empty.title")}
              description={t("tools.policies.empty.description")}
              action={t("tools.policies.builder.newTitle")}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-xs text-muted-foreground">
                    ...
                    <th className="px-2 py-2.5 font-medium">{t("tools.policies.table.rule")}</th>
                    <th className="px-2 py-2.5 font-medium">{t("tools.policies.table.outcome")}</th>
                    <th className="px-2 py-2.5 text-right font-medium">{t("tools.policies.table.last24h")}</th>
                  ...
                  {policyList.map((policy) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-2 py-2 text-right text-xs text-muted-foreground">{t("tools.policies.table.hitCount", { count: hits })}</td>
                        ...
                        <td className="px-2 py-2 text-right">
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button size="icon" variant="ghost" aria-label={t("tools.policies.table.actions")}>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem onSelect={() => updatePolicy.mutate({ policyId: policy.id, body: { enabled: false } })}>
                                ...
                                {t("tools.policies.table.turnOff")}
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem
      ...
      <div className="space-y-2">
        <h3 className="text-sm font-semibold text-foreground">{t("tools.policies.remembered.title")}</h3>
        <p className="text-sm text-muted-foreground">{t("tools.policies.remembered.description")}</p>
        {trustRules.isLoading ? (
          ...
          <div className="rounded-md border border-border px-4 py-6 text-sm text-muted-foreground">
            {t("tools.policies.remembered.empty")}
          </div>
        ) : (
              ...
              <div key={rule.id} className="flex flex-wrap items-center gap-3 px-3 py-2">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {t("tools.policies.remembered.remembered")} <RelativeTime value={rule.updatedAt} />
                  </div>
                </div>
                <Button
                  size="sm"
                ...
                >
                  {t("tools.policies.remembered.forget")}
                </Button>
              </div>
            ))}
          </div>
      ...
      <Dialog open={Boolean(confirm)} onOpenChange={(open) => !open && setConfirm(null)}>
        {confirm ? (
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{confirm.kind === "delete-rule" ? t("tools.policies.confirm.deleteTitle") : t("tools.policies.confirm.forgetTitle")}</DialogTitle>
              <DialogDescription>
                {confirm.kind === "delete-rule"
                  ? t("tools.policies.confirm.deleteDescription", { count: confirm.hits })
                  : t("tools.policies.confirm.forgetDescription")}
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              ...
              >
                {confirm.kind === "delete-rule" ? t("common.delete") : t("tools.policies.remembered.forget")}
              </Button>
            </DialogFooter>
          </DialogContent>
        ) : null}
