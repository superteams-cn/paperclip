...
function humanizeKey(raw: string): string {
  ...
  if (!cleaned) return translate("tools.pasteConfig.key");
...
function draftSummary(draft: McpJsonImportDraft): string {
  ...
  const where = draft.transport === "local_stdio"
    ? translate("tools.pasteConfig.summary.local")
    : translate("tools.pasteConfig.summary.remote");
  if (keyCount === 0) return translate("tools.pasteConfig.summary.noKeys", { where });
  return translate("tools.pasteConfig.summary.needsKeys", { where, count: keyCount });
}
...
export function PasteConfigTab({ companyId }: { companyId: string }) {
  ...
  const connectMutation = useMutation({
    mutationFn: (draft: McpJsonImportDraft) => {
      ...
      if (!url) throw new Error(t("tools.pasteConfig.errors.remoteOnly"));
  ...
  const finishMutation = useMutation({
    ...
    onSuccess: () => setActivatedName(connectResult?.application.name ?? t("tools.pasteConfig.importedApp")),
  });
  ...
  const localParseError = useMemo(() => {
    ...
    try {
      ...
      return t("tools.pasteConfig.errors.invalidJson");
    }
  }, [draftText]);
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-xs text-muted-foreground">
        {t("tools.pasteConfig.justUrl")}{" "}
        <Link to="/apps/browse" className="text-primary hover:underline">
          {t("tools.pasteConfig.browseConnections")}
        </Link>{" "}
        {t("tools.pasteConfig.instead")}
      </p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {importMutation.isPending ? t("tools.pasteConfig.checking") : t("tools.pasteConfig.checkConfig")}
        </Button>
        ...
        drafts.length === 0 ? (
          ...
          <div className="space-y-3">
            <h3 className="flex items-center gap-2 text-sm font-semibold text-foreground">
              ...
              {t("tools.pasteConfig.foundApps", { count: drafts.length })}
            </h3>
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        {onCheck ? (
          <Button size="sm" className="shrink-0" onClick={onCheck} disabled={checking || !canCheck}>
            ...
            {t("tools.pasteConfig.checkActions")}
          </Button>
        ) : null}
      </div>
      ...
      {draft.credentialFields.length > 0 ? (
        ...
        <p className="mt-3 text-xs text-muted-foreground">{t("tools.pasteConfig.noKeys")}</p>
      )}
...
function CatalogReview({
  ...
  return (
    <div className="space-y-4 border-t border-border pt-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="flex items-center gap-2 text-sm font-semibold text-foreground">
            ...
            {t("tools.pasteConfig.reviewActions", { app: result.application.name })}
          </h3>
        ...
        <Button size="sm" onClick={onFinish} disabled={finishing || enabledCount === 0 || Boolean(activatedName)}>
          ...
          {t("tools.pasteConfig.activateCount", { enabled: enabledCount, total })}
        </Button>
      </div>
      <ActionGroup
        title={t("tools.pasteConfig.readOnly")}
      ...
      />
      <ActionGroup
        title={t("tools.pasteConfig.canChange")}
      ...
      />
      {activatedName ? (
        <p className="text-xs font-medium text-emerald-700">{t("tools.pasteConfig.activeForAll", { app: activatedName })}</p>
      ) : null}
    </div>
  );
}
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-xs text-muted-foreground">
                  {askFirstLevels.includes(action.riskLevel)
                    ? t("tools.pasteConfig.askFirstEnabled")
