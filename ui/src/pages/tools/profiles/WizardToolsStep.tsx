...
export function WizardToolsStep(props: WizardToolsStepProps) {
  ...
  if (catalogLoading) return <LoadingState label={t("tools.profiles.toolsStep.loading")} />;
  ...
  if (appGroups.length === 0) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed border-border py-12 text-center">
        ...
        <div>
          <p className="text-sm font-medium text-foreground">{t("tools.profiles.toolsStep.connectionsComingSoon")}</p>
          <p className="mx-auto max-w-sm text-sm text-muted-foreground">
            {t("tools.profiles.toolsStep.connectionsComingSoonDescription")}
          </p>
        </div>
        <Button asChild variant="outline">
          <Link to="/apps/browse">{t("tools.profiles.toolsStep.browseConnections")}</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-(--sz-220px) flex-1">
          ...
          <Input
            ...
            placeholder={t("tools.profiles.toolsStep.searchPlaceholder")}
            className="pl-8"
          />
        </div>
      ...
      {filteredGroups.length === 0 ? (
        ...
        <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed border-border py-10 text-center">
          <p className="text-sm font-medium text-foreground">{t("tools.profiles.toolsStep.noMatches", { search })}</p>
          <button
            type="button"
            onClick={() => {
              setSearch("");
          ...
          >
            {t("tools.profiles.toolsStep.clearSearch")}
          </button>
        </div>
      ) : (
    ...
    <div>
      <div className="flex items-center gap-2.5 px-3 py-2">
        <Checkbox
          ...
          aria-label={translate("tools.profiles.selection.allAppTools", { app: group.name, count: group.tools.length })}
        />
        <button
          type="button"
        ...
        >
          ...
          <span className="flex flex-col">
            <span className="text-sm font-medium text-foreground">
              {translate("tools.profiles.selection.allAppTools", { app: group.name, count: group.tools.length })}
            </span>
            <span className="text-xs text-muted-foreground">
              {state === "indeterminate"
                ? appSelectionLabel(group, selection)
                : translate("tools.profiles.toolsStep.includesFutureTools", { app: group.name })}
            </span>
          </span>
        </button>
      </div>
...
function NewToolsRadio({
  ...
  const options: Array<{ value: NewToolsAction; label: string; hint: string; recommended?: boolean }> = [
    {
      value: "deny",
      label: translate("tools.profiles.toolsStep.newTools.blockedTitle"),
      hint: translate("tools.profiles.toolsStep.newTools.blockedHint"),
      recommended: true,
    },
    {
      value: "allow",
      label: translate("tools.profiles.toolsStep.newTools.allowedTitle"),
      hint: translate("tools.profiles.toolsStep.newTools.allowedHint"),
    },
  ];
  return (
    <fieldset className="space-y-2 rounded-lg border border-border p-4">
      <legend className="px-1 text-sm font-medium text-foreground">{translate("tools.profiles.toolsStep.newTools.title")}</legend>
            ...
            <span className="flex flex-col gap-0.5">
              <span className="flex items-center gap-2 text-sm font-medium text-foreground">
                ...
                {opt.recommended ? (
                  ...
                  <span className="text-xs font-normal text-amber-600">({translate("tools.profiles.toolsStep.newTools.risky")})</span>
                )}
              </span>
...
const RULE_KIND_OPTIONS: Array<{ value: AdvancedRuleKind; label: string }> = [
  { value: "tool_name", get label() { return translate("tools.profiles.toolsStep.rules.toolNamePattern"); } },
  { value: "risk_level", get label() { return translate("tools.profiles.toolsStep.rules.riskLevel"); } },
  { value: "catalog_entry", get label() { return translate("tools.profiles.toolsStep.rules.toolId"); } },
];
...
function ruleSummary(rule: AdvancedRule): string {
  const verb = translate(`tools.profiles.toolsStep.rules.${rule.effect === "include" ? "allow" : "block"}`);
  if (rule.kind === "tool_name") return translate("tools.profiles.toolsStep.rules.summaryPattern", { verb, value: rule.value });
  if (rule.kind === "risk_level") return translate("tools.profiles.toolsStep.rules.summaryRisk", { verb, value: rule.riskLevel ?? rule.value });
  return translate("tools.profiles.toolsStep.rules.summaryTool", { verb, value: rule.value });
}
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="rounded-lg border border-border">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-4 py-3 text-left">
        <span className="text-sm font-medium text-foreground">{translate("tools.profiles.toolsStep.rules.title")}</span>
        <ChevronDown className={cn("h-4 w-4 text-muted-foreground transition-transform", open && "rotate-180")} />
      </CollapsibleTrigger>
      <CollapsibleContent className="space-y-3 border-t border-border px-4 py-3">
        <p className="text-xs text-muted-foreground">
          {translate("tools.profiles.toolsStep.rules.description")}
        </p>
          ...
          <ul className="space-y-1.5">
            {rules.map((rule) => (
              ...
              >
                ...
                <button
                  type="button"
                  aria-label={translate("tools.profiles.toolsStep.rules.remove")}
        ...
        <div className="flex flex-wrap items-end gap-2">
          <Select value={effect} onValueChange={(v) => setEffect(v as "include" | "exclude")}>
            ...
            <SelectContent>
              <SelectItem value="include">{translate("tools.profiles.toolsStep.rules.allow")}</SelectItem>
              <SelectItem value="exclude">{translate("tools.profiles.toolsStep.rules.block")}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {kind === "risk_level" ? (
            <Select value={value || "destructive"} onValueChange={setValue}>
              ...
              <SelectContent>
                <SelectItem value="read">{CAPABILITY_LABEL.read}</SelectItem>
                <SelectItem value="write">{CAPABILITY_LABEL.write}</SelectItem>
                <SelectItem value="destructive">{CAPABILITY_LABEL.destructive}</SelectItem>
              </SelectContent>
            </Select>
          ) : (
            <Input
              ...
              placeholder={kind === "tool_name" ? translate("tools.profiles.toolsStep.rules.patternPlaceholder") : translate("tools.profiles.toolsStep.rules.toolIdPlaceholder")}
              className="w-44"
            />
          )}
          <Button type="button" variant="outline" size="sm" onClick={addRule}>
            <Plus className="mr-1 h-3.5 w-3.5" />
            {translate("tools.profiles.toolsStep.rules.add")}
          </Button>
        </div>
      </CollapsibleContent>
    </Collapsible>
