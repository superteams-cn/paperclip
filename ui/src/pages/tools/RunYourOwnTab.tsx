...
export function RunYourOwnTab({ companyId }: { companyId: string }) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("tools.runYourOwn.toasts.added"), body: t("tools.runYourOwn.toasts.ready", { name: name.trim() }), tone: "success" });
      setName("");
      setCommand("");
      setKeyRows([]);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-5 rounded-lg border border-border bg-card p-5">
        <div className="space-y-1.5">
          <Label htmlFor="ryo-name">{t("tools.runYourOwn.name")}</Label>
          <Input
            id="ryo-name"
            value={name}
            onChange={(event) => setName(event.target.value)}
            placeholder={t("tools.runYourOwn.namePlaceholder")}
            maxLength={160}
          />
          <p className="text-xs text-muted-foreground">{t("tools.runYourOwn.nameHint")}</p>
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="ryo-command">{t("tools.runYourOwn.command")}</Label>
          <Input
            id="ryo-command"
            value={command}
          ...
          />
          <p className="text-xs text-muted-foreground">{t("tools.runYourOwn.commandHint")}</p>
        </div>

        <div className="space-y-2">
          <div className="flex items-baseline gap-2">
            <Label>{t("tools.runYourOwn.keys")}</Label>
            <span className="text-xs text-muted-foreground">· {t("tools.runYourOwn.keysOptional")}</span>
          </div>
          {keyRows.length > 0 ? (
            <div className="space-y-2">
              {keyRows.map((row) => {
                  ...
                  <div key={row.id} className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        aria-label={t("tools.runYourOwn.removeKey")}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button onClick={() => createMutation.mutate()} disabled={!canSubmit || createMutation.isPending}>
            {createMutation.isPending ? t("tools.runYourOwn.adding") : t("tools.runYourOwn.checkContinue")}
          </Button>
          <span className="text-xs text-muted-foreground">
            {t("tools.runYourOwn.registerHint")}
          </span>
        </div>
      </div>

      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground">{t("tools.runYourOwn.yourTools")}</h3>
        {templates.isLoading ? (
          <LoadingState />
        ) : templates.isError ? (
          <ErrorState error={templates.error} onRetry={() => templates.refetch()} />
        ) : adminTemplates.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("tools.runYourOwn.empty")}</p>
        ) : (
          <div className="overflow-hidden rounded-lg border border-border">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40 text-left text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                  <th className="px-4 py-2.5">{t("tools.runYourOwn.name")}</th>
                  <th className="px-4 py-2.5">{t("tools.runYourOwn.command")}</th>
                  <th className="px-4 py-2.5">{t("tools.runYourOwn.keysColumn")}</th>
                  <th className="px-4 py-2.5">{t("tools.runYourOwn.added")}</th>
...
function RunYourOwnRow({
  ...
  const disableMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("tools.runYourOwn.toasts.turnedOff"), tone: "success" });
      qc.invalidateQueries({ queryKey: queryKeys.tools.stdioTemplates(companyId) });
    },
    onError: (error) => {
      pushToast({
        title: t("tools.runYourOwn.toasts.turnOffFailed"),
  ...
  return (
    <tr className="border-b border-border last:border-0">
      <td className="px-4 py-3">
        ...
        {disabled ? <Badge variant="outline">{t("common.off")}</Badge> : null}
      </td>
      <td className="px-4 py-3">
        <code className="font-mono text-(length:--text-micro) text-muted-foreground">{fullCommand || "—"}</code>
      </td>
      <td className="px-4 py-3 text-muted-foreground">
        {template.envKeys.length > 0 ? template.envKeys.join(", ") : t("common.none")}
      </td>
      <td className="px-4 py-3">
