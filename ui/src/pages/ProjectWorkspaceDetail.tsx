...
function parseRuntimeConfigJson(value: string, t?: TFunction) {
  const trimmed = value.trim();
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t
          ? t("pages.projectWorkspaceDetail.errors.runtimeJsonObject", { defaultValue: "Workspace commands JSON must be a JSON object." })
    ...
    return {
      ok: false as const,
      error: t ? t("pages.projectWorkspaceDetail.errors.invalidJson", { defaultValue: "Invalid JSON." }) : "Invalid JSON.",
    };
  }
}
...
function validateWorkspaceForm(form: WorkspaceFormState, t: TFunction) {
  ...
  if (form.sourceType === "remote_managed") {
    if (!remoteWorkspaceRef && !repoUrl) {
      return t("pages.projectWorkspaceDetail.errors.remoteRequired", { defaultValue: "Remote-managed workspaces require a remote workspace ref or repo URL." });
    }
  } else if (!cwd && !repoUrl) {
    return t("pages.projectWorkspaceDetail.errors.pathOrRepoRequired", { defaultValue: "Workspace requires at least one local path or repo URL." });
  }

  if (cwd && (form.sourceType === "local_path" || form.sourceType === "non_git_path") && !isAbsolutePath(cwd)) {
    return t("pages.projectWorkspaceDetail.errors.localPathAbsolute", { defaultValue: "Local workspace path must be absolute." });
  }
  ...
  if (repoUrl) {
    try {
      ...
      return t("pages.projectWorkspaceDetail.errors.repoUrlInvalid", { defaultValue: "Repo URL must be a valid URL." });
    }
  }
...
export function ProjectWorkspaceDetail() {
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      ...
      { label: t("nav.workspaces", { defaultValue: "Workspaces" }), href: `/projects/${canonicalProjectRef}/workspaces` },
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(formatApiError(error, t, t("pages.projectWorkspaceDetail.errors.saveFailed", { defaultValue: "Failed to save workspace." })));
    },
  });
  ...
  const setPrimaryWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(formatApiError(error, t, t("pages.projectWorkspaceDetail.errors.updateFailed", { defaultValue: "Failed to update workspace." })));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("pages.projectWorkspaceDetail.runtime.jobCompleted", { defaultValue: "Workspace job completed." })
          : request.action === "stop"
            ? t("pages.projectWorkspaceDetail.runtime.serviceStopped", { defaultValue: "Workspace service stopped. Issue execution is not paused." })
            : request.action === "restart"
              ? t("pages.projectWorkspaceDetail.runtime.serviceRestarted", { defaultValue: "Workspace service restarted. Issue execution is not paused." })
              : t("pages.projectWorkspaceDetail.runtime.serviceStarted", { defaultValue: "Workspace service started." }),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(formatApiError(error, t, t("pages.projectWorkspaceDetail.errors.controlFailed", { defaultValue: "Failed to control workspace commands." })));
    },
  });

  if (projectQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("pages.projectWorkspaceDetail.loading", { defaultValue: "Loading workspace..." })}</p>;
  if (projectQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {formatApiError(projectQuery.error, t, t("pages.projectWorkspaceDetail.errors.loadFailed", { defaultValue: "Failed to load workspace" }))}
      </p>
    );
  }
  if (!project || !workspace || !form || !initialState) {
    return <p className="text-sm text-muted-foreground">{t("pages.projectWorkspaceDetail.notFound", { defaultValue: "Workspace not found for this project." })}</p>;
  }
  ...
  const sourceTypeOption = SOURCE_TYPE_OPTIONS.find((option) => option.value === form.sourceType) ?? null;
  const sourceTypeDescription = sourceTypeOption
    ? t(`pages.projectWorkspaceDetail.sourceTypeOptions.${sourceTypeOption.value}.description`, {
        defaultValue: sourceTypeOption.description,
      })
    : null;
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link to={`/projects/${canonicalProjectRef}/workspaces`}>
            ...
            {t("pages.projectWorkspaceDetail.backToWorkspaces", { defaultValue: "Back to workspaces" })}
          </Link>
        </Button>
      </div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-2">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
            {t("components.projectWorkspaceSummary.projectWorkspace", { defaultValue: "Project workspace" })}
          </div>
        ...
        {!workspace.isPrimary ? (
          ...
          >
            ...
            {t("pages.projectWorkspaceDetail.makePrimary", { defaultValue: "Make primary" })}
          </Button>
        ) : (
          <div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300 sm:max-w-sm">
            ...
            {t("pages.projectWorkspaceDetail.primaryWorkspace", { defaultValue: "This is the project's primary codebase workspace." })}
          </div>
        )}
      </div>
      ...
      <Tabs value={activeTab} onValueChange={(value) => handleTabChange(value as ProjectWorkspaceTab)}>
        <PageTabBar
          items={tabItems.map((item) => ({
            value: item.value,
            label: item.value === "configuration"
              ? t("pages.projectDetail.tabs.configuration", { defaultValue: "Configuration" })
              : item.label,
          }))}
          align="start"
          value={activeTab}
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("pages.projectWorkspaceDetail.description", {
                defaultValue: "Configure the concrete workspace Paperclip attaches to this project. These values drive per-workspace checkout behavior, default runtime services for child execution workspaces, and let you override setup or cleanup commands when one workspace needs special handling.",
              })}
            </p>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label={t("pages.projectWorkspaceDetail.workspaceName", { defaultValue: "Workspace name" })}>
                <input
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                  value={form.name}
                  onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                  placeholder={t("pages.projectWorkspaceDetail.workspaceName", { defaultValue: "Workspace name" })}
                />
              </Field>

              <Field label={t("pages.projectWorkspaceDetail.visibility", { defaultValue: "Visibility" })}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                >
                  {VISIBILITY_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>
                      {t(`pages.projectWorkspaceDetail.visibilityOptions.${option.value}`, { defaultValue: option.label })}
                    </option>
                  ))}
                </select>
              </Field>
            ...
            <div className="mt-4 grid gap-4">
              <Field label={t("pages.projectWorkspaceDetail.sourceType", { defaultValue: "Source type" })} hint={sourceTypeDescription ?? undefined}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                >
                  {SOURCE_TYPE_OPTIONS.map((option) => (
                    <option key={option.value} value={option.value}>
                      {t(`pages.projectWorkspaceDetail.sourceTypeOptions.${option.value}.label`, { defaultValue: option.label })}
                    </option>
                  ))}
                </select>
              </Field>
              ...
              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <Field label={t("pages.projectWorkspaceDetail.localPath", { defaultValue: "Local path" })}>
                  <input
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("pages.projectWorkspaceDetail.repoUrl", { defaultValue: "Repo URL" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.repoUrl}
                ...
                <Field label={t("pages.projectWorkspaceDetail.repoRef", { defaultValue: "Repo ref" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.repoRef}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("pages.projectWorkspaceDetail.defaultRef", { defaultValue: "Default ref" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                ...
                <Field label={t("pages.projectWorkspaceDetail.sharedWorkspaceKey", { defaultValue: "Shared workspace key" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("pages.projectWorkspaceDetail.remoteProvider", { defaultValue: "Remote provider" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                ...
                <Field label={t("pages.projectWorkspaceDetail.remoteWorkspaceRef", { defaultValue: "Remote workspace ref" })}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field
                  label={t("pages.projectWorkspaceDetail.setupCommand", { defaultValue: "Setup command" })}
                  hint={t("pages.projectWorkspaceDetail.setupCommandHint", { defaultValue: "Runs when this workspace needs custom bootstrap" })}
                >
                  <textarea
                ...
                <Field
                  label={t("pages.projectWorkspaceDetail.cleanupCommand", { defaultValue: "Cleanup command" })}
                  hint={t("pages.projectWorkspaceDetail.cleanupCommandHint", { defaultValue: "Runs before project-level execution workspace teardown" })}
                >
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
              ...
              <details className="rounded-xl border border-dashed border-border/70 bg-background px-3 py-3">
                <summary className="cursor-pointer text-sm font-medium">
                  {t("pages.projectWorkspaceDetail.advancedRuntimeJson", { defaultValue: "Advanced runtime JSON" })}
                </summary>
                <p className="mt-2 text-sm text-muted-foreground">
                  {t("pages.projectWorkspaceDetail.advancedRuntimeJsonDescription", {
                    defaultValue: "Paperclip derives Services and Jobs from this JSON. Prefer editing named commands first; use raw JSON for advanced lifecycle, port, readiness, or environment settings.",
                  })}
                </p>
                <div className="mt-3">
                  <Field
                    label={t("pages.projectWorkspaceDetail.workspaceCommandsJson", { defaultValue: "Workspace commands JSON" })}
                    hint={t("pages.projectWorkspaceDetail.workspaceCommandsJsonHint", { defaultValue: "Execution workspaces inherit this config unless they override it. Legacy `services` arrays still work, but `commands` supports both services and jobs." })}
                  >
                    <textarea
            ...
            <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              <Button className="w-full sm:w-auto" disabled={!isDirty || updateWorkspace.isPending} onClick={saveChanges}>
                ...
                {t("pages.projectWorkspaceDetail.saveChanges", { defaultValue: "Save changes" })}
              </Button>
              <Button
                variant="outline"
                className="w-full sm:w-auto"
              ...
              >
                {t("pages.projectWorkspaceDetail.reset", { defaultValue: "Reset" })}
              </Button>
              ...
              {!errorMessage && !isDirty ? (
                <p className="text-sm text-muted-foreground">
                  {t("pages.projectWorkspaceDetail.noUnsavedChanges", { defaultValue: "No unsaved changes." })}
                </p>
              ) : null}
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                {t("pages.projectWorkspaceDetail.workspaceFacts", { defaultValue: "Workspace facts" })}
              </div>
              <h2 className="text-lg font-semibold">{t("pages.projectWorkspaceDetail.currentState", { defaultValue: "Current state" })}</h2>
            </div>
            <Separator className="my-4" />
            <DetailRow label={t("pages.projectWorkspaceDetail.project", { defaultValue: "Project" })}>
              <Link to={`/projects/${canonicalProjectRef}`} className="hover:underline">{project.name}</Link>
            </DetailRow>
            <DetailRow label={t("pages.projectWorkspaceDetail.workspaceId", { defaultValue: "Workspace ID" })}>
              <span className="break-all font-mono text-xs">{workspace.id}</span>
            </DetailRow>
            <DetailRow label={t("pages.projectWorkspaceDetail.localPath", { defaultValue: "Local path" })}>
              <span className="break-all font-mono text-xs">{workspace.cwd ?? t("common.none", { defaultValue: "None" })}</span>
            </DetailRow>
            <DetailRow label={t("pages.projectWorkspaceDetail.repo", { defaultValue: "Repo" })}>
            ...
            <DetailRow label={t("pages.projectWorkspaceDetail.defaultRef", { defaultValue: "Default ref" })}>{workspace.defaultRef ?? t("common.none", { defaultValue: "None" })}</DetailRow>
            <DetailRow label={t("components.projectProperties.updated", { defaultValue: "Updated" })}>{new Date(workspace.updatedAt).toLocaleString()}</DetailRow>
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                  {t("pages.projectWorkspaceDetail.workspaceCommands", { defaultValue: "Workspace commands" })}
                </div>
                <h2 className="text-lg font-semibold">{t("pages.projectWorkspaceDetail.servicesAndJobs", { defaultValue: "Services and jobs" })}</h2>
                <p className="text-sm text-muted-foreground">
                  {t("pages.projectWorkspaceDetail.servicesAndJobsDescription", {
                    defaultValue: "Long-running services stay supervised here, while one-shot jobs run on demand against this workspace. Execution workspaces inherit this config unless they override it.",
                  })}
                </p>
              </div>
            </div>
            <WorkspaceRuntimeControls
              ...
              serviceEmptyMessage={
                workspace.runtimeConfig?.workspaceRuntime
                  ? t("pages.projectWorkspaceDetail.noServicesStarted", { defaultValue: "No services have been started for this workspace yet." })
                  : t("pages.projectWorkspaceDetail.noWorkspaceCommandConfig", { defaultValue: "No workspace command config is defined for this workspace yet." })
              }
              jobEmptyMessage={t("pages.projectWorkspaceDetail.noJobsConfigured", { defaultValue: "No one-shot jobs are configured for this workspace yet." })}
              disabledHint={t("pages.projectWorkspaceDetail.disabledHint", { defaultValue: "Project workspaces need a working directory before local commands can run, and services also need runtime config." })}
      ...
      {isProjectWorkspacePluginTab(activeTab) ? (
        activePluginTab ? (
          ...
          <div className="rounded-lg border border-dashed border-border bg-background px-4 py-8 text-sm text-muted-foreground">
            {pluginDetailSlotsError ? pluginDetailSlotsError : t("pages.projectWorkspaceDetail.loadingPlugin", { defaultValue: "Loading workspace plugin..." })}
          </div>
	        ) : (
	          <MissingPluginTabPlaceholder
	            defaultTabHref={`${projectWorkspaceUrl(project, routeWorkspaceId)}?tab=configuration`}
	            defaultTabLabel={t("pages.projectWorkspaceDetail.backToConfiguration", { defaultValue: "Back to configuration" })}
	          />
        )
      ) : null}
    </div>
