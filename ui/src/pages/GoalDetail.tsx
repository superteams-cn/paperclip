...
export function GoalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("nav.goals", { defaultValue: "Goals" }), href: "/goals" },
      { label: goal?.title ?? goalId ?? t("pages.goals.goal", { defaultValue: "Goal" }) }
    ]);
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {t("pages.goals.subGoalsCount", { defaultValue: "Sub-Goals ({{count}})", count: childGoals.length })}
          </TabsTrigger>
          <TabsTrigger value="projects">
            {t("pages.goals.projectsCount", { defaultValue: "Projects ({{count}})", count: linkedProjects.length })}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          <div className="flex items-center justify-start">
            ...
            >
              ...
              {t("pages.goals.subGoal", { defaultValue: "Sub Goal" })}
            </Button>
          </div>
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("pages.goals.noSubGoals", { defaultValue: "No sub-goals." })}</p>
          ) : (
        ...
        <TabsContent value="projects" className="mt-4">
          {linkedProjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("pages.goals.noLinkedProjects", { defaultValue: "No linked projects." })}</p>
          ) : (
