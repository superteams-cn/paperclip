  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        ...
        <CardDescription>
          {t("pages.costs.financeLedgerDescription", {
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={t("pages.costs.totalEventsInRange", {
            defaultValue_one: "{{count}} total event in range",
            defaultValue_other: "{{count}} total events in range",
            count: eventCount,
          })}
          icon={ArrowUpRight}
        />
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("nav.costs", { defaultValue: "Costs" }) }]);
  }, [setBreadcrumbs, t]);
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("nav.costs", { defaultValue: "Costs" })}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("pages.costs.description", {
            ...
            <div className="flex flex-wrap items-center gap-2">
              {PRESET_KEYS.map((key) => (
                ...
                >
                  {t(`pages.costs.dateRange.${key}`, {
                    defaultValue: PRESET_LABELS[key],
                  })}
                </Button>
              ))}
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={t("pages.costs.tokensAcrossEvents", {
            ...
            <MetricTile
              ...
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ...
                  : t("pages.costs.open", { defaultValue: "Open" })
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("pages.costs.pausedSummary", {
                    defaultValue: "{{agents}} agents paused · {{projects}} projects paused",
                    agents: budgetData?.pausedAgentCount ?? 0,
                    projects: budgetData?.pausedProjectCount ?? 0,
                  })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("pages.costs.ofBudget", {
                      defaultValue: "{{spent}} of {{budget}}",
            ...
            <MetricTile
              ...
              subtitle={t("pages.costs.financeNetSubtitle", {
            ...
            <MetricTile
              ...
              subtitle={t("pages.costs.estimatedInRange", {
                defaultValue: "{{amount}} estimated in range",
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("pages.projectDetail.tabs.overview", { defaultValue: "Overview" })}</TabsTrigger>
          <TabsTrigger value="budgets">{t("pages.costs.tabs.budgets", { defaultValue: "Budgets" })}</TabsTrigger>
          <TabsTrigger value="providers">{t("pages.costs.tabs.providers", { defaultValue: "Providers" })}</TabsTrigger>
          <TabsTrigger value="billers">{t("pages.costs.tabs.billers", { defaultValue: "Billers" })}</TabsTrigger>
          <TabsTrigger value="finance">{t("pages.costs.tabs.finance", { defaultValue: "Finance" })}</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">
              {t("pages.costs.selectDateRange", { defaultValue: "Select a start and end date to load data." })}
            </p>
          ) : showOverviewLoading ? (
            <PageSkeleton variant="costs" />
          ) : overviewError ? (
            <p className="text-sm text-destructive">{formatApiError(overviewError, t)}</p>
          ) : (
            <>
              {activeBudgetIncidents.length > 0 ? (
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
	                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
	                            ? t("pages.costs.budgetAmount", { defaultValue: "Budget {{amount}}", amount: formatCents(spendData.summary.budgetCents) })
	                            : t("pages.costs.unlimitedBudget", { defaultValue: "Unlimited budget" })}
                        </div>
                      </div>
                      <div className="border border-border px-4 py-3 text-right">
	                        <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("pages.costs.usage", { defaultValue: "usage" })}</div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
	                          {t("pages.costs.monthlyBudgetConsumed", {
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
	                      <p className="text-sm text-muted-foreground">{t("pages.costs.noCostEventsYet", { defaultValue: "No cost events yet." })}</p>
                    ) : (
                ...
                <div className="space-y-4">
	                  <Card>
	                    <CardHeader className="px-5 pt-5 pb-2">
	                      ...
	                      <CardDescription>{t("pages.costs.byProjectDescription", { defaultValue: "Run costs attributed through project-linked issues." })}</CardDescription>
	                    </CardHeader>
	                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
	                      {(spendData?.byProject.length ?? 0) === 0 ? (
	                        <p className="text-sm text-muted-foreground">{t("pages.costs.noProjectRunCosts", { defaultValue: "No project-attributed run costs yet." })}</p>
	                      ) : (
	                  ...
	                  <FinanceTimelineCard
	                    ...
	                    emptyMessage={t("pages.costs.noFinanceEventsYet", { defaultValue: "No finance events yet. Add account-level charges once biller invoices or credits land." })}
	                  />
                </div>
              </div>
            </>
        ...
        <TabsContent value="budgets" className="mt-4 space-y-4">
          {budgetLoading ? (
            ...
            <p className="text-sm text-destructive">{formatApiError(budgetError, t)}</p>
          ) : (
            <>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
	                    <h2 className="text-lg font-semibold">{t("pages.costs.activeIncidents", { defaultValue: "Active incidents" })}</h2>
	                    <p className="text-sm text-muted-foreground">
	                      {t("pages.costs.resolveHardStops", { defaultValue: "Resolve hard stops here by raising the budget or explicitly keeping the scope paused." })}
	                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">{t(`pages.costs.budgetScopes.${scopeType}.title`, { defaultValue: `${scopeType} budgets` })}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("pages.costs.budgetScopes.company.description", { defaultValue: "Company-wide monthly policy." })
                            : scopeType === "agent"
                              ? t("pages.costs.budgetScopes.agent.description", { defaultValue: "Recurring monthly spend policies for individual agents." })
                              : t("pages.costs.budgetScopes.project.description", { defaultValue: "Lifetime spend policies for execution-bound projects." })}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
        ...
        <TabsContent value="finance" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            ...
            <p className="text-sm text-destructive">{formatApiError(financeError, t)}</p>
          ) : (
            <>
              <FinanceSummaryCard
                debitCents={financeData?.summary.debitCents ?? 0}
