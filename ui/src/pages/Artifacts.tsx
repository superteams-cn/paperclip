...
export const ARTIFACT_KIND_FILTERS: { value: ArtifactKindFilter; label: string }[] = [
  { value: "all", label: t("pages.artifacts.kind.all", { defaultValue: "All" }) },
  { value: "image", label: t("pages.artifacts.kind.image", { defaultValue: "Images" }) },
  { value: "video", label: t("pages.artifacts.kind.video", { defaultValue: "Videos" }) },
  { value: "document", label: t("pages.artifacts.kind.document", { defaultValue: "Documents" }) },
  { value: "text", label: t("pages.artifacts.kind.text", { defaultValue: "Text" }) },
  { value: "file", label: t("pages.artifacts.kind.file", { defaultValue: "Files" }) },
];

export const ARTIFACT_GROUP_OPTIONS: { value: ArtifactGroupBy; label: string }[] = [
  { value: "none", label: t("pages.artifacts.groupOption.none", { defaultValue: "None" }) },
  { value: "task", label: t("pages.artifacts.groupOption.task", { defaultValue: "Task" }) },
  { value: "parent_task", label: t("pages.artifacts.groupOption.parentTask", { defaultValue: "Parent task" }) },
];
...
export function artifactGroupByLabel(value: ArtifactGroupBy): string {
  return (
    ...
    t("pages.artifacts.groupOption.none", { defaultValue: "None" })
  );
}
...
export function Artifacts() {
  ...
  useEffect(() => {
    if (viewingSelectedStack && selectedGroup) {
      setBreadcrumbs([
        { label: t("pages.artifacts.breadcrumb", { defaultValue: "Artifacts" }), href: "/artifacts" },
        { label: `${selectedGroup.issue.identifier} · ${selectedGroup.title}` },
      ]);
    } else {
      setBreadcrumbs([{ label: t("pages.artifacts.breadcrumb", { defaultValue: "Artifacts" }) }]);
    }
  ...
  if (!selectedCompanyId) {
    return (
      <EmptyState
        icon={Package}
        message={t("pages.artifacts.selectCompany", {
          defaultValue: "Select a company to view artifacts.",
        })}
      />
    );
  }
  ...
  const emptyMessage = showGroupCards
    ? searching
      ? t("pages.artifacts.emptyStacksSearch", {
          defaultValue: "No artifact stacks match this search.",
        })
      : t("pages.artifacts.emptyStacks", { defaultValue: "No artifact stacks yet." })
    : searching
      ? t("pages.artifacts.emptySearch", { defaultValue: "No artifacts match this search." })
      : viewingSelectedStack
        ? t("pages.artifacts.emptyStackFiltered", {
            defaultValue: "No artifacts in this stack match the current filters.",
          })
        : kind === "all"
          ? t("pages.artifacts.emptyAll", {
              defaultValue: "No artifacts yet. Outputs attached to issues will appear here.",
            })
          : t("pages.artifacts.emptyKind", { defaultValue: "No artifacts of this type yet." });
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input
            ...
            aria-label={t("pages.artifacts.searchAriaLabel", { defaultValue: "Search artifacts" })}
          ...
          {draftQuery.length > 0 ? (
            <button
              ...
              aria-label={t("pages.artifacts.clearSearch", { defaultValue: "Clear artifact search" })}
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                variant="outline"
                size="icon"
                aria-label={t("pages.artifacts.groupAriaLabel", {
                  defaultValue: "Group artifacts (currently {{label}})",
                  label: artifactGroupByLabel(groupBy),
                })}
                title={t("pages.artifacts.groupTitle", { defaultValue: "Group artifacts" })}
            ...
            <DropdownMenuContent align="end" className="w-44">
              <DropdownMenuLabel>
                {t("pages.artifacts.groupByLabel", { defaultValue: "Group by" })}
              </DropdownMenuLabel>
          ...
          <div
            ...
            role="tablist"
            aria-label={t("pages.artifacts.filterByType", { defaultValue: "Filter artifacts by type" })}
          >
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div ref={loadMoreRef} className="flex min-h-10 items-center justify-center pb-2 text-xs text-muted-foreground">
            {isFetchingNextPage
              ? t("pages.artifacts.loadingMore", { defaultValue: "Loading more artifacts..." })
              : hasNextPage
                ? null
                : isFetching
                  ? t("pages.artifacts.updating", { defaultValue: "Updating artifacts..." })
                  : null}
          </div>
        </>
      )}
