...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  const title = scope === "invalid_company_prefix"
    ? t("pages.notFound.companyTitle", { defaultValue: "Company not found" })
    : t("pages.notFound.pageTitle", { defaultValue: "Page not found" });
  const description =
    scope === "invalid_company_prefix"
      ? t("pages.notFound.companyDescription", {
        defaultValue: "No company matches prefix \"{{prefix}}\".",
        prefix: normalizedPrefix ?? t("common.unknown", { defaultValue: "unknown" }),
      })
      : t("pages.notFound.pageDescription", { defaultValue: "This route does not exist." });
    ...
    <div className="mx-auto max-w-2xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("pages.notFound.requestedPath", { defaultValue: "Requested path:" })}{" "}
