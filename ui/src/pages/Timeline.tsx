...
function rangeError(range: DateRangeState): string | null {
  if (!range.fromDate || !range.toDate)
    return t("pages.timeline.rangeErrorMissing", { defaultValue: "Choose a start and end date." });
  if (!rangeWindow(range))
    return t("pages.timeline.rangeErrorOrder", { defaultValue: "Start date must be before end date." });
  return null;
}
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 border-b border-border px-3.5 py-2 text-xs text-muted-foreground">
      <span className="flex items-center gap-1.5">
        ...
        {t("pages.timeline.legendDelegated", { defaultValue: "Delegated" })}
      </span>
      <span className="flex items-center gap-1.5">
        <span className="h-2.5 w-4 rounded-sm" style={{ backgroundColor: TIMELINE_COLORS.automation }} />
        {t("pages.timeline.legendAutomation", { defaultValue: "Automation" })}
      </span>
      <span className="flex items-center gap-1.5">
        <span
        ...
        />
        {t("pages.timeline.legendCancelled", { defaultValue: "Cancelled" })}
      </span>
      <span className="flex items-center gap-1.5">
        <span className="h-3.5 w-0.5" style={{ backgroundColor: TIMELINE_COLORS.now }} />
        {t("pages.timeline.legendNow", { defaultValue: "Now" })}
      </span>
    </div>
  );
}
...
function TimelineSummaryStats({
  ...
  const stats: { label: string; value: string; icon: LucideIcon }[] = [
    { label: t("pages.timeline.statRuns", { defaultValue: "Runs" }), value: formatInteger(summary.runs), icon: GanttChartSquare },
    { label: t("pages.timeline.statAgents", { defaultValue: "Agents" }), value: formatInteger(summary.agents), icon: Bot },
    { label: t("pages.timeline.statRunTime", { defaultValue: "Run time" }), value: formatDuration(0, summary.activeMs), icon: Clock3 },
    {
      label: t("pages.timeline.statTokensUsed", { defaultValue: "Tokens used" }),
      value:
        summary.totalTokens > 0
          ? formatCompactInteger(summary.totalTokens)
          : t("pages.timeline.statNotTracked", { defaultValue: "Not tracked" }),
      icon: Coins,
    },
  ];
...
export function Timeline() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.timeline.breadcrumb", { defaultValue: "Timeline" }) }]);
  }, [setBreadcrumbs, t]);
    ...
    return (
      <>
        ...
        <EmptyState
          ...
          message={t("pages.timeline.emptyNoCompany", {
            defaultValue: "Select a company to view its work timeline.",
          })}
        />
      </>
    );
  ...
  const header = (
    <div className="flex items-center gap-2">
      ...
      <h1 className="text-3xl font-semibold tracking-tight">
        {t("pages.timeline.pageTitle", { defaultValue: "Work Timeline" })}
      </h1>
    </div>
  );
  ...
  const rangeControls = (
    <label className="flex min-w-0 flex-wrap items-center gap-2 text-xs text-muted-foreground">
      {t("pages.timeline.rangeLabel", { defaultValue: "Range" })}
      <Segmented
        value={rangePreset}
        onChange={(preset) => {
        ...
        options={[
          { value: "today", label: t("pages.timeline.presetToday", { defaultValue: "Today" }) },
          { value: "7d", label: t("pages.timeline.preset7d", { defaultValue: "7 days" }) },
          { value: "30d", label: t("pages.timeline.preset30d", { defaultValue: "30 days" }) },
        ]}
      />
      <Input
        type="date"
        ...
        aria-label={t("pages.timeline.startDateAria", { defaultValue: "Timeline start date" })}
      />
      <span>{t("pages.timeline.dateRangeTo", { defaultValue: "to" })}</span>
      <Input
        type="date"
        ...
        className="h-8 w-(--sz-150px) text-xs"
        aria-label={t("pages.timeline.endDateAria", { defaultValue: "Timeline end date" })}
      />
    </label>
  );
  ...
  const toolbar = (
    <div className="flex flex-wrap items-start gap-3">
      ...
      <div
        ...
        aria-label={t("pages.timeline.zoomControlsAria", { defaultValue: "Timeline zoom controls" })}
      >
        <Button
          type="button"
          variant="outline"
        ...
        <Button
          ...
          size="icon-xs"
          onClick={resetZoom}
          aria-label={t("pages.timeline.zoomReset", { defaultValue: "Reset zoom" })}
          title={t("pages.timeline.zoomReset", { defaultValue: "Reset zoom" })}
        >
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <EmptyState
          icon={GanttChartSquare}
          message={t("pages.timeline.errorLoad", {
      ...
      {data && !isLoading && !dateRangeError && (
        data.spans.length === 0 ? (
          <div className="space-y-3">
            <EmptyState
              icon={GanttChartSquare}
              message={t("pages.timeline.emptyNoActivity", { defaultValue: "No activity in this window." })}
            />
            <div className="flex flex-wrap items-center justify-end gap-3">
              {rangeControls}
            </div>
