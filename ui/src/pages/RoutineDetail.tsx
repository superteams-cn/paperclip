...
export function RoutineDetail() {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("pages.routineDetail.secrets.selectCompanyToCreate", { defaultValue: "Select a company to create secrets" }));
  ...
  const dirtyFields = useMemo<RoutineHistoryDirtyFieldDescriptor[]>(() => {
    ...
    if (editDraft.title !== routineDefaults.title) result.push({ key: "title", label: t("pages.routineDetail.dirtyFields.title", { defaultValue: "the title" }) });
    if (editDraft.description !== routineDefaults.description) {
      result.push({ key: "description", label: t("pages.routineDetail.dirtyFields.description", { defaultValue: "the description" }) });
    }
    if (editDraft.projectId !== routineDefaults.projectId) {
      result.push({ key: "projectId", label: t("pages.routineDetail.dirtyFields.project", { defaultValue: "the project" }) });
    }
    if (editDraft.assigneeAgentId !== routineDefaults.assigneeAgentId) {
      result.push({ key: "assigneeAgentId", label: t("pages.routineDetail.dirtyFields.defaultAgent", { defaultValue: "the default agent" }) });
    }
    if (editDraft.priority !== routineDefaults.priority) {
      result.push({ key: "priority", label: t("pages.routineDetail.dirtyFields.priority", { defaultValue: "the priority" }) });
    }
    if (editDraft.concurrencyPolicy !== routineDefaults.concurrencyPolicy) {
      result.push({ key: "concurrencyPolicy", label: t("pages.routineDetail.dirtyFields.concurrency", { defaultValue: "the concurrency policy" }) });
    }
    if (editDraft.catchUpPolicy !== routineDefaults.catchUpPolicy) {
      result.push({ key: "catchUpPolicy", label: t("pages.routineDetail.dirtyFields.catchUp", { defaultValue: "the catch-up policy" }) });
    }
    if (JSON.stringify(editDraft.variables) !== JSON.stringify(routineDefaults.variables)) {
      result.push({ key: "variables", label: t("pages.routineDetail.dirtyFields.variables", { defaultValue: "the variables" }) });
    }
    if (JSON.stringify(editDraft.env ?? null) !== JSON.stringify(routineDefaults.env ?? null)) {
      result.push({ key: "env", label: t("pages.routineDetail.dirtyFields.secrets", { defaultValue: "the secrets" }) });
    }
    return result;
  ...
  useEffect(() => {
    if (!routine) return;
    setBreadcrumbs([{ label: t("nav.routines", { defaultValue: "Routines" }), href: "/routines" }, { label: routine.title }]);
    if (!routineDefaults) return;
  ...
  const copySecretValue = useCallback(
    async (label: string, value: string) => {
      try {
        ...
        pushToast({ title: t("pages.routineDetail.toasts.copied", { defaultValue: "{{label}} copied", label }), tone: "success" });
      } catch (copyError) {
        pushToast({
          title: t("pages.routineDetail.toasts.copyFailed", { defaultValue: "Failed to copy {{label}}", label: label.toLowerCase() }),
          body: copyError instanceof Error ? copyError.message : t("pages.routineDetail.toasts.clipboardDenied", { defaultValue: "Clipboard access was denied." }),
          tone: "error",
        });
      }
    },
  ...
  const saveRoutine = useMutation({
    ...
    onError: (mutationError) => {
      if (mutationError instanceof ApiError && mutationError.status === 409) {
        ...
        pushToast({
          title: t("pages.routineDetail.toasts.routineChangedTitle", { defaultValue: "Routine changed" }),
          body: t("pages.routineDetail.toasts.routineChangedBody", { defaultValue: "Someone else updated this routine. Reload to see the latest revision." }),
          tone: "warn",
        });
        return;
      }
      pushToast({
        title: t("pages.routineDetail.toasts.saveFailedTitle", { defaultValue: "Failed to save routine" }),
        body: mutationError instanceof Error ? mutationError.message : t("pages.routineDetail.toasts.saveFailedBody", { defaultValue: "Paperclip could not save the routine." }),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("pages.routineDetail.toasts.runStarted", { defaultValue: "Routine run started" }), tone: "success" });
    ...
    onError: (runError) => {
      pushToast({
        title: t("pages.routines.toasts.runFailedTitle", { defaultValue: "Routine run failed" }),
        body: runError instanceof Error ? runError.message : t("pages.routines.toasts.runFailedBody", { defaultValue: "Paperclip could not start the routine run." }),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("pages.routineDetail.toasts.savedTitle", { defaultValue: "Routine saved" }),
        body: status === "paused"
          ? t("pages.routineDetail.toasts.automationPaused", { defaultValue: "Automation paused." })
          : t("pages.routineDetail.toasts.automationEnabled", { defaultValue: "Automation enabled." }),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (statusError) => {
      pushToast({
        title: t("pages.routines.toasts.updateFailedTitle", { defaultValue: "Failed to update routine" }),
        body: statusError instanceof Error ? statusError.message : t("pages.routines.toasts.updateFailedBody", { defaultValue: "Paperclip could not update the routine." }),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: t("pages.routineDetail.toasts.webhookCreated", { defaultValue: "Webhook trigger created" }),
          entries: [{ webhookUrl: result.secretMaterial.webhookUrl, webhookSecret: result.secretMaterial.webhookSecret }],
        });
      } else {
        pushToast({
          title: t("pages.routineDetail.toasts.triggerAdded", { defaultValue: "Trigger added" }),
          body: t("pages.routineDetail.toasts.scheduleSaved", { defaultValue: "The routine schedule was saved." }),
          tone: "success",
        });
      }
      await Promise.all([
    ...
    onError: (triggerError) => {
      pushToast({
        title: t("pages.routineDetail.toasts.addTriggerFailedTitle", { defaultValue: "Failed to add trigger" }),
        body: triggerError instanceof Error ? triggerError.message : t("pages.routineDetail.toasts.addTriggerFailedBody", { defaultValue: "Paperclip could not create the trigger." }),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("pages.routineDetail.toasts.triggerSaved", { defaultValue: "Trigger saved" }),
        body: t("pages.routineDetail.toasts.cadenceSaved", { defaultValue: "The routine cadence update was saved." }),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        title: t("pages.routineDetail.toasts.updateTriggerFailedTitle", { defaultValue: "Failed to update trigger" }),
        body: triggerError instanceof Error ? triggerError.message : t("pages.routineDetail.toasts.updateTriggerFailedBody", { defaultValue: "Paperclip could not update the trigger." }),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("pages.routineDetail.toasts.triggerDeleted", { defaultValue: "Trigger deleted" }), tone: "success" });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.list(selectedCompanyId!) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.activity(selectedCompanyId!, routineId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        title: t("pages.routineDetail.toasts.deleteTriggerFailedTitle", { defaultValue: "Failed to delete trigger" }),
        body: triggerError instanceof Error ? triggerError.message : t("pages.routineDetail.toasts.deleteTriggerFailedBody", { defaultValue: "Paperclip could not delete the trigger." }),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("pages.routineDetail.toasts.webhookSecretRotated", { defaultValue: "Webhook secret rotated" }),
        entries: [{ webhookUrl: result.secretMaterial.webhookUrl, webhookSecret: result.secretMaterial.webhookSecret }],
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (triggerError) => {
      pushToast({
        title: t("pages.routineDetail.toasts.rotateSecretFailedTitle", { defaultValue: "Failed to rotate webhook secret" }),
        body: triggerError instanceof Error ? triggerError.message : t("pages.routineDetail.toasts.rotateSecretFailedBody", { defaultValue: "Paperclip could not rotate the webhook secret." }),
        tone: "error",
      });
    },
  });
      ...
      setSecretMessage({
        title:
          response.secretMaterials.length === 1
            ? t("pages.routineDetail.toasts.webhookRestored", { defaultValue: "Webhook trigger restored" })
            : t("pages.routineDetail.toasts.webhooksRestored", {
  ...
  if (error || !routine || !routineDefaults) {
    return (
      <EmptyState
        icon={AlertCircle}
        message={error instanceof Error ? error.message : t("pages.routineDetail.notFound", { defaultValue: "We couldn't load this routine." })}
      />
    );
  }
  ...
  const automationLabel =
    routine.status === "archived"
      ? t("components.routineList.state.archived", { defaultValue: "Archived" })
      : !routine.assigneeAgentId
        ? t("components.routineList.state.draft", { defaultValue: "Draft" })
        : automationEnabled
  ...
  const contextValue: RoutineDetailContextValue = {
    ...
    onToggleAutomation: () => {
      if (!automationEnabled && !routine.assigneeAgentId) {
        pushToast({
          title: t("pages.routines.toasts.defaultAgentRequiredTitle", { defaultValue: "Default agent required" }),
          body: t("pages.routines.toasts.defaultAgentRequiredBody", { defaultValue: "Set a default agent before enabling routine automation." }),
          tone: "warn",
        });
        return;
      }
        ...
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-background px-6">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <textarea
              ...
              placeholder={t("pages.routines.composer.titlePlaceholder", { defaultValue: "Routine title" })}
              rows={1}
          ...
          <div className="ml-auto flex shrink-0 items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <ToggleSwitch
                ...
                aria-label={automationEnabled ? t("pages.routineDetail.pauseAutomation", { defaultValue: "Pause automatic triggers" }) : t("pages.routineDetail.enableAutomation", { defaultValue: "Enable automatic triggers" })}
              />
