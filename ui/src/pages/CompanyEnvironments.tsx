...
function SupportMark({ supported }: { supported: boolean }) {
  ...
  return supported ? (
    ...
    <span className="text-muted-foreground">{t("common.no", { defaultValue: "No" })}</span>
  );
}
...
export function CompanyEnvironments() {
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("pages.companyEnvironments.selectCompany", { defaultValue: "Select a company to manage environments." })}</div>;
  }
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("pages.companyEnvironments.title", { defaultValue: "Company Environments" })}</h1>
        </div>
    ...
    <div className="max-w-5xl space-y-6" data-testid="company-settings-environments-section">
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("pages.companyEnvironments.description", {
      ...
      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("pages.companyEnvironments.supportMatrixHint", {
        ...
        {sandboxCreationEnabled ? (
          <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            {t("pages.companyEnvironments.installedSandboxProviders", { defaultValue: "Installed sandbox providers:" })}{" "}
            <span className="font-medium text-foreground">
              {discoveredPluginSandboxProviders.map((provider) => provider.displayName).join(", ")}
            </span>
            {t("pages.companyEnvironments.sandboxProvidersHint", {
              defaultValue: ". These are not adapter types. They back the Sandbox driver for adapters that support sandbox execution.",
            })}
          </div>
        ) : null}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("pages.companyEnvironments.supportCaption", { defaultValue: "Environment support by adapter" })}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("components.agentProperties.adapter", { defaultValue: "Adapter" })}</th>
                <th className="px-3 py-2 font-medium">{t("pages.companyEnvironments.drivers.local", { defaultValue: "Local" })}</th>
                <th className="px-3 py-2 font-medium">SSH</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("pages.companyEnvironments.sandboxViaPlugin", { defaultValue: "Sandbox via plugin" })}</th>
                ) : null}
              </tr>
            </thead>
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("pages.companyEnvironments.empty", { defaultValue: "No environments saved for this company yet." })}</div>
          ) : (
                    ...
                    <div className="space-y-1">
                      ...
                      {environment.driver === "ssh" ? (
                        <div className="text-xs text-muted-foreground">
                          {typeof environment.config.host === "string" ? environment.config.host : t("pages.companyEnvironments.sshHost", { defaultValue: "SSH host" })} ·{" "}
                          {typeof environment.config.username === "string" ? environment.config.username : t("common.user", { defaultValue: "User" })}
                        </div>
                        ...
                        <div className="text-xs text-muted-foreground">{t("pages.companyEnvironments.runsOnHost", { defaultValue: "Runs on this Paperclip host." })}</div>
                      )}
                    </div>
        ...
        <div className="border-t border-border/60 pt-4">
          <div className="mb-3 text-sm font-medium">
            {editingEnvironmentId
              ? t("pages.companyEnvironments.editEnvironment", { defaultValue: "Edit environment" })
              : t("pages.companyEnvironments.addEnvironment", { defaultValue: "Add environment" })}
          </div>
          <div className="space-y-3">
            <Field label={t("components.projectProperties.name", { defaultValue: "Name" })} hint={t("pages.companyEnvironments.nameHint", { defaultValue: "Operator-facing name for this execution target." })}>
              <input
            ...
            <Field label={t("components.projectProperties.description", { defaultValue: "Description" })} hint={t("pages.companyEnvironments.descriptionHint", { defaultValue: "Optional note about what this machine is for." })}>
              <input
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                type="text"
            ...
            <Field label={t("pages.companyEnvironments.driver", { defaultValue: "Driver" })} hint={t("pages.companyEnvironments.driverHint", { defaultValue: "Local runs on this host. SSH stores a remote machine target. Sandbox stores plugin-backed provider config on the shared environment seam." })}>
              <select
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              ...
              >
                ...
                {sandboxCreationEnabled || environmentForm.driver === "sandbox" ? (
                  <option value="sandbox">{t("pages.companyEnvironments.drivers.sandbox", { defaultValue: "Sandbox" })}</option>
                ) : null}
                <option value="local">{t("pages.companyEnvironments.drivers.local", { defaultValue: "Local" })}</option>
              </select>
            </Field>

            {environmentForm.driver === "ssh" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("pages.companyEnvironments.host", { defaultValue: "Host" })} hint={t("pages.companyEnvironments.hostHint", { defaultValue: "DNS name or IP address for the remote machine." })}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("pages.companyEnvironments.port", { defaultValue: "Port" })} hint={t("pages.companyEnvironments.portHint", { defaultValue: "Defaults to 22." })}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="number"
                    min={1}
                ...
                <Field label={t("pages.companyEnvironments.username", { defaultValue: "Username" })} hint={t("pages.companyEnvironments.usernameHint", { defaultValue: "SSH login user." })}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("pages.companyEnvironments.remoteWorkspacePath", { defaultValue: "Remote workspace path" })} hint={t("pages.companyEnvironments.remoteWorkspacePathHint", { defaultValue: "Absolute path that Paperclip will verify during SSH connection tests." })}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("pages.companyEnvironments.privateKey", { defaultValue: "Private key" })} hint={t("pages.companyEnvironments.privateKeyHint", { defaultValue: "Optional PEM private key. Leave blank to rely on the server's SSH agent or default keychain." })}>
                  <div className="space-y-2">
                    <select
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                ...
                <Field label={t("pages.companyEnvironments.knownHosts", { defaultValue: "Known hosts" })} hint={t("pages.companyEnvironments.knownHostsHint", { defaultValue: "Optional known_hosts block used when strict host key checking is enabled." })}>
                  <textarea
                    className="h-32 w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-xs font-mono outline-none"
                ...
                <div className="md:col-span-2">
                  <ToggleField
                    label={t("pages.companyEnvironments.strictHostKeyChecking", { defaultValue: "Strict host key checking" })}
                    hint={t("pages.companyEnvironments.strictHostKeyCheckingHint", { defaultValue: "Keep this on unless you deliberately want probe-time host key acceptance disabled." })}
            ...
            {environmentForm.driver === "sandbox" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("pages.costs.tabs.providers", { defaultValue: "Providers" })} hint={t("pages.companyEnvironments.providerHint", { defaultValue: "Installed run-capable sandbox provider plugins appear here." })}>
                  <select
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              ...
              {environmentForm.driver !== "local" ? (
                ...
                >
                  {draftEnvironmentProbeMutation.isPending
                    ? t("pages.companyEnvironments.testing", { defaultValue: "Testing..." })
                    : t("pages.companyEnvironments.testDraft", { defaultValue: "Test draft" })}
                </Button>
              ) : null}
              {environmentMutation.isError ? (
                <span className="text-xs text-destructive">
                  {formatApiError(environmentMutation.error, t, t("pages.companyEnvironments.saveFailed", { defaultValue: "Failed to save environment" }))}
                </span>
              ) : null}
